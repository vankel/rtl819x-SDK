...
unsigned int CheckDramFreq(void)                       //JSW:For 8196C
{
	...
	unsigned short usFreqVal[] = {106, 118, 131,143,156,168,181,193,212,237,262,287,312,337,362,387};
	usFreqBit = (0x00007C00 & (*(unsigned int*)0xb8000008)) >> 11 ;
#else
...
void setFSCR(unsigned char ucChip, unsigned int uiClkMhz, unsigned int uiRBO, unsigned int uiWBO, unsigned int uiTCS)
{
	...
	//SPI_REG_LOAD(SFCR, SFCR_SPI_CLK_DIV(7) | SFCR_RBO(uiRBO) | SFCR_WBO(uiWBO) | SFCR_SPI_TCS(uiTCS));//safe
...
unsigned int ComSrlCmd_ComWriteSector(unsigned char ucChip, unsigned int uiAddr, unsigned char* pucBuffer)
{
	...
	#if defined( CONFIG_RTL8881A_FT1_TEST) ||defined( CONFIG_RTL8881A_FT2_TEST)
		prom_printf("#");
	#else
		NDEBUG(".");
