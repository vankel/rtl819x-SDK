...
void Si3226x_Set_Ring_Cadence_ON(proslicChanType *pProslic, unsigned short msec, unsigned char timer_enable)
{
	...
	tmp = ReadReg (pProHW, pProslic->channel, RINGCON);
	if (timer_enable == 1)
		WriteReg(pProHW,pProslic->channel,RINGCON, tmp|0x10 );
	else if (timer_enable == 0)
		WriteReg(pProHW,pProslic->channel,RINGCON, tmp&0xEF);
}

void Si3226x_Set_Ring_Cadence_OFF(proslicChanType *pProslic, unsigned short msec, unsigned char timer_enable)
{
	unsigned int l_time, h_time, tmp;
	...
	tmp = ReadReg (pProHW, pProslic->channel, RINGCON);
	if (timer_enable == 1)
		WriteReg(pProHW,pProslic->channel,RINGCON, tmp|0x08 );
	else if (timer_enable == 0)
		WriteReg(pProHW,pProslic->channel,RINGCON, tmp&0xF7);
}

static unsigned char ring_cad[4] = {0};
static unsigned char ring_ctrl = 0;

static void Si3226x_Backup_Ring_Cadence(proslicChanType *pProslic)
{
	// backup ring on
	...
	// backup ring control
	ring_ctrl = ReadReg(pProHW,pProslic->channel,RINGCON);
}

static void Si3226x_Restore_Ring_Cadence(proslicChanType *pProslic)
{
	...
	WriteReg(pProHW,pProslic->channel,RINGCON, ring_ctrl);
}
...
void Si3226x_SendNTTCAR(proslicChanType *pProslic)
{
	int i;
	/************** backup the register ***************/
	Si3226x_Backup_Ring_Cadence(pProslic);

	/*********** To Create Short Ring *****************/
	Si3226x_Set_Ring_Cadence_ON(pProslic, 500, 1);
	Si3226x_Set_Ring_Cadence_OFF(pProslic, 500, 1);
...
*/
	...
	Si3226x_Restore_Ring_Cadence(pProslic);
...
void Si3226x_Set_Impendance_Silicon(proslicChanType *pProslic, unsigned short country, unsigned short impd /*reserve*/)
{
	...
	switch(country)
	{
		...
		case COUNTRY_UK:				// 370R+620R//310nF ??
			Si3226x_ZsynthSetup(pProslic, 5); 	// 270_750_150 (ETSI)
			break;
