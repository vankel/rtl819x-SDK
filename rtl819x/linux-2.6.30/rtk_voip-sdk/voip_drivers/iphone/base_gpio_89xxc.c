...
#if 0
#ifdef CONFIG_RTK_VOIP_GPIO_IPP_89XXC_V00
#define IPPHONE_PIN_HOOK	GPIO_ID(GPIO_PORT_F,0)	//input
...
void ipphone_hook_interrupt( int enable )
{
	...
	_rtl8954C_initGpioPin( IPPHONE_PIN_HOOK, GPIO_CONT_GPIO, GPIO_DIR_IN, int_type );
#endif
}
...
void init_ipphone_hook_detect( void )
{
  ...
  #else
	_rtl8954C_initGpioPin( IPPHONE_PIN_HOOK, GPIO_CONT_GPIO, GPIO_DIR_IN, GPIO_INT_DISABLE );
  #endif
#endif
}
...
#ifdef CONFIG_RTK_VOIP_GPIO_IPP_89XXC_V00
  #ifdef IPPHONE_PIN_HOOK
	_rtl8954C_getGpioDataBit( IPPHONE_PIN_HOOK, &data );
  #else
