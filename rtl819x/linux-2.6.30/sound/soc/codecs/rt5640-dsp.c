...
static const u16 rt5640_dsp_16[][2] = {
	{0x22c8, 0x0022}, {0x22fe, 0x0fa0}, {0x22ff, 0x3893}, {0x22fa, 0x2484},
	{0x2301, 0x0002},
};
#else //org
static const u16 rt5640_dsp_16[][2] = {
	{0x22c8, 0x0022}, {0x22fa, 0x2484}, {0x2301, 0x0002},
};
#endif
#define RT5640_DSP_16_NUM (sizeof(rt5640_dsp_16) / sizeof(rt5640_dsp_16[0]))

static const u16 rt5640_dsp_8[][2] = {
	{0x22c8, 0x0021}, {0x22fa, 0x2484}, {0x2301, 0x0000},
};

#define RT5640_DSP_8_NUM (sizeof(rt5640_dsp_8) / sizeof(rt5640_dsp_8[0]))

static const u16 rt5640_dsp_aec_ns_fens[][2] = {
	{0x22f8, 0x8005}, {0x2303, 0x0971}, {0x2304, 0x0312}, {0x2305, 0x0005},
	{0x2309, 0x0400}, {0x230a, 0x1b00}, {0x230c, 0x0100}, {0x230d, 0x0300},
	...
	{0x23ea, 0x7ff0}, {0x23ed, 0x0300}, {0x22fb, 0x0000},// {0x2300, 0x0004},
};
		...
		struct rt5640_dsp_param *param)
{
	...
	if (ret < 0) {
		printk(KERN_ERR "DSP is busy: %d\n", ret);
		goto err;
	}
	ret = snd_soc_write(codec, RT5640_DUMMY3, param->cmd_fmt);
	if (ret < 0) {
		printk(KERN_ERR "Failed to write cmd format: %d\n", ret);
		goto err;
	}
	ret = snd_soc_write(codec, RT5640_DSP_CTRL1, param->addr);
	if (ret < 0) {
		printk(KERN_ERR "Failed to write DSP addr reg: %d\n", ret);
		goto err;
	}
	ret = snd_soc_write(codec, RT5640_DSP_CTRL2, param->data);
	if (ret < 0) {
		printk(KERN_ERR "Failed to write DSP data reg: %d\n", ret);
		goto err;
	}
	...
	if (ret < 0) {
		printk(KERN_ERR "Failed to write DSP cmd reg: %d\n", ret);
		goto err;
	}
	mdelay(10);
	return 0;
	...
	struct snd_soc_codec *codec, unsigned int reg)
{
	...
	if (ret < 0) {
		printk(KERN_ERR "Failed to write fc = 0: %d\n", ret);
		goto err;
	}
	...
	if (ret < 0) {
		printk(KERN_ERR "Failed to read DSP data reg: %d\n", ret);
		goto err;
	}
	val_h = ret;
...
static const struct soc_enum rt5640_src_rxdp_enum = SOC_ENUM_SINGLE(
	RT5640_DSP_PATH1, RT5640_RXDP_SRC_SFT,
	ARRAY_SIZE(rt5640_src_rxdp_mode), rt5640_src_rxdp_mode);

static const char *rt5640_src_txdp_mode[] = {
	"Normal", "Multiplied by 3"};

static const struct soc_enum rt5640_src_txdp_enum = SOC_ENUM_SINGLE(
	RT5640_DSP_PATH1, RT5640_TXDP_SRC_SFT,
	ARRAY_SIZE(rt5640_src_txdp_mode), rt5640_src_txdp_mode);

/* DSP data select */
static const char *rt5640_dsp_data_select[] = {
	"Normal", "left copy to right", "right copy to left", "Swap"};

static const struct soc_enum rt5640_rxdc_data_enum = SOC_ENUM_SINGLE(
	RT5640_DSP_PATH2, RT5640_RXDC_SEL_SFT,
	ARRAY_SIZE(rt5640_dsp_data_select), rt5640_dsp_data_select);

static const struct soc_enum rt5640_rxdp_data_enum = SOC_ENUM_SINGLE(
	RT5640_DSP_PATH2, RT5640_RXDP_SEL_SFT,
	ARRAY_SIZE(rt5640_dsp_data_select), rt5640_dsp_data_select);

static const struct soc_enum rt5640_txdc_data_enum = SOC_ENUM_SINGLE(
	RT5640_DSP_PATH2, RT5640_TXDC_SEL_SFT,
	ARRAY_SIZE(rt5640_dsp_data_select), rt5640_dsp_data_select);

static const struct soc_enum rt5640_txdp_data_enum = SOC_ENUM_SINGLE(
	RT5640_DSP_PATH2, RT5640_TXDP_SEL_SFT,
	ARRAY_SIZE(rt5640_dsp_data_select), rt5640_dsp_data_select);
...
static const struct soc_enum rt5640_dsp_enum = SOC_ENUM_SINGLE(0, 0,
	ARRAY_SIZE(rt5640_dsp_mode), rt5640_dsp_mode);
...
static int rt5640_dsp_patch_3(struct snd_soc_codec *codec)
{
	...
	for(i = 0; i < RT5640_DSP_PATCH3_NUM; i++) {
		...
		ret = rt5640_dsp_write(codec, &param);
		if (ret < 0) {
			printk(KERN_ERR "Fail to patch Dsp: %d\n", ret);
			goto patch_err;
		}
	}
...
static int rt5640_dsp_patch(struct snd_soc_codec *codec)
{
	...
	if (ret < 0) {
		printk(KERN_ERR "Failed to power up DSP: %d\n", ret);
		goto patch_err;
	}
	...
	if (ret < 0) {
		printk(KERN_ERR "Failed to reset DSP: %d\n", ret);
		goto patch_err;
	}
	...
	if (ret < 0) {
		printk(KERN_ERR "Failed to recover DSP: %d\n", ret);
		goto patch_err;
	}
...
static void rt5640_do_dsp_patch(struct work_struct *work)
{
	...
	if (rt5640_dsp_patch(codec) < 0)
		printk(KERN_ERR "Patch DSP rom code Fail !!!\n");
}
...
static int rt5640_dsp_conf(struct snd_soc_codec *codec)
{
	...
	for(i = 0; i < RT5640_DSP_INIT_NUM; i++) {
		...
		ret = rt5640_dsp_write(codec, &param);
		if (ret < 0) {
			printk(KERN_ERR "Fail to config Dsp: %d\n", ret);
			goto conf_err;
		}
	}
...
static int rt5640_dsp_rate(struct snd_soc_codec *codec, int rate)
{
	...
	case 16000:
		...
		break;
	case 8000:
		rate_tab = rt5640_dsp_8;
		tab_num = RT5640_DSP_8_NUM;
		break;
	default:
		return -EINVAL;
		break;
...
rate_err:
	...
	printk(KERN_ERR "Fail to set rate %d parameters: %d\n", rate, ret);
	return ret;
}
...
mode_err:
	...
	printk(KERN_ERR "Fail to set mode %d parameters: %d\n", mode, ret);
	return ret;
}
...
#if 0
	for (i = 0; i < RT5640_DSP_AEC_NUM; i++) {

		val = rt5640_dsp_read(codec, rt5640_dsp_aec_ns_fens[i][0]);
		
//		printk("%04x: %04x\n", rt5640_dsp_aec_ns_fens[i][0], val);
	}
#endif
...
static int rt5640_dsp_add_controls(struct snd_soc_codec *codec)
{
	int err, i;
	
	for (i = 0; i < ARRAY_SIZE(rt5640_dsp_snd_controls); i ++) {

		err = snd_ctl_add(codec->card, 
			snd_soc_cnew(&rt5640_dsp_snd_controls[i],codec, NULL));
		if (err < 0)
			return err;	
	}	
...
int rt5640_dsp_probe(struct snd_soc_codec *codec)
{
	...
	rt5640_dsp_add_controls(codec);
	snd_soc_dapm_new_controls(codec, rt5640_dsp_dapm_widgets,
		ARRAY_SIZE(rt5640_dsp_dapm_widgets));
	snd_soc_dapm_add_routes(codec, rt5640_dsp_dapm_routes,
...
int rt56xx_dsp_ioctl_common(struct snd_hwdep *hw, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (copy_from_user(&rt56xx, _rt56xx, sizeof(rt56xx))) {
		printk(KERN_ERR "copy_from_user faild\n");
		return -EFAULT;
	}
//	printk(KERN_DEBUG "rt56xx.number=%d\n",rt56xx.number);
...
//			printk(KERN_DEBUG "codec = null\n");
	...
	default:
//		printk(KERN_INFO, "unsported dsp command\n");
		break;
	}
...
int rt5640_dsp_suspend(struct snd_soc_codec *codec, pm_message_t state)
{
	...
	ret = snd_soc_update_bits(codec, RT5640_DSP_CTRL3,
		RT5640_DSP_PD_PIN_MASK, RT5640_DSP_PD_PIN_LO);
	if (ret < 0) {
		printk(KERN_ERR "Failed to power down DSP: %d\n", ret);
		goto rsm_err;
	}
