...
static const struct soc_enum rt5670_src_rxdp_enum = SOC_ENUM_SINGLE(
	RT5670_DSP_PATH1,
	RT5670_RXDP_SRC_SFT, ARRAY_SIZE(rt5670_src_rxdp_mode), rt5670_src_rxdp_mode);

static const char * const rt5670_src_txdp_mode[] = {
	"Normal", "Multiplied by 3"
};

static const struct soc_enum rt5670_src_txdp_enum = SOC_ENUM_SINGLE(
	RT5670_DSP_PATH1,
	RT5670_TXDP_SRC_SFT, ARRAY_SIZE(rt5670_src_txdp_mode), rt5670_src_txdp_mode);
...
static const struct soc_enum rt5670_dsp_enum = SOC_ENUM_SINGLE(0, 0,
	ARRAY_SIZE(rt5670_dsp_mode), rt5670_dsp_mode);
...
static const struct snd_soc_dapm_widget rt5670_dsp_dapm_widgets[] = {
	SND_SOC_DAPM_MIXER_NAMED_CTL("Voice DSP", SND_SOC_NOPM,
...
static const struct snd_soc_dapm_route rt5670_dsp_dapm_routes[] = {
	...
	{"Voice DSP", NULL, "DSP Upstream"},
	{"Voice DSP", NULL, "DSP Downstream"},
	{"DSP DL Mux", "DSP", "Voice DSP"},
	{"DSP UL Mux", "DSP", "Voice DSP"},
};
...
static int rt5670_dsp_add_controls(struct snd_soc_codec *codec)
{
	int err, i;
	
	for (i = 0; i < ARRAY_SIZE(rt5670_dsp_snd_controls); i ++) {

		err = snd_ctl_add(codec->card, 
			snd_soc_cnew(&rt5670_dsp_snd_controls[i],codec, NULL));
		if (err < 0)
			return err;	
	}	
...
int rt5670_dsp_probe(struct snd_soc_codec *codec)
{
	...
	rt5670_dsp_add_controls(codec);
	snd_soc_dapm_new_controls(codec, rt5670_dsp_dapm_widgets,
			ARRAY_SIZE(rt5670_dsp_dapm_widgets));
	snd_soc_dapm_add_routes(codec, rt5670_dsp_dapm_routes,
