...
#include "rt5640.h"
#if (CONFIG_SND_SOC_RT5642_MODULE | CONFIG_SND_CODEC_RT5642)
#include "rt5640-dsp.h"
#endif

#define RT5640_VERSION "0.01 alsa 1.0.20"

#define RT5640_REG_RW 0 /* for debug */
#define RT5640_DET_EXT_MIC 0

#define ALC5640_I2C_ADDR	0x38

static struct snd_soc_codec *rt5640_codec;
static struct snd_soc_device *rt5640_socdev;
...
static int rt5640_volatile_register(unsigned int reg)
{
	switch (reg) {
	case RT5640_RESET:
...
/*
 * read rt5640 register cache
 */
static inline unsigned int rt5640_read_reg_cache(struct snd_soc_codec *codec,
	unsigned int reg)
{
	u16 *cache = codec->reg_cache;
	if (reg < 1 || reg > (ARRAY_SIZE(rt5640_reg) + 1))
		return -1;
	return cache[reg];
}
...
/*
 * write rt5640 register cache
 */

static inline void rt5640_write_reg_cache(struct snd_soc_codec *codec,
...
static int rt5640_write(struct snd_soc_codec *codec, unsigned int reg, unsigned int val)
{
	u8 data[3];
	int ret;
...
//	printk(KERN_INFO "write 0x%02x 0x%04x\n",reg,val);
	...
	rt5640_write_reg_cache(codec, reg, val);

	serial_out_i2c(ALC5640_I2C_ADDR, reg, val);

}

static unsigned int rt5640_read(struct snd_soc_codec *codec, unsigned int reg)
{
	//unsigned int val;
	if(rt5640_volatile_register(reg))
	{
		return serial_in_i2c(ALC5640_I2C_ADDR, reg);
	}
	return rt5640_read_reg_cache(codec, reg);
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i <= RT5640_VENDOR_ID2; i++) {
		if (cnt + 23 >= PAGE_SIZE)
			break;
		val = snd_soc_read(rt5640_codec, i);
		if (!val)
			continue;
				  ...
				  const char *buf, size_t count)
{
	...
	if (i == count) {
		pr_info("0x%02x = 0x%04x\n", addr,
			 rt5640_read(rt5640_codec, addr));
	} else {
		rt5640_write(rt5640_codec, addr, val);
	}

	return count;
}

static DEVICE_ATTR(codec_reg_w, 0666, rt5640_codec_show, rt5640_codec_store);
...
static struct rt5640_init_reg init_list[] = {
	...
	{RT5640_ADDA_CLK1	, 0x0014},//73[2] = 1'b //org:1114
	...
	/*playback*/
	{RT5640_STO_DAC_MIXER	, 0x4444},//Dig inf 2 -> Sto DAC mixer -> DACL
#if 0 //loop back test
	{RT5640_OUT_L3_MIXER	, 0x01de},//DACL1 -> OUTMIXL
	{RT5640_OUT_R3_MIXER	, 0x01de},//DACR1 -> OUTMIXR
#else
...
//	{RT5640_SPO_R_MIXER	, 0x1800},//DAC -> SPORMIX  
	...
	/*record*/
	{RT5640_IN1_IN2		, 0x1080},//IN1 boost 20db and differential mode
	{RT5640_IN3_IN4		, 0x0100},//IN2 boost 20db and signal ended mode
...
#endif
#if 0
	{RT5640_DSP_PATH2	, 0x0c00}, //bypass dac
#else
	{RT5640_DSP_PATH2	, 0xa000}, //use dsp
#endif
...
static int rt5640_readable_register(unsigned int reg)
{
	switch (reg) {
	case RT5640_RESET:
	case RT5640_SPK_VOL:
...
void rt5640_dump_reg(struct snd_soc_codec *codec)
{
	int reg;
#if 1	
//	printk(KERN_INFO "re5640 reg:\n");
	...
	{
		if(rt5640_readable_register(reg) == 1)
		{
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	struct rt5640_priv *rt5640 = codec->private_data;
...
static const struct soc_enum rt5640_in1_mode_enum = SOC_ENUM_SINGLE(RT5640_IN1_IN2,
	RT5640_IN_SFT1, ARRAY_SIZE(rt5640_input_mode),rt5640_input_mode);

static const struct soc_enum rt5640_in2_mode_enum = SOC_ENUM_SINGLE(RT5640_IN3_IN4,
	RT5640_IN_SFT2,	ARRAY_SIZE(rt5640_input_mode),rt5640_input_mode);

/* Interface data select */
static const char *rt5640_data_select[] = {
	"Normal", "left copy to right", "right copy to left", "Swap"};

static const struct soc_enum rt5640_if1_dac_enum = SOC_ENUM_SINGLE(RT5640_DIG_INF_DATA,
	RT5640_IF1_DAC_SEL_SFT, ARRAY_SIZE(rt5640_data_select), rt5640_data_select);

static const struct soc_enum rt5640_if1_adc_enum = SOC_ENUM_SINGLE(RT5640_DIG_INF_DATA,
	RT5640_IF1_ADC_SEL_SFT, ARRAY_SIZE(rt5640_data_select), rt5640_data_select);

static const struct soc_enum rt5640_if2_dac_enum = SOC_ENUM_SINGLE(RT5640_DIG_INF_DATA,
	RT5640_IF2_DAC_SEL_SFT, ARRAY_SIZE(rt5640_data_select), rt5640_data_select);

static const struct soc_enum rt5640_if2_adc_enum = SOC_ENUM_SINGLE(RT5640_DIG_INF_DATA,
	RT5640_IF2_ADC_SEL_SFT, ARRAY_SIZE(rt5640_data_select), rt5640_data_select);

static const struct soc_enum rt5640_if3_dac_enum = SOC_ENUM_SINGLE(RT5640_DIG_INF_DATA,
	RT5640_IF3_DAC_SEL_SFT, ARRAY_SIZE(rt5640_data_select), rt5640_data_select);

static const struct soc_enum rt5640_if3_adc_enum = SOC_ENUM_SINGLE(RT5640_DIG_INF_DATA,
	RT5640_IF3_ADC_SEL_SFT, ARRAY_SIZE(rt5640_data_select), rt5640_data_select);

/* Class D speaker gain ratio */
static const char *rt5640_clsd_spk_ratio[] = {"1.66x", "1.83x", "1.94x", "2x",
	"2.11x", "2.22x", "2.33x", "2.44x", "2.55x", "2.66x", "2.77x"};

static const struct soc_enum rt5640_clsd_spk_ratio_enum = SOC_ENUM_SINGLE(
	RT5640_CLS_D_OUT, RT5640_CLSD_RATIO_SFT,
	ARRAY_SIZE(rt5640_clsd_spk_ratio), rt5640_clsd_spk_ratio);

/* DMIC */
static const char *rt5640_dmic_mode[] = {"Disable", "DMIC1", "DMIC2"};

static const struct soc_enum rt5640_dmic_enum = SOC_ENUM_SINGLE(
	0, 0, ARRAY_SIZE(rt5640_dmic_mode), rt5640_dmic_mode);
...
static int rt5640_add_controls(struct snd_soc_codec *codec)
{
	int err, i;
	...
	for (i = 0; i < ARRAY_SIZE(rt5640_snd_controls); i ++) {
		...
		err = snd_ctl_add(codec->card, 
			snd_soc_cnew(&rt5640_snd_controls[i],codec, NULL));
		if (err < 0)
			return err;	
	}	
...
static const struct soc_enum rt5640_inl_enum = SOC_ENUM_SINGLE(
	RT5640_INL_INR_VOL, RT5640_INL_SEL_SFT,
	ARRAY_SIZE(rt5640_inl_src), rt5640_inl_src);
...
static const struct soc_enum rt5640_inr_enum = SOC_ENUM_SINGLE(
	RT5640_INL_INR_VOL, RT5640_INR_SEL_SFT,
	ARRAY_SIZE(rt5640_inr_src), rt5640_inr_src);
...
static const struct soc_enum rt5640_stereo_adc1_enum = SOC_ENUM_SINGLE(
	RT5640_STO_ADC_MIXER, RT5640_ADC_1_SRC_SFT,
	ARRAY_SIZE(rt5640_stereo_adc1_src), rt5640_stereo_adc1_src);
...
static const struct soc_enum rt5640_stereo_adc2_enum = SOC_ENUM_SINGLE(
	RT5640_STO_ADC_MIXER, RT5640_ADC_2_SRC_SFT,
	ARRAY_SIZE(rt5640_stereo_adc2_src), rt5640_stereo_adc2_src);
...
static const struct soc_enum rt5640_mono_adc_l1_enum = SOC_ENUM_SINGLE(
	RT5640_MONO_ADC_MIXER, RT5640_MONO_ADC_L1_SRC_SFT,
	ARRAY_SIZE(rt5640_mono_adc_l1_src), rt5640_mono_adc_l1_src);
...
static const struct soc_enum rt5640_mono_adc_l2_enum = SOC_ENUM_SINGLE(
	RT5640_MONO_ADC_MIXER, RT5640_MONO_ADC_L2_SRC_SFT,
	ARRAY_SIZE(rt5640_mono_adc_l2_src), rt5640_mono_adc_l2_src);
...
static const struct soc_enum rt5640_mono_adc_r1_enum = SOC_ENUM_SINGLE(
	RT5640_MONO_ADC_MIXER, RT5640_MONO_ADC_R1_SRC_SFT,
	ARRAY_SIZE(rt5640_mono_adc_r1_src), rt5640_mono_adc_r1_src);
...
static const struct soc_enum rt5640_mono_adc_r2_enum = SOC_ENUM_SINGLE(
	RT5640_MONO_ADC_MIXER, RT5640_MONO_ADC_R2_SRC_SFT,
	ARRAY_SIZE(rt5640_mono_adc_r2_src), rt5640_mono_adc_r2_src);
...
static const struct soc_enum rt5640_dac_l2_enum = SOC_ENUM_SINGLE(
	RT5640_DSP_PATH2, RT5640_DAC_L2_SEL_SFT,
	ARRAY_SIZE(rt5640_dac_l2_src), rt5640_dac_l2_src);
...
static const struct soc_enum rt5640_dac_r2_enum = SOC_ENUM_SINGLE(
	RT5640_DSP_PATH2, RT5640_DAC_R2_SEL_SFT,
	ARRAY_SIZE(rt5640_dac_r2_src), rt5640_dac_r2_src);
...
static const struct soc_enum rt5640_if2_adc_l_enum = SOC_ENUM_SINGLE(
	RT5640_DSP_PATH2, RT5640_IF2_ADC_L_SEL_SFT,
	ARRAY_SIZE(rt5640_if2_adc_l_src), rt5640_if2_adc_l_src);
...
static const struct soc_enum rt5640_if2_adc_r_enum = SOC_ENUM_SINGLE(
	RT5640_DSP_PATH2, RT5640_IF2_ADC_R_SEL_SFT,
	ARRAY_SIZE(rt5640_if2_adc_r_src), rt5640_if2_adc_r_src);
...
static const struct soc_enum rt5640_dai_iis_map_enum = SOC_ENUM_SINGLE(
	RT5640_I2S1_SDP, RT5640_I2S_IF_SFT,
	ARRAY_SIZE(rt5640_dai_iis_map), rt5640_dai_iis_map);
...
static const struct soc_enum rt5640_sdi_sel_enum = SOC_ENUM_SINGLE(
	RT5640_I2S2_SDP, RT5640_I2S2_SDI_SFT,
	ARRAY_SIZE(rt5640_sdi_sel), rt5640_sdi_sel);
...
void rt5640_pmu_depop(struct snd_soc_codec *codec)
{
	unsigned int val_fa = snd_soc_read(codec,RT5640_DUMMY1);

	snd_soc_write(codec,RT5640_DUMMY1,0x3401);
	/* depop parameters */
	...
	snd_soc_write(codec,RT5640_DUMMY1,val_fa);
}

void rt5640_pmd_depop(struct snd_soc_codec *codec)
{
	snd_soc_update_bits(codec, RT5640_HP_CALIB_AMP_DET,
	...
	msleep(60);
	rt5640_index_update_bits(codec, RT5640_CHOP_DAC_ADC, 0x0200, 0x0);
	snd_soc_update_bits(codec, RT5640_PWR_ANLG1,
		RT5640_PWR_HP_L | RT5640_PWR_HP_R | RT5640_PWR_HA,
		0);
...
static int rt5640_if1_event(struct snd_soc_dapm_widget *w, 
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	static int if1_pwd;
	...
	case SND_SOC_DAPM_POST_PMU:
		if1_pwd++;
		snd_soc_update_bits(codec,RT5640_PWR_DIG1,RT5640_PWR_I2S1,RT5640_PWR_I2S1);
		break;

	case SND_SOC_DAPM_PRE_PMD:
		if1_pwd--;
		if(if1_pwd <= 0) {
			snd_soc_update_bits(codec,RT5640_PWR_DIG1,RT5640_PWR_I2S1,0);
			if1_pwd = 0;
		}
		break;
...
static int rt5640_if2_event(struct snd_soc_dapm_widget *w, 
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	static int if2_pwd;
	...
	case SND_SOC_DAPM_POST_PMU:
		if2_pwd++;
		snd_soc_update_bits(codec,RT5640_PWR_DIG1,RT5640_PWR_I2S2,RT5640_PWR_I2S2);
		break;

	case SND_SOC_DAPM_PRE_PMD:
		if2_pwd--;
		if(if2_pwd <= 0) {
			snd_soc_update_bits(codec,RT5640_PWR_DIG1,RT5640_PWR_I2S2,0);
			if2_pwd = 0;
		}
		break;
...
static int rt5640_if3_event(struct snd_soc_dapm_widget *w, 
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	static int if3_pwd;
	...
	case SND_SOC_DAPM_POST_PMU:
		if3_pwd++;
		snd_soc_update_bits(codec,RT5640_PWR_DIG1,RT5640_PWR_I2S3,RT5640_PWR_I2S3);
		break;

	case SND_SOC_DAPM_PRE_PMD:
		if3_pwd--;
		if(if3_pwd <= 0) {
			snd_soc_update_bits(codec,RT5640_PWR_DIG1,RT5640_PWR_I2S3,0);
			if3_pwd = 0;
		}
		break;
...
static const struct snd_soc_dapm_widget rt5640_dapm_widgets[] = {
	SND_SOC_DAPM_MIXER_NAMED_CTL("PLL1", RT5640_PWR_ANLG2,
			RT5640_PWR_PLL_BIT, 0, NULL, 0),
	/* Input Side */
	/* micbias */
	SND_SOC_DAPM_MIXER_NAMED_CTL("LDO2", RT5640_PWR_ANLG1,
	...
	SND_SOC_DAPM_PGA_E("DMIC CLK", SND_SOC_NOPM,
		0, 0, NULL, 0, set_dmic_clk,
		SND_SOC_DAPM_PRE_PMU),
	/* Boost */
	SND_SOC_DAPM_MIXER_NAMED_CTL("BST1", RT5640_PWR_ANLG2,
		RT5640_PWR_BST1_BIT, 0, NULL, 0),
	SND_SOC_DAPM_MIXER_NAMED_CTL("BST2", RT5640_PWR_ANLG2,
		RT5640_PWR_BST4_BIT, 0, NULL, 0),
	/* Input Volume */
	SND_SOC_DAPM_MIXER_NAMED_CTL("INL VOL", RT5640_PWR_VOL,
		RT5640_PWR_IN_L_BIT, 0, NULL, 0),
	SND_SOC_DAPM_MIXER_NAMED_CTL("INR VOL", RT5640_PWR_VOL,
	...
	SND_SOC_DAPM_PGA_E("ADC Clock", SND_SOC_NOPM,
		0, 0, NULL, 0, rt5640_adc_event,
	...
	/* ADC Mixer */
	SND_SOC_DAPM_MIXER_NAMED_CTL("stereo filter", RT5640_PWR_DIG2,
		RT5640_PWR_ADC_SF_BIT, 0, NULL, 0),
	SND_SOC_DAPM_MIXER("Stereo ADC MIXL", RT5640_PWR_DIG2, RT5640_PWR_ADC_SF_BIT, 0,
		rt5640_sto_adc_l_mix, ARRAY_SIZE(rt5640_sto_adc_l_mix)),
	SND_SOC_DAPM_MIXER("Stereo ADC MIXR", RT5640_PWR_DIG2, RT5640_PWR_ADC_SF_BIT, 0,
		rt5640_sto_adc_r_mix, ARRAY_SIZE(rt5640_sto_adc_r_mix)),

	SND_SOC_DAPM_MIXER_NAMED_CTL("mono left filter", SND_SOC_NOPM,
		0, 0, NULL, 0),

	SND_SOC_DAPM_MIXER("Mono ADC MIXL", RT5640_PWR_DIG2, RT5640_PWR_ADC_MF_L_BIT, 0,
		rt5640_mono_adc_l_mix, ARRAY_SIZE(rt5640_mono_adc_l_mix)),

	SND_SOC_DAPM_MIXER_NAMED_CTL("mono right filter", SND_SOC_NOPM,
		0, 0, NULL, 0),

	SND_SOC_DAPM_MIXER("Mono ADC MIXR", RT5640_PWR_DIG2, RT5640_PWR_ADC_MF_R_BIT, 0,
	...
	SND_SOC_DAPM_MIXER_NAMED_CTL_E("IF1 DAC", SND_SOC_NOPM, 
		0, 0, NULL, 0, rt5640_if1_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_MIXER_NAMED_CTL("IF1 DAC L", SND_SOC_NOPM, 0, 0, NULL, 0),
	SND_SOC_DAPM_MIXER_NAMED_CTL("IF1 DAC R", SND_SOC_NOPM, 0, 0, NULL, 0),

	SND_SOC_DAPM_MIXER_NAMED_CTL_E("IF1 ADC", SND_SOC_NOPM,
		0, 0, NULL, 0, rt5640_if1_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_MIXER_NAMED_CTL("IF1 ADC L", SND_SOC_NOPM, 0, 0, NULL, 0),
	SND_SOC_DAPM_MIXER_NAMED_CTL("IF1 ADC R", SND_SOC_NOPM, 0, 0, NULL, 0),

	SND_SOC_DAPM_MIXER_NAMED_CTL_E("IF2 DAC", SND_SOC_NOPM, 
		0, 0, NULL, 0,  rt5640_if2_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_MIXER_NAMED_CTL("IF2 DAC L", SND_SOC_NOPM, 0, 0, NULL, 0),
	SND_SOC_DAPM_MIXER_NAMED_CTL("IF2 DAC R", SND_SOC_NOPM, 0, 0, NULL, 0),

	SND_SOC_DAPM_MIXER_NAMED_CTL_E("IF2 ADC", SND_SOC_NOPM, 
		0, 0, NULL, 0,  rt5640_if2_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_MIXER_NAMED_CTL("IF2 ADC L", SND_SOC_NOPM, 0, 0, NULL, 0),
	SND_SOC_DAPM_MIXER_NAMED_CTL("IF2 ADC R", SND_SOC_NOPM, 0, 0, NULL, 0),

	SND_SOC_DAPM_MIXER_NAMED_CTL_E("IF3 DAC", SND_SOC_NOPM, 
		0, 0, NULL, 0, rt5640_if3_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_MIXER_NAMED_CTL("IF3 DAC L", SND_SOC_NOPM, 0, 0, NULL, 0),
	SND_SOC_DAPM_MIXER_NAMED_CTL("IF3 DAC R", SND_SOC_NOPM, 0, 0, NULL, 0),

	SND_SOC_DAPM_MIXER_NAMED_CTL_E("IF3 ADC", SND_SOC_NOPM, 
		0, 0, NULL, 0, rt5640_if3_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_MIXER_NAMED_CTL("IF3 ADC L", SND_SOC_NOPM, 0, 0, NULL, 0),
	SND_SOC_DAPM_MIXER_NAMED_CTL("IF3 ADC R", SND_SOC_NOPM, 0, 0, NULL, 0),
	...
	SND_SOC_DAPM_DAC("AIF3RX", "AIF3 Playback", SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_ADC("AIF3TX", "AIF3 Capture", SND_SOC_NOPM, 0, 0),

	/* Audio DSP */
	SND_SOC_DAPM_MIXER_NAMED_CTL("Audio DSP", SND_SOC_NOPM, 0, 0, NULL, 0),

	/* ANC */
	SND_SOC_DAPM_MIXER_NAMED_CTL("ANC", SND_SOC_NOPM, 0, 0, NULL, 0),
	...
	SND_SOC_DAPM_MUX("DAC R2 Mux", SND_SOC_NOPM, 0, 0,
				&rt5640_dac_r2_mux),
	SND_SOC_DAPM_MIXER_NAMED_CTL("DAC L2 Volume", RT5640_PWR_DIG1,
			RT5640_PWR_DAC_L2_BIT, 0, NULL, 0),
	SND_SOC_DAPM_MIXER_NAMED_CTL("DAC R2 Volume", RT5640_PWR_DIG1,
	...
	/* Ouput Volume */
	SND_SOC_DAPM_MIXER_NAMED_CTL("SPKVOL L", RT5640_PWR_VOL,
		RT5640_PWR_SV_L_BIT, 0, NULL, 0),
	SND_SOC_DAPM_MIXER_NAMED_CTL("SPKVOL R", RT5640_PWR_VOL,
		RT5640_PWR_SV_R_BIT, 0, NULL, 0),
	SND_SOC_DAPM_MIXER_NAMED_CTL("OUTVOL L", RT5640_PWR_VOL,
		RT5640_PWR_OV_L_BIT, 0, NULL, 0),
	SND_SOC_DAPM_MIXER_NAMED_CTL("OUTVOL R", RT5640_PWR_VOL,
		RT5640_PWR_OV_R_BIT, 0, NULL, 0),
	SND_SOC_DAPM_MIXER_NAMED_CTL("HPOVOL L", RT5640_PWR_VOL,
		RT5640_PWR_HV_L_BIT, 0, NULL, 0),
	SND_SOC_DAPM_MIXER_NAMED_CTL("HPOVOL R", RT5640_PWR_VOL,
...
static const struct snd_soc_dapm_route rt5640_dapm_routes[] = {
	...
	{"IF2 ADC L Mux", NULL, "Mono ADC MIXL"},
	{"IF2 ADC R Mux", NULL, "Mono ADC MIXR"},
...
static int rt5640_add_widgets(struct snd_soc_codec *codec)
{
	snd_soc_dapm_new_controls(codec, rt5640_dapm_widgets,
		ARRAY_SIZE(rt5640_dapm_widgets));
	snd_soc_dapm_add_routes(codec, rt5640_dapm_routes,
...
static void rt5640_sync_cache(struct snd_soc_codec *codec)
{
...
static int rt5640_init(struct snd_soc_device *socdev)
{
	...
	codec->read = rt5640_read;
	codec->write = rt5640_write;
	codec->set_bias_level = rt5640_set_bias_level;
	codec->dai = rt5640_dai;
	codec->num_dai = ARRAY_SIZE(rt5640_dai);
	codec->reg_cache_size = RT5640_VENDOR_ID2 + 1;
	codec->reg_cache_step = 1,
	codec->reg_cache = kmemdup(rt5640_reg, sizeof(rt5640_reg), GFP_KERNEL);
	...
	if (ret < 0) {
		printk(KERN_ERR "rt5640: failed to create pcms\n");
		goto pcm_err;	
	}
	...
	if (ret < 0) {
		printk(KERN_ERR "rt5640: failed to register card!\n");
		goto card_err;	
	}
	
	rt5640_add_controls(codec);
	rt5640_add_widgets(codec);
...
//	printk(KERN_INFO "rt5640 initial ok!\n");
	return 0;
...
#if 0
static void rt5640_work(struct work_struct *work)
{
	struct snd_soc_codec *codec = container_of(work, struct snd_soc_codec, delayed_work.work);
	
	rt5640_set_bias_level(codec, codec->bias_level);	
}
#endif
static int rt5640_probe(struct platform_device *pdev)
{
	struct snd_soc_device *socdev = platform_get_drvdata(pdev);
	struct rt5640_setup_data *setup;
	struct snd_soc_codec *codec;
	struct rt5640_priv *rt5640;
	int ret = 0;

//	pr_info("RT5640 Audio Codec %s", RT5640_VERSION);
	...
	codec->private_data = rt5640;
	codec->dev = &pdev->dev;
	socdev->card->codec = codec;
	mutex_init(&codec->mutex);
	rt5640_socdev = socdev;
	INIT_LIST_HEAD(&codec->dapm_widgets);
	INIT_LIST_HEAD(&codec->dapm_paths);
#if 0	
	INIT_DELAYED_WORK(&codec->delayed_work, rt5640_work);
#endif
...
#ifdef CONFIG_PM
static int rt5640_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct snd_soc_device *socdev = platform_get_drvdata(pdev);
	struct snd_soc_codec *codec = socdev->card->codec;
...
static int rt5640_resume(struct platform_device *pdev)
{
	struct snd_soc_device *socdev = platform_get_drvdata(pdev);
	struct snd_soc_codec *codec = socdev->card->codec;
...
struct snd_soc_codec_device soc_codec_dev_rt5640 = {
	.probe = rt5640_probe,
	.remove = rt5640_remove,
	.suspend = rt5640_suspend,
	.resume = rt5640_resume,
};
EXPORT_SYMBOL_GPL(soc_codec_dev_rt5640);

static int __init rt5640_modinit(void)
{
	return snd_soc_register_dais(rt5640_dai, ARRAY_SIZE(rt5640_dai));
}
module_init(rt5640_modinit);

static void __exit rt5640_modexit(void)
{
	snd_soc_unregister_dais(rt5640_dai, ARRAY_SIZE(rt5640_dai));
}
