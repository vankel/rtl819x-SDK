...
int32 apollomp_raw_l2_lookUpTb_set(apollomp_lut_table_t *pL2Table)
{
   int32 ret = RT_ERR_FAILED;
    ...
    RT_PARAM_CHK(pL2Table->address > APOLLOMP_LUT_4WAY_NO+APOLLOMP_LUT_CAM_NO, RT_ERR_OUT_OF_RANGE);
    ...
    RT_PARAM_CHK(pL2Table->dsl_mbr > APOLLOMP_RAW_LUT_DSLMBR, RT_ERR_OUT_OF_RANGE);
...
int32 apollomp_raw_unkn_mcPri_set(uint32 priority)
{
    ...
    if ((ret = reg_field_write(APOLLOMP_UNKN_MC_PRIr, APOLLOMP_UNKN_MC_PRIf, &priority)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
...
int32 apollomp_raw_unkn_mcPri_get(uint32* pPriority)
{
    ...
    if ((ret = reg_field_read(APOLLOMP_UNKN_MC_PRIr, APOLLOMP_UNKN_MC_PRIf, pPriority)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
...
int32 apollomp_raw_l2_camEnable_set(rtk_enable_t enable)
{
    ...
    if ((ret = reg_field_write(APOLLOMP_LUT_CFGr, APOLLOMP_BCAM_DISf, &enable)) != RT_ERR_OK)
    {
...
int32 apollomp_raw_l2_camEnable_get(rtk_enable_t *pEnable)
{
    ...
    if ((ret = reg_field_read(APOLLOMP_LUT_CFGr, APOLLOMP_BCAM_DISf, pEnable)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
