...
dal_apollo_rate_portEgrBandwidthCtrlRate_set(rtk_port_t port, uint32 rate)
{
    ...
    if(HAL_IS_PON_PORT(port))
    {
        if ((ret = reg_field_write(PON_CFGr,EGR_RATEf, &rate)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_RATE|MOD_DAL), "");
            return ret;
        }
...
dal_apollo_rate_stormBypass_set(rtk_storm_bypass_t type, rtk_enable_t enable)
{
    ...
    if(BYPASS_IGMP == type)
    {
        if (( ret = apollo_raw_trap_igmpBypassStrom_set(enable)) != RT_ERR_OK )
        {
            RT_ERR(ret, (MOD_RATE|MOD_DAL), "");
            return ret;
        }         
    ...
    else if(BYPASS_CDP == type || BYPASS_CSSTP == type)
    {
        ...
        if (( ret = apollo_raw_trap_rmaCiscoAction_set(rmaTail, data)) != RT_ERR_OK )
        {
            RT_ERR(ret, (MOD_RATE|MOD_DAL), "");
            return ret;
        }
    ...
    {
        rmaTail = (uint32)type;
        if (( ret = apollo_raw_trap_rmaFeature_set(rmaTail, RAW_RMA_FUN_BYPASS_STORM, enable)) != RT_ERR_OK )
        {
            RT_ERR(ret, (MOD_RATE|MOD_DAL), "");
            return ret;
        }         
...
dal_apollo_rate_stormBypass_get(rtk_storm_bypass_t type, rtk_enable_t *pEnable)
{
    ...
    if(BYPASS_IGMP == type)
    {
        if (( ret = apollo_raw_trap_igmpBypassStrom_get(pEnable)) != RT_ERR_OK )
        {
            RT_ERR(ret, (MOD_RATE|MOD_DAL), "");
            return ret;
        }         
    ...
    else if(BYPASS_CDP == type || BYPASS_CSSTP == type)
    {
        ...
        if (( ret = apollo_raw_trap_rmaCiscoAction_get(rmaTail, &data)) != RT_ERR_OK )
        {
            RT_ERR(ret, (MOD_RATE|MOD_DAL), "");
            return ret;
        }
    ...
    {
        rmaTail = (uint32)type;
        if (( ret = apollo_raw_trap_rmaFeature_get(rmaTail, RAW_RMA_FUN_BYPASS_STORM, pEnable)) != RT_ERR_OK )
        {
            RT_ERR(ret, (MOD_RATE|MOD_DAL), "");
            return ret;
        }         
