 ...
 *
 * $Revision: 10372 $
 * $Date: 2010-06-22 20:39:54 +0800 (Tue, 22 Jun 2010) $
 *
...
dal_apollo_stp_init(void)
{
    ...
    for (port = 0; port < max_port; port++)
    {
        ...
        {
            if ((ret = dal_apollo_stp_mstpState_set(RTK_DEFAULT_MSTI, port, STP_STATE_FORWARDING)) != RT_ERR_OK)
            {
        ...
        {
            if ((ret = dal_apollo_stp_mstpState_set(RTK_DEFAULT_MSTI, port, RTK_DEFAULT_STP_PORT_STATE)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_STP|MOD_DAL), "");
                stp_init = INIT_NOT_COMPLETED;
                return ret;
...
dal_apollo_stp_mstpState_get(uint32 msti, rtk_port_t port, rtk_stp_state_t *pStp_state)
{
    ...
    if((ret = reg_array_field_read(MSTI_CTRLr, port , msti, STATEf, (uint32 *) pStp_state)) != RT_ERR_OK)
    {
    ...
    return RT_ERR_OK;
} /* end of dal_apollo_stp_mstpState_get */
...
dal_apollo_stp_mstpState_set(uint32 msti, rtk_port_t port, rtk_stp_state_t stp_state)
{
    ...
    if((ret = reg_array_field_write(MSTI_CTRLr, port , msti, STATEf, (uint32 *) &stp_state)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_STP), "");        
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_apollo_stp_mstpState_set */
