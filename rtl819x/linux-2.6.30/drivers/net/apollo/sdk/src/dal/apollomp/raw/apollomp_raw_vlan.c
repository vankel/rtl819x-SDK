 ...
 *      None
 */
int32 apollomp_raw_vlan_portBasedVID_set(rtk_port_t port, uint32 index, uint32 pri)
{
    uint32  ret;
    ...
    if(pri > HAL_INTERNAL_PRIORITY_MAX())
        return RT_ERR_QOS_INT_PRIORITY;
    ...
    if ((ret = reg_array_field_write(APOLLOMP_VLAN_PB_PRIr, port, REG_ARRAY_INDEX_NONE, APOLLOMP_PB_PRIf, &pri)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_VLAN|MOD_DAL), "");
        return ret;
    }
 ...
 *      None
 */
int32 apollomp_raw_vlan_portBasedVID_get(rtk_port_t port, uint32 *pIndex, uint32 *pPri)
{
    uint32  ret;
    ...
    if ((ret = reg_array_field_read(APOLLOMP_VLAN_PB_PRIr, port, REG_ARRAY_INDEX_NONE, APOLLOMP_PB_PRIf, (uint32 *)pPri)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_VLAN|MOD_DAL), "");
        return ret;
    }
...
/* Function Name:
 *      apollomp_raw_vlan_portBasedPri_set
 * Description:
 *      set  vlan port based FID
 * Input:
 *      port 		- port number
 ...
 *      None
 */
int32 apollomp_raw_vlan_portBasedPri_set(rtk_port_t port, uint32 pri)
{
	uint32  tmpVal;
	int32   ret;
    ...
    tmpVal = pri;
    if ((ret = reg_array_field_write(APOLLOMP_VLAN_PB_PRIr,port,REG_ARRAY_INDEX_NONE,APOLLOMP_PB_PRIf,&tmpVal)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_VLAN|MOD_DAL), "");
        return ret;
    }
...
/* Function Name:
 *      apollomp_raw_vlan_portBasedPri_get
 * Description:
 *      set  vlan port based FID
 * Input:
 *      port 		- port number
 ...
 *      None
 */
int32 apollomp_raw_vlan_portBasedPri_get(rtk_port_t port, uint32 *pPri)
{
	uint32  tmpVal;
	int32   ret;
    ...
    if ((ret = reg_array_field_read(APOLLOMP_VLAN_PB_PRIr,port,REG_ARRAY_INDEX_NONE,APOLLOMP_PB_PRIf,&tmpVal)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_VLAN|MOD_DAL), "");
        return ret;
    }
