...
int32 apollo_raw_meter_rate_set(uint32 meterIdx, uint32 meterRate)
{
    ...
    if ((ret = reg_array_field_write(METER_GLB_CTRLr, REG_ARRAY_INDEX_NONE, meterIdx, RATEf, &meterRate)) != RT_ERR_OK )
    {
...
int32 apollo_raw_meter_rate_get(uint32 meterIdx, uint32 *pMeterRate)
{
	...
	if (( ret = reg_array_field_read(METER_GLB_CTRLr, REG_ARRAY_INDEX_NONE, meterIdx, RATEf, pMeterRate)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_RATE|MOD_DAL), "");
        return ret;
    }
