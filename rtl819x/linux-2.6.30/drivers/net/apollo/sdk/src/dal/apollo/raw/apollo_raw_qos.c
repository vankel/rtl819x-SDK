...
int32 apollo_raw_qos_dscpRemarkAbility_set(rtk_port_t port, rtk_enable_t enable)
{
    ...
    if ((ret = reg_array_field_write(RMK_DSCP_RMK_EN_CTRLr, port, REG_ARRAY_INDEX_NONE, ENf, &enable)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_QOS|MOD_DAL), "");
        return ret;
    }
...
int32 apollo_raw_qos_dscpRemarkAbility_get(rtk_port_t port, rtk_enable_t *pEnable)
{
    ...
    if ((ret = reg_array_field_read(RMK_DSCP_RMK_EN_CTRLr, port, REG_ARRAY_INDEX_NONE, ENf, pEnable)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_QOS|MOD_DAL), "");
        return ret;
    }
