 ...
 * Data Declaration
 */
static uint32   oam_init = {INIT_NOT_COMPLETED}; 
...
dal_apollo_oam_init(void)
{
    ...
    oam_init = INIT_COMPLETED;
    
    HAL_SCAN_ALL_PORT(port)
    {
        if ((ret = dal_apollo_oam_parserAction_set(port, OAM_PARSER_ACTION_FORWARD)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
            oam_init = INIT_NOT_COMPLETED;
            return ret;
        }
        
        if ((ret = dal_apollo_oam_multiplexerAction_set(port, OAM_MULTIPLEXER_ACTION_FORWARD)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
            oam_init = INIT_NOT_COMPLETED;
            return ret;
...
dal_apollo_oam_parserAction_set(rtk_port_t port, rtk_oam_parser_act_t action)
{
    ...
    val = (uint32)action;
    if ((ret = reg_array_field_write(OAM_P_CTRL_0r, port, REG_ARRAY_INDEX_NONE, OAM_PARSERf, &val)) != RT_ERR_OK)
    {
    ...
    return RT_ERR_OK;
} /* end of dal_apollo_oam_parserAction_set */
...
dal_apollo_oam_parserAction_get(rtk_port_t port, rtk_oam_parser_act_t *pAction)
{
    ...
    if ((ret = reg_array_field_read(OAM_P_CTRL_0r, port, REG_ARRAY_INDEX_NONE, OAM_PARSERf, &val)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_OAM), "");
        return ret;
    }
    ...
    *pAction = (rtk_oam_parser_act_t)val;    
    ...
    return RT_ERR_OK;
} /* end of dal_apollo_oam_parserAction_get */
...
dal_apollo_oam_multiplexerAction_set(rtk_port_t port, rtk_oam_multiplexer_act_t action)
{
    ...
    val = (uint32)action;
    if ((ret = reg_array_field_write(OAM_P_CTRL_1r, port, REG_ARRAY_INDEX_NONE, OAM_MULTIPLEXERf, &val)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_OAM), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_apollo_oam_multiplexerAction_set */
...
dal_apollo_oam_multiplexerAction_get(rtk_port_t port, rtk_oam_multiplexer_act_t *pAction)
{
    ...
    if ((ret = reg_array_field_read(OAM_P_CTRL_1r, port, REG_ARRAY_INDEX_NONE, OAM_MULTIPLEXERf, &val)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_OAM), "");
        return ret;
    }
