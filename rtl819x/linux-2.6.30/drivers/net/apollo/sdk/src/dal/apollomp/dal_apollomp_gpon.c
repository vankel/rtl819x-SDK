...
int32 dal_apollomp_gpon_driver_initialize(void)
{
    ...
    /* GPON use internal clock */
    data = 0x3;
    if ((ret = reg_field_write(APOLLOMP_WSDS_DIG_01r, APOLLOMP_CFG_FRC_NOTIFYf, &data)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_GPON | MOD_DAL), "");
        return ret;
    }
    ...
    /* set the packet buffer and flow control threshold */
...
int32 dal_apollomp_gpon_activate(rtk_gpon_initialState_t initState)
{
    ...
    apollomp_raw_intr_ims_clear(SW_INTR_TYPE_GPON);

    /* switch interrupt mask for GPON */
    apollomp_raw_intr_imr_set(SW_INTR_TYPE_GPON,ENABLED);
#endif
    GPON_OS_Unlock(g_gponmac_drv->lock);
...
#if 0
    /* switch interrupt mask for GPON */
    apollomp_raw_intr_imr_set(SW_INTR_TYPE_GPON,DISABLED);
#endif
