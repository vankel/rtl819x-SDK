...
dal_apollo_acl_igrRuleEntry_delAll(void)
{
    ...
    if((ret = apollo_l2_table_clear(ACL_DATAt,0,index)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
		return ret;
    }

    if((ret = apollo_l2_table_clear(ACL_MASKt,0,index)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
		return ret;
    }
    ...
    if((ret = apollo_l2_table_clear(ACL_ACTIONt,0,index)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
		return ret;
    }
