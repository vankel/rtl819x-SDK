...
int32 apollo_raw_flowctrl_publicJumboUsedPageCnt_get(uint32 *pCurrent, uint32 *pMaximum)
{
    ...
    if ((ret = reg_field_read(FC_PUB_JUMBO_USED_PAGE_CNTr, PUB_JUMBO_MAX_USED_PAGE_CNTf, pCurrent)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL|MOD_DAL), "");
        return ret;
    }
...
static int32 _patch_35m_gpon(void)
{
    ...
    if ((ret = apollo_raw_flowctrl_ponQueueEegressDropThreshold_set(0, 70)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL | MOD_DAL), "");
        return ret;
    }
