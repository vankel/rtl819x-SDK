...
#if defined(CONFIG_SDK_APOLLOMP)
        case APOLLOMP_CHIP_ID:
            if ((ret = reg_field_read(APOLLOMP_FB_CTRLr, APOLLOMP_PL_DEC_ENf, &state)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_PORT), "");
                return ret;
            }
...
#if defined(CONFIG_SDK_APOLLOMP)
        case APOLLOMP_CHIP_ID:
            if ((ret = reg_field_write(APOLLOMP_FB_CTRLr, APOLLOMP_PL_DEC_ENf, &state)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_PORT), "");
                return ret;
            }
