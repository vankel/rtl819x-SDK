...
int32 dal_port_isolationEntry_test(uint32 caseNo)
{
    ...
    rtk_portmask_t extPortmaskW;
    rtk_portmask_t extPortmaskR;
    rtk_enable_t incCpuW;
    rtk_enable_t incCpuR;

    RTK_PORTMASK_RESET(portmaskW);
    RTK_PORTMASK_RESET(extPortmaskW);
    if(rtk_port_isolationEntry_set(RTK_PORT_ISO_CFG_0, 0, &portmaskW, DISABLED, &extPortmaskW) == RT_ERR_FEATURE_NOT_SUPPORTED)
    {
        return RT_ERR_OK;
    }
    ...
    /*error input check*/
    /*out of range*/
    if(rtk_port_isolationEntry_set(RTK_PORT_ISO_CFG_END, 0, &portmaskW, DISABLED, &extPortmaskW) == RT_ERR_OK)
    {
        osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }

    if(rtk_port_isolationEntry_set(RTK_PORT_ISO_CFG_0, HAL_GET_MAX_PORT() + 1, &portmaskW, DISABLED, &extPortmaskW) == RT_ERR_OK)
    {
        osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }

    if(rtk_port_isolationEntry_set(RTK_PORT_ISO_CFG_0, 0, &portmaskW, RTK_ENABLE_END, &extPortmaskW) == RT_ERR_OK)
    {
        osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
    ...
    portmaskW.bits[0] = 0xFF;
    extPortmaskW.bits[0] = 0xFF;
    if(rtk_port_isolationEntry_set(RTK_PORT_ISO_CFG_0, 0, &portmaskW, DISABLED, &extPortmaskW) == RT_ERR_OK)
    {
        osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
    ...
    /*null pointer*/
    if(rtk_port_isolationEntry_get(RTK_PORT_ISO_CFG_0, 0, NULL, NULL, NULL) == RT_ERR_OK)
    {
        osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
    ...
    for(modeW = RTK_PORT_ISO_CFG_0; modeW < RTK_PORT_ISO_CFG_END; modeW++)
    {
        for(portW = 0; portW <= HAL_GET_MAX_PORT(); portW++)
        {
            for(portmaskW.bits[0] = 0x00; HAL_IS_PORTMASK_VALID(portmaskW); portmaskW.bits[0]++)
            {
                for(incCpuW = DISABLED; incCpuW < RTK_ENABLE_END; incCpuW++)
                {
                    for(extPortmaskW.bits[0] = 0x00; HAL_IS_EXTPORTMASK_VALID(extPortmaskW); extPortmaskW.bits[0]++)
                    {
                        if(!RTK_PORTMASK_IS_PORT_SET(extPortmaskW, HAL_GET_EXT_CPU_PORT()))
                        {
                            if(rtk_port_isolationEntry_set(modeW, portW, &portmaskW, incCpuW, &extPortmaskW) != RT_ERR_OK)
                            {
                                osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
                                return RT_ERR_FAILED;
                            }

                            if(rtk_port_isolationEntry_get(modeW, portW, &portmaskR, &incCpuR, &extPortmaskR) != RT_ERR_OK)
                            {
                                osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
                                return RT_ERR_FAILED;
                            }
...
int32 dal_port_isolationEntryExt_test(uint32 caseNo)
{
    ...
    RTK_PORTMASK_RESET(portmaskW);
    RTK_PORTMASK_RESET(extPortmaskW);
    if(rtk_port_isolationEntryExt_set(RTK_PORT_ISO_CFG_0, 0, &portmaskW, DISABLED, &extPortmaskW) == RT_ERR_FEATURE_NOT_SUPPORTED)
    {
        return RT_ERR_OK;
    }
    ...
    /*error input check*/
    /*out of range*/
    if(rtk_port_isolationEntryExt_set(RTK_PORT_ISO_CFG_END, 0, &portmaskW, DISABLED, &extPortmaskW) == RT_ERR_OK)
    {
        osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }

    if(rtk_port_isolationEntryExt_set(RTK_PORT_ISO_CFG_0, HAL_GET_MAX_EXT_PORT() + 1, &portmaskW, DISABLED, &extPortmaskW) == RT_ERR_OK)
    {
        osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }

    if(rtk_port_isolationEntryExt_set(RTK_PORT_ISO_CFG_0, 0, &portmaskW, RTK_ENABLE_END, &extPortmaskW) == RT_ERR_OK)
    {
        osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
    ...
    portmaskW.bits[0] = 0xFF;
    extPortmaskW.bits[0] = 0xFF;
    if(rtk_port_isolationEntryExt_set(RTK_PORT_ISO_CFG_0, 0, &portmaskW, DISABLED, &extPortmaskW) == RT_ERR_OK)
    {
        osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
    ...
    /*null pointer*/
    if(rtk_port_isolationEntryExt_get(RTK_PORT_ISO_CFG_0, 0, NULL, NULL, NULL) == RT_ERR_OK)
    {
        osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
            ...
            for(portmaskW.bits[0] = 0x00; HAL_IS_PORTMASK_VALID(portmaskW); portmaskW.bits[0]++)
            {
                for(incCpuW = DISABLED; incCpuW < RTK_ENABLE_END; incCpuW++)
                {
                    for(extPortmaskW.bits[0] = 0x00; HAL_IS_EXTPORTMASK_VALID(extPortmaskW); extPortmaskW.bits[0]++)
                    {
                        if(!RTK_PORTMASK_IS_PORT_SET(extPortmaskW, HAL_GET_EXT_CPU_PORT()))
                        {
                            if(rtk_port_isolationEntryExt_set(modeW, portW, &portmaskW, incCpuW, &extPortmaskW) != RT_ERR_OK)
                            {
                                osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
                                return RT_ERR_FAILED;
                            }

                            if(rtk_port_isolationEntryExt_get(modeW, portW, &portmaskR, &incCpuR, &extPortmaskR) != RT_ERR_OK)
                            {
                                osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
                                return RT_ERR_FAILED;
                            }
