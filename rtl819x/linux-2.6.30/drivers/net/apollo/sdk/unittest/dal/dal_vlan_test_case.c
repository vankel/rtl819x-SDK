...
int32 dal_vlan_test(uint32 caseNo)
{
    ...
    for(memPmskW.bits[0] = 0; memPmskW.bits[0] <= extpmsk.bits[0]; memPmskW.bits[0]++)
    {
        ...
        if( rtk_vlan_extPort_get(1, &memPmskR) != RT_ERR_OK )
        {
            osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
...
int32 dal_vlan_portEgrTagKeepType_test(uint32 caseNo)
{
    ...
    if( rtk_vlan_portEgrTagKeepType_set(HAL_GET_MAX_PORT() + 1, &portmaskW, TAG_KEEP_TYPE_NOKEEP) == RT_ERR_OK )
    {
        osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
    ...
    if( rtk_vlan_portEgrTagKeepType_set(0, &portmaskW, TAG_KEEP_TYPE_NOKEEP) == RT_ERR_OK )
    {
        osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
    ...
    RTK_PORTMASK_PORT_SET(portmaskW, 1);
    if( rtk_vlan_portEgrTagKeepType_set(0, &portmaskW, TAG_KEEP_TYPE_FORMAT) == RT_ERR_OK )
    {
        osal_printf("\n %s %d\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
    ...
    for(portW = 0; portW <= HAL_GET_MAX_PORT(); portW++)
    {
        ...
        {
            for(typeW = TAG_KEEP_TYPE_NOKEEP; typeW < TAG_KEEP_TYPE_END; typeW++)
            {
                if(typeW == TAG_KEEP_TYPE_CONTENT)
                {
