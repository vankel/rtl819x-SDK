 ...
 *
 *  $Id: 8190n.h,v 1.3 2009/10/02 04:23:58 george Exp $
 *
...
#define EXT_VAR_MAPPING(dst) 	\
		extern unsigned char *data_##dst##_start ; \
		extern unsigned char *data_##dst##_end ; 

EXT_VAR_MAPPING(MACPHY_REG);
EXT_VAR_MAPPING(AGC_TAB);
EXT_VAR_MAPPING(phy_reg);
EXT_VAR_MAPPING(phy_reg_MP);
EXT_VAR_MAPPING(PHY_to1T1R);
EXT_VAR_MAPPING(PHY_to1T1R_b);
EXT_VAR_MAPPING(PHY_to1T2R);
EXT_VAR_MAPPING(PHY_to1T2R_b);
EXT_VAR_MAPPING(PHY_REG_PG);
EXT_VAR_MAPPING(radio_a);
EXT_VAR_MAPPING(radio_a_hp);
EXT_VAR_MAPPING(radio_b);
#endif
...
/*
	...
	struct lls_mesh_header meshhdr;
#endif
*/
...
/*#ifdef SUPPORT_SNMP_MIB
	...
	//mac80211_support --chris
	struct mac80211_shared_priv *share_priv;
...
#define GET_HW(priv)		(priv->pshare_hw->phw)
