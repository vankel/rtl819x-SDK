 ...
 *
 *      $Id: 8190n_tkip.c,v 1.12 2009/01/09 14:55:42 jimmylin Exp $
 */
...
__DRAM_IN_865X unsigned char pTkip_Sbox_Lower[256], pTkip_Sbox_Upper[256];
#endif
#endif
...
#if defined(_USE_DRAM_) || defined(CONFIG_RTL865X)
...
static inline void rtl_tkip_swap(unsigned char *a, unsigned char *b)
{
    unsigned char tmp;
...
#else
__DRAM_IN_865X unsigned char en_cipherstream[MAX_MESSAGE_LENGTH+1];
#endif
...
// __IRAM_IN_865X
void tkip_encrypt(
				struct rtl8190_priv *priv, unsigned char *pwlhdr, unsigned int hdrlen,
				unsigned char *frag1, unsigned int frag1_len,
				unsigned char *frag2, unsigned int frag2_len,
				unsigned char *frag3, unsigned int frag3_len)
{
...
// __IRAM_IN_865X
unsigned int tkip_decrypt(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo, unsigned int fr_len)
{
	union TSC48		tsc48;
	unsigned char	*da;
	unsigned char	*sa;
...
void wep_encrypt(struct rtl8190_priv *priv, unsigned char *pwlhdr, unsigned int hdrlen,
				unsigned char *frag1, unsigned int frag1_len,
				unsigned char *frag2, unsigned int frag2_len,
				unsigned char *frag3, unsigned int frag3_len,
				int type)
...
#ifdef _DEBUG_RTL8190_
{
	...
	sprintf(tmpbuf, "wep encrypt: iv=%d, keyid=%d, type=%s, key=",
		ntohl(*((unsigned long *)tmp1)), keyid,
...
unsigned int wep_decrypt(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo,
...
#ifdef _DEBUG_RTL8190_
{
	...
	sprintf(tmpbuf, "wep decript: iv=%d, keyid=%d, type=%s, key=",
		ntohl(*((unsigned long *)tmp1)), (int)((iv[3]>>6)&3),
		(type==_WEP_40_PRIVACY_ ? "64b" : "128b"));
...
int tkip_rx_mic(struct rtl8190_priv *priv, unsigned char *pframe, unsigned char *da,
