 ...
 *
 *      $Id: 8190n_ioctl.c,v 1.146 2009/07/17 12:59:21 button Exp $
 */
...
#define RTL8190_IOCTL_WRITE_EDCA_REG		0x8B76
#endif
...
#ifdef RTL8192SE // temporary solution
...
#ifdef RTL_PATCHED_26
static int set_edca_reg(struct rtl8190_priv *priv, edcaPara* edcap)
{
	unsigned int reg;
	unsigned int val;
	unsigned char acmEn;
	...
	val=RTL_R32(reg);
	if(EDCA_ALL==edcap->flag)
	{
		val=edcap->value;
	}
	else if(EDCA_TXOP==edcap->flag)
	{	
		val &=~((0xFFFF)<<16);
		val |=((edcap->value & 0xFFFF) << 16);
	}else if(EDCA_ECWMAX==edcap->flag)
	{
		val &=~((0xF)<<12);
		val |=((edcap->value & 0xF)<<12);
	}else if(EDCA_ECWMIN==edcap->flag)
	{
		val &=~((0xF)<<8);
		val |=((edcap->value & 0xF)<<8);
	}else if(EDCA_AIFS==edcap->flag)
	{
		val &=(~0xFF);
		val |=(edcap->value & 0xFF);
	}else if(EDCA_ACM==edcap->flag)
	{
		if(edcap->value > 0)
		{
			RTL_W8(_ACM_CTRL_, RTL_R8(_ACM_CTRL_)|acmEn);
...
static int parse_edcaCmd(char *cmd,edcaPara* edcap)
{
	char cmdname[32];
	char cmdvalue[32];
	char *p;
	...
	cmdvalue[i]='\0';
	if(!strncmp(cmdname,"vi",strlen("vi")))
	{
		edcap->type=AC_VI;
	}else if(!strncmp(cmdname,"vo",strlen("vo")))
	{
		edcap->type=AC_VO;
	}else if(!strncmp(cmdname,"be",strlen("be")))
	{
		edcap->type=AC_BE;
	}else if(!strncmp(cmdname,"bk",strlen("bk")))
	{
		edcap->type=AC_BK;
	}
	else 
		return -1;
	if(!strncmp(cmdname+3,"ixop",strlen("ixop")))
	{
		edcap->flag=EDCA_TXOP;
	}else if(!strncmp(cmdname+3,"cwmax",strlen("cwmax")))
	{
		edcap->flag=EDCA_ECWMAX;
	}else if(!strncmp(cmdname+3,"cwmin",strlen("cwmin")))
	{
		edcap->flag=EDCA_ECWMIN;
	}else if(!strncmp(cmdname+3,"aifs",strlen("aifs")))
	{
		edcap->flag=EDCA_AIFS;
	}else if(!strncmp(cmdname+3,"acm",strlen("acm")))
	{
		edcap->flag=EDCA_ACM;
	}
	else
		return -1;
	edcap->value=_atoi(cmdvalue,10);
	return 0;
}
/*cmd format vi=ixop,CWMax,cwmin,AIFS,acm*/
/*cmd format vi_ixop=
 *                 vi_cwmax=
 *                 vi_cwmin=
 *                 vi_aifs=
 *                 vi_acm=
 */
static int set_edca(struct rtl8190_priv *priv, unsigned char *data)
{
	int ret;
	edcaPara edca;
	DEBUG_TRACE;
	DEBUG_INFO("set_edca %s\n", data);
	memset(&edca,0x0,sizeof(edca));
	ret=parse_edcaCmd((char *)data,&edca);
	if(ret < 0)
		return ret;
	return set_edca_reg(priv,&edca);
}
#endif
...
#endif
#ifdef RTL_PATCHED_26
				{ RTL8190_IOCTL_WRITE_EDCA_REG, IW_PRIV_TYPE_CHAR | 128, 0, "set_edca" },
#endif
