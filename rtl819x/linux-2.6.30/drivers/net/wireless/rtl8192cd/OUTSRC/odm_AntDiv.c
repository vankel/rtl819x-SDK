...
odm_UpdateTxAnt(IN PDM_ODM_T pDM_Odm, IN u1Byte Ant, IN u4Byte MacId)
{
	...
	if (pDM_Odm->antdiv_rssi)
        {
		panic_printk("[Update TxInfo]: MacID:(( %d )),  TxAnt = (( %s ))\n",MacId,(Ant==MAIN_ANT)?"MAIN_ANT":"AUX_ANT");
...
odm_BDCcoexType_withBferClient(IN PDM_ODM_T pDM_Odm, IN u1Byte swch)
{
	...
	{
		ODM_AntDiv_Printk( ("[BDCcoexType: 1] {DIV,CSI} ={1,0} \n")) ;
		BDCcoexType_wBfer=1;
	...
	{
		ODM_AntDiv_Printk( ("[BDCcoexType: 2] {DIV,CSI} ={0,1}\n")) ;
		BDCcoexType_wBfer=2;
...
odm_BF_AntDiv_ModeArbitration(IN PDM_ODM_T pDM_Odm)
{
		...
		if((pDM_BdcTable->num_Txbfee_Client !=0) && (pDM_BdcTable->num_Txbfer_Client == 0))
		{
			...
			if(current_BDC_Mode != pDM_BdcTable->BDC_Mode)
			{
				...
				ODM_AntDiv_Printk( ("Change to (( Mode1 ))\n")) ;
			}

			ODM_AntDiv_Printk( ("[Antdiv + BF coextance Mode] : (( Mode1 ))\n")) ;
		}
		//2 Mode 2
		else if((pDM_BdcTable->num_Txbfee_Client ==0) && (pDM_BdcTable->num_Txbfer_Client != 0))
		{
			...
			if(current_BDC_Mode != pDM_BdcTable->BDC_Mode)
			{
				...
				pDM_BdcTable->BDC_state=BDC_DIV_TRAIN_STATE;
				pDM_BdcTable->BDC_Try_flag=0;
				ODM_AntDiv_Printk( ("Change to (( Mode2 ))\n")) ;
			}
	
			ODM_AntDiv_Printk( ("[Antdiv + BF coextance Mode] : (( Mode2 ))\n")) ;
		}
		//2 Mode 3
		else if((pDM_BdcTable->num_Txbfee_Client !=0) && (pDM_BdcTable->num_Txbfer_Client != 0))
		{
			...
			if(current_BDC_Mode != pDM_BdcTable->BDC_Mode)
			{
				...
				pDM_BdcTable->BDC_Try_flag=0;
				pDM_BdcTable->BDC_RxIdleUpdate_counter=1;
				ODM_AntDiv_Printk( ("Change to (( Mode3 ))\n")) ;
			}

			ODM_AntDiv_Printk( ("[Antdiv + BF coextance Mode] : (( Mode3 ))\n")) ;
		}
		//2 Mode 4
		else if((pDM_BdcTable->num_Txbfee_Client ==0) && (pDM_BdcTable->num_Txbfer_Client == 0))
		{
			...
			if(current_BDC_Mode != pDM_BdcTable->BDC_Mode)
			{
				...
				odm_BDCcoexType_withBferClient( pDM_Odm, DIVON_CSIOFF);
				ODM_AntDiv_Printk( ("Change to (( Mode4 ))\n")) ;
			}

			ODM_AntDiv_Printk( ("[Antdiv + BF coextance Mode] : (( Mode4 ))\n")) ;			
		}
	#endif
...
)
{
	...
	u1Byte 	BDC_active_Mode;
		...
		if (pDM_Odm->bdc_holdstate==2) 
		{
			...
			ODM_AntDiv_Printk(("Force in [ BF STATE] \n"));
			return;	
		}
		else if (pDM_Odm->bdc_holdstate==1) 
		{
			...
			odm_BDCcoexType_withBferClient( pDM_Odm, DIVON_CSIOFF); 
			ODM_AntDiv_Printk(("Force in [ DIV STATE] \n"));
			return;	
		}
		...
		if(pDM_BdcTable->BDC_Mode==BDC_MODE_2 ||pDM_BdcTable->BDC_Mode==BDC_MODE_3)
		{
			...
			{
				ODM_AntDiv_Printk( ("BFer STA only?  (( No ))\n"));
			}
			//
			if(pDM_BdcTable->bAll_BFSta_Idle==FALSE && pDM_BdcTable->bAll_DivSta_Idle==TRUE)
			{
				ODM_AntDiv_Printk( ("All DIV-STA are idle, but BF-STA not\n"));
				pDM_BdcTable->BDC_Try_flag=0;
				pDM_BdcTable->BDC_state=BDC_BFer_TRAIN_STATE;
				odm_BDCcoexType_withBferClient( pDM_Odm, DIVOFF_CSION);
				return;
			...
			{
				ODM_AntDiv_Printk( ("All BF-STA are idle, but DIV-STA not\n"));
				pDM_BdcTable->BDC_Try_flag=0;
				pDM_BdcTable->BDC_state=BDC_DIV_TRAIN_STATE;
				odm_BDCcoexType_withBferClient( pDM_Odm, DIVON_CSIOFF);
				return;
			...
			if(pDM_BdcTable->num_BfTar ==0) //  Selsect_1,  Selsect_2
			{
				...
				{
					ODM_AntDiv_Printk( ("Select active mode (( 1 )) \n"));
					pDM_BdcTable->BDC_active_Mode=1;
				}
				else
				{
					ODM_AntDiv_Printk( ("Select active mode  (( 2 ))\n"));
			...
			{			
				if(pDM_BdcTable->num_DivTar ==0)  // Selsect_3
				{
					ODM_AntDiv_Printk( ("Select active mode (( 3 ))\n"));
				...
				{
					BDC_active_Mode=4;
					ODM_AntDiv_Printk( ("Select active mode (( 4 ))\n"));
					
					if(BDC_active_Mode!=pDM_BdcTable->BDC_active_Mode)
					{
						pDM_BdcTable->BDC_active_Mode=4;
						ODM_AntDiv_Printk( ("Change to active mode (( 4 ))  &  return!!! \n"));
						return;
					}
				}
			}
		...
		if (pDM_Odm->bdc_holdstate==0xff) 
		{
			...
			{ 
				pDM_BdcTable->BDC_Try_flag=0;
				pDM_BdcTable->BDC_state=BDC_DIV_TRAIN_STATE;
				ODM_AntDiv_Printk(("[  The number of client has been changed !!!]   return to (( BDC_DIV_TRAIN_STATE )) \n"));
			}
			...
			if( pDM_BdcTable->BDC_Try_flag==0)
			{
				...
				{
					ODM_AntDiv_Printk(("*****[2-1. BFer_TRAIN_STATE ]*****  \n"));
					
					//if(pDM_BdcTable->num_BfTar==0) 
					//{
					//	ODM_AntDiv_Printk(("BF_tars exist?  : (( No )),   [ BDC_BFer_TRAIN_STATE ] >> [BDC_DIV_TRAIN_STATE] \n"));
					//	odm_DivTrainState_setting( pDM_Odm);
					//}
					//else //num_BfTar != 0
					//{
						...
						odm_BDCcoexType_withBferClient( pDM_Odm, DIVOFF_CSION); 
						ODM_AntDiv_Printk(("BF_tars exist?  : (( Yes )),   [ BDC_BFer_TRAIN_STATE ] >> [BDC_DECISION_STATE] \n"));
					//}
				}
				//2 DECISION_STATE (Mode 2-2)
				else if(pDM_BdcTable->BDC_state==BDC_DECISION_STATE)
				{			
					ODM_AntDiv_Printk(("*****[2-2. DECISION_STATE]***** \n"));
					//if(pDM_BdcTable->num_BfTar==0) 
					//{
					...
					//{
						...
						if( (pDM_Odm->antdiv_rssi))
							panic_printk("BF_tars exist?  : (( Yes )),  {BF_pass, DIV_pass, StopBF_flag }  = { %d, %d, %d } \n" ,pDM_BdcTable->BF_pass,pDM_BdcTable->DIV_pass,StopBF_flag);	
						...
						if(StopBF_flag==TRUE) //DIV_en
						{
							...
							odm_BDCcoexType_withBferClient( pDM_Odm, DIVON_CSIOFF); 
							pDM_BdcTable->BDC_state=BDC_DIV_HOLD_STATE;
							ODM_AntDiv_Printk(("[ StopBF_flag= ((TRUE)),   BDC_DECISION_STATE ] >> [BDC_DIV_HOLD_STATE] \n"));
						}
						else //BF_en
						{
							pDM_BdcTable->BDC_Hold_counter=10; //20
							odm_BDCcoexType_withBferClient( pDM_Odm, DIVOFF_CSION); 
							pDM_BdcTable->BDC_state=BDC_BF_HOLD_STATE;
							ODM_AntDiv_Printk(("[StopBF_flag= ((FALSE)),   BDC_DECISION_STATE ] >> [BDC_BF_HOLD_STATE] \n"));
						}
					//}
				}
				...
				{
					ODM_AntDiv_Printk(("*****[2-3. BF_HOLD_STATE ]*****\n"));

					if (pDM_Odm->antdiv_rssi)
						panic_printk("BDC_Hold_counter = (( %d )) \n",pDM_BdcTable->BDC_Hold_counter );

					if(pDM_BdcTable->BDC_Hold_counter==1)
					{
						ODM_AntDiv_Printk(("[ BDC_BF_HOLD_STATE ] >> [BDC_DIV_TRAIN_STATE] \n"));
						odm_DivTrainState_setting( pDM_Odm);
					}
					else
					{
						...
						//if(pDM_BdcTable->num_BfTar==0) 
						//{
						//	ODM_AntDiv_Printk(("BF_tars exist?  : (( No )),   [ BDC_BF_HOLD_STATE ] >> [BDC_DIV_TRAIN_STATE] \n"));
						//	odm_DivTrainState_setting( pDM_Odm);
						//}
						//else //num_BfTar != 0
						//{
							//ODM_AntDiv_Printk(("BF_tars exist?  : (( Yes ))\n"));
							pDM_BdcTable->BDC_state=BDC_BF_HOLD_STATE;
							odm_BDCcoexType_withBferClient( pDM_Odm, DIVOFF_CSION);
							ODM_AntDiv_Printk(("[ BDC_BF_HOLD_STATE ] >> [BDC_BF_HOLD_STATE] \n"));
						//}
					}
				...
				{
					ODM_AntDiv_Printk(("*****[2-4. DIV_HOLD_STATE ]*****\n"));
					...
					if(pDM_BdcTable->BDC_Hold_counter==1)
					{
						ODM_AntDiv_Printk(("[ BDC_DIV_HOLD_STATE ] >> [BDC_DIV_TRAIN_STATE] \n"));
						odm_DivTrainState_setting( pDM_Odm);
					}
					else
					{
						...
						pDM_BdcTable->BDC_state=BDC_DIV_HOLD_STATE;
						odm_BDCcoexType_withBferClient( pDM_Odm, DIVON_CSIOFF); 
						ODM_AntDiv_Printk(("[ BDC_DIV_HOLD_STATE ] >> [BDC_DIV_HOLD_STATE] \n"));
					}
...
)
{
	...
	ODM_SetBBReg(pDM_Odm, ODM_REG_ANT_MAPPING1_11N , 0xFFFF, 0x0100);	//antenna mapping table
...
)
{
	...
	ODM_SetBBReg(pDM_Odm, 0x8CC , BIT20|BIT19|BIT18, 3); //settling time of antdiv by RF  1 uint=25ns
...
)
{
	...
	else
	{
		if(pDM_FatTable->bBecomeLinked ==FALSE)
		{
			...
			ODM_AntDiv_Printk( ("[Linked!!!]\n")) ;
			
			odm_AntDiv_on_off(pDM_Odm, ANTDIV_ON);
			odm_Tx_By_TxDesc_or_Reg(pDM_Odm , TX_DESC);
	...
	for (i=0; i<ODM_ASSOCIATE_ENTRY_NUM; i++)
	{
		...
		if(IS_STA_VALID(pEntry))
		{
			...
			if (pDM_Odm->antdiv_rssi)
			{
				...
				panic_printk("*** Client[ %d ] : Main_Cnt = (( %d ))  , Main_RSSI= ((  %d )) \n",i, pDM_FatTable->MainAnt_Cnt[i], Main_RSSI);
				panic_printk("*** Client[ %d ] : Aux_Cnt   = (( %d ))  , Aux_RSSI = ((  %d )) \n" ,i, pDM_FatTable->AuxAnt_Cnt[i] , Aux_RSSI);	
	...
	//2 Set RX Idle Antenna & TX Antenna(Because of HW Bug )	
		...
		if (pDM_Odm->antdiv_rssi)
			...
			if(pDM_BdcTable->BDC_Mode==BDC_MODE_1 ||pDM_BdcTable->BDC_Mode==BDC_MODE_3)
			{
				...
				{
					pDM_BdcTable->BDC_RxIdleUpdate_counter--;
					ODM_AntDiv_Printk(("***NOT update RxIdle Antenna because of BF  ( need to fix TX-ant)\n"));
				}
			}
			else
		#endif	
	...
	)
{
	...
	else//if( try_flag != 0xff ) 
	{
		...
		if(pDM_SWAT_Table->try_flag == 0) 
		{
			...
			if (curTxOkCnt > 1875000 || curRxOkCnt > 1875000)//if(PlatformDivision64(curTxOkCnt+curRxOkCnt, 2) > 1875000)  ( 1.875M * 8bit ) / 2sec = 7.5M bits /sec )
			{
...
)
{
	...
	ODM_AntDiv_Printk(("odm_SetNextMACAddrTarget() ==>\n"));
	if(pDM_Odm->bLinked)
	{
		for (i=0; i<ODM_ASSOCIATE_ENTRY_NUM; i++)
		{
...
)
{
	...
	ODM_AntDiv_Printk(("==>odm_FastAntTraining()\n"));
	...
	//1 DECISION STATE
		...
		if(bPktFilterMacth_pathA  == FALSE)
		{
			...
			ODM_AntDiv_Printk(("{Path-A}: None Packet is matched\n"));
			odm_AntDiv_on_off(pDM_Odm, ANTDIV_OFF);
		}
		else
		{
			if (pDM_Odm->antdiv_rssi)
			{
				panic_printk("TargetAnt_pathA = (( %d )) , MaxRSSI_pathA = (( %d )) \n",TargetAnt_pathA,MaxRSSI_pathA);
			}
	...
	if(pDM_FatTable->FAT_State == FAT_NORMAL_STATE)
	{
		...
		ODM_AntDiv_Printk(("[ Start  Normal State] \n"));
		...
		ODM_AntDiv_Printk(("[Start Training State] \n"));
...
#else
	ODM_AntDiv_Printk(("******odm_FastAntTrainingCallback****** \n"));
...
{
	ODM_AntDiv_Printk(("******odm_FastAntTrainingWorkItemCallback****** \n"));
	odm_FastAntTraining(pDM_Odm);
}
//#endif
//#endif
...
{	
	...
	//----------
	if (pDM_Odm->antdiv_select==0)
		pDM_Odm->AntType = ODM_AUTO_ANT;
	else if (pDM_Odm->antdiv_select==1)
		pDM_Odm->AntType = ODM_FIX_MAIN_ANT;
	else if (pDM_Odm->antdiv_select==2)
		pDM_Odm->AntType = ODM_FIX_AUX_ANT;
	else if (pDM_Odm->antdiv_select==0xff)
		pDM_Odm->AntType = ODM_STAY_CURR_ANT;
...
pFAT_T			pDM_FatTable = &pDM_Odm->DM_FatTable;
	...
	if(pDM_Odm->SupportICType & ODM_N_ANTDIV_SUPPORT)
		CCKMaxRate=DESC92C_RATE11M;
	else if(pDM_Odm->SupportICType & ODM_AC_ANTDIV_SUPPORT)
		CCKMaxRate=DESC8812_RATE11M;
	...
	if(pDM_Odm->AntDivType == CG_TRX_SMART_ANTDIV)
	{
		if( (pDM_Odm->SupportICType & ODM_SMART_ANT_SUPPORT) &&  (pPktinfo->bPacketToSelf)   &&  (pDM_FatTable->FAT_State == FAT_TRAINING_STATE)   ) 
		{
