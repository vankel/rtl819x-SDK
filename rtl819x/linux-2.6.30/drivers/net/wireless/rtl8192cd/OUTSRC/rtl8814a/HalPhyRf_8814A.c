	...
	)
{
	...
	if(Method == MIX_MODE)	//normal Tx power tracking
	{
		...
		bitMask = BIT19;
		pDM_Odm->Absolute_OFDMSwingIdx[RFPath] = pDM_Odm->Absolute_OFDMSwingIdx[RFPath] + pDM_Odm->KfreeOffset[RFPath];

		if( pDM_Odm->Absolute_OFDMSwingIdx[RFPath] >= 0)				// check if RF_Index is positive or not
			bPositive = TRUE;
		else
			bPositive = FALSE;
		...
		Final_RF_Index = (pDM_Odm->Absolute_OFDMSwingIdx[RFPath] / 2);		//TxBB 1 step equal 1dB, BB swing 1step equal 0.5dB
	...
	if(Final_RF_Index > UpperBound)		//Upper bound = 10dB, if more htan upper bound, then move to bb swing max = +2dB
	{
		...
		Final_OFDM_Swing_Index = pDM_Odm->DefaultOfdmIndex + (pDM_Odm->Absolute_OFDMSwingIdx[RFPath] - (UpperBound*2));
	...
	else if(Final_RF_Index < LowerBound)	// lower bound = -5dB
	{
		...
		Final_OFDM_Swing_Index = pDM_Odm->DefaultOfdmIndex - ((LowerBound*2) - pDM_Odm->Absolute_OFDMSwingIdx[RFPath]);
	...
	else		// normal case
	{
		...
		Final_OFDM_Swing_Index = pDM_Odm->DefaultOfdmIndex + (pDM_Odm->Absolute_OFDMSwingIdx[RFPath])%2;
		return Final_OFDM_Swing_Index;	
	}
	...
	)
{
	...
	u4Byte			rate				= *(pDM_Odm->pForcedDataRate);
		...
		)
{
	...
	pConfig->DoIQK = PHY_IQCalibrate_8814A;
