			...
			if(pDM_Odm->SupportICType & (ODM_RTL8188E|ODM_RTL8192E))
			{
				if(pDM_Odm->cck_agc_report_type == 0 && (pDM_Odm->SupportICType & ODM_RTL8192E) )
				{
					switch(LNA_idx)
					{
						...
						case 7:
							rx_pwr_all = -(pDM_Odm->priv->pshare->rf_ft_var.rx_pwr_7)  -2*(VGA_idx); 
						case 6:
							rx_pwr_all = -(pDM_Odm->priv->pshare->rf_ft_var.rx_pwr_6) -2*(VGA_idx); 
							break;
						case 5:
							rx_pwr_all = -(pDM_Odm->priv->pshare->rf_ft_var.rx_pwr_5) - 2*(VGA_idx); 
							break;
						case 4:
							rx_pwr_all = -(pDM_Odm->priv->pshare->rf_ft_var.rx_pwr_4) - 2*(VGA_idx); 
							break;
						case 3:
							rx_pwr_all = -(pDM_Odm->priv->pshare->rf_ft_var.rx_pwr_3) - 2*(VGA_idx); 
							break;
						case 2:
							rx_pwr_all = -(pDM_Odm->priv->pshare->rf_ft_var.rx_pwr_2) - 2*(VGA_idx);
							break;
						case 1:
							rx_pwr_all = (pDM_Odm->priv->pshare->rf_ft_var.rx_pwr_1)-2*(VGA_idx);
							break;
						case 0:
							rx_pwr_all = (pDM_Odm->priv->pshare->rf_ft_var.rx_pwr_0)-2*(VGA_idx);
							break;
#else
						case 7:
