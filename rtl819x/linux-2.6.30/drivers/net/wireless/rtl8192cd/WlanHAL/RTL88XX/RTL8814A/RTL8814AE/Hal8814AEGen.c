	...
	)
{
	...
	/*RTL_W16( REG_SOUNDING_PTCL_CTRL+1, 0x0202);*/
	RTL_W8( REG_SOUNDING_PTCL_CTRL+1, 0x02);
	RTL_W8( REG_SOUNDING_PTCL_CTRL+2, 0x02);
	...
	if ((pBeamformingInfo->BeamformCap & BEAMFORMEE_CAP) && (BFerIdx < BEAMFORMER_ENTRY_NUM)) {
		...
		RTL_W8( REG_SOUNDING_PTCL_CTRL+3, 0x40);				//ndp_rx_standby_timer, 8814 need > 0x56, suggest from Dvaid
	}
