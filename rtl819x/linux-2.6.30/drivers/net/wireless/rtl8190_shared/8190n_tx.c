 ...
 *      TX handle routines
 *
 *      $Id: 8190n_tx.c,v 1.189 2009/07/17 09:05:29 jimmylin Exp $
 */
...
extern int rtl_fill_fwinfo(struct mac80211_shared_priv *priv, struct tx_info *txinfo, struct tx_desc *pdesc);
#else
extern int rtl_fill_fwinfo(struct rtl8190_priv *priv, struct tx_insn *txcfg, struct tx_desc *pdesc, unsigned int frag_idx);
#endif
...
__IRAM_FASTEXTDEV
#endif
int generate_legacy_phdr(struct rtl8190_priv *priv, struct tx_insn *txcfg)
{
	unsigned int 		fr_len, tx_len, i, q_num, q_select, keyid;
	int *tx_head;
