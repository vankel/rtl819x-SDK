 ...
 *
 *      $Id: 8190n_headers.h,v 1.78 2009/07/17 09:05:29 jimmylin Exp $
 */
...
EXTERN unsigned int find_rate_MP(struct rtl8190_priv *priv, struct stat_info *pstat, struct ht_cap_elmt * peer_ht_cap, int peer_ht_cap_len, char * peer_rate, int peer_rate_len, int mode, int isBasicRate);
...
EXTERN int get_bit_value_from_ieee_value(u32 val);
#else
...
EXTERN int should_restrict_Nrate(struct rtl8190_priv *priv, struct stat_info *pstat);
...
EXTERN int get_assoc_sta_num(struct rtl8190_priv *priv);
...
EXTERN void free_skb_queue(struct RTL_PRIV *priv, struct sk_buff_head *skb_que);
...
EXTERN unsigned char fw_was_full(struct rtl8190_priv *priv);
...
#ifdef RTL8192SE
EXTERN unsigned int set_fw_reg(struct RTL_PRIV *priv, unsigned int cmd, unsigned int val, unsigned int with_val);
...
#ifdef NEW_MAC80211_DRV
EXTERN bool rtl_tx_aggr_check(struct mac80211_shared_priv *, struct priv_stainfo *, u8);
EXTERN void rtl_tx_aggr_resume(struct mac80211_shared_priv *, struct ieee80211_sta *, u16);
EXTERN int rtl_tx_aggr_start(struct mac80211_shared_priv *, struct ieee80211_sta *, u16 , u16 *);
EXTERN int rtl_tx_aggr_stop(struct mac80211_shared_priv *, struct ieee80211_sta *, u16 );

EXTERN void rtl80211_addRATid(struct mac80211_shared_priv *priv, struct ieee80211_sta *sta);
EXTERN void rtl80211_delRATid(struct mac80211_shared_priv *priv, struct ieee80211_sta *sta);
EXTERN int rtl80211_isErpSta(struct ieee80211_sta *sta);
EXTERN int rtl80211_mac_set_key(struct ieee80211_hw *hw, enum set_key_cmd cmd,
...
EXTERN void ampdu_timeout(struct RTL_PRIV *priv, unsigned int current_time);
#endif
...
#endif
extern int rtl8192SE_SetupOneCmdPacket(struct RTL_PRIV *priv, unsigned char *dat_content, unsigned short txLength, unsigned char LastPkt);
#endif
#ifdef CONFIG_RTK_MESH
...
#ifdef NEW_MAC80211_DRV
EXTERN void rtl80211_tx_tasklet(unsigned long task_priv);
EXTERN int rtl80211_tx(struct ieee80211_hw *hw, struct sk_buff *skb);
EXTERN int __rtl80211_tx(struct ieee80211_hw *hw, struct sk_buff *skb, int);
EXTERN void mac80211_send_beacon(struct mac80211_shared_priv *priv);
EXTERN int TxBeacon(struct mac80211_shared_priv *priv, unsigned char *dat_content,unsigned short txLength);
#endif
...
EXTERN void proxy_table_chkcln(struct rtl8190_priv* priv, struct sk_buff *pskb);
...
#endif
EXTERN void rtl8190_rx_isr(struct RTL_PRIV *priv);
...
#ifdef NEW_MAC80211_DRV
EXTERN void rtl80211_rx_tasklet(unsigned long task_priv);
#endif
...
EXTERN void set_slot_time(struct RTL_PRIV *priv, int use_short);
EXTERN void SetTxPowerLevel(struct rtl8190_priv *priv);
EXTERN void SwChnl(struct RTL_PRIV *priv, unsigned char channel, int offset);
EXTERN void SwitchAntenna(struct rtl8190_priv *priv);
EXTERN void enable_hw_LED(struct RTL_PRIV *priv, unsigned int led_type);
EXTERN void GetHardwareVersion(struct rtl8190_priv *priv);
EXTERN void init_EDCA_para(struct RTL_PRIV *priv, int mode);
...
EXTERN unsigned int PHY_QueryBBReg(struct RTL_PRIV *priv, unsigned int RegAddr, unsigned int BitMask);
EXTERN void PHY_SetBBReg(struct RTL_PRIV *priv, unsigned int RegAddr, unsigned int BitMask, unsigned int Data);
EXTERN void PHY_SetRFReg(struct RTL_PRIV *priv, RF90_RADIO_PATH_E eRFPath, unsigned int RegAddr,
				unsigned int BitMask, unsigned int Data);
EXTERN int phy_BB8192SE_Config_ParaFile(struct RTL_PRIV *priv);
EXTERN int phy_BB8190_Config_ParaFile(struct rtl8190_priv *priv);
EXTERN int phy_RF8225_Config_ParaFile(struct RTL_PRIV *priv);
EXTERN int phy_RF8256_Config_ParaFile(struct rtl8190_priv *priv);
EXTERN int PHY_ConfigMACWithParaFile(struct RTL_PRIV *priv);
EXTERN void SwBWMode(struct RTL_PRIV *priv, unsigned int bandwidth, int offset);
#ifdef NEW_MAC80211_DRV
EXTERN void setup_timer1(struct RTL_PRIV *priv, int timeout);
EXTERN void cancel_timer1(struct RTL_PRIV *priv);
EXTERN void setup_timer2(struct RTL_PRIV *priv, unsigned int timeout);
EXTERN void cancel_timer2(struct RTL_PRIV *priv);
#else
...
EXTERN int rtl819x_init_hw_PCI(struct RTL_PRIV *priv);
EXTERN int rtl819x_stop_hw(struct RTL_PRIV *priv, int reset_bb);
...
EXTERN void check_and_set_ampdu_spacing(struct RTL_PRIV *priv, struct stat_info *pstat);
EXTERN void rtl8192se_ePhyInit(struct RTL_PRIV * priv);
EXTERN void Switch_1SS_Antenna(struct RTL_PRIV *priv, unsigned int antPath );
EXTERN void Switch_OFDM_Antenna(struct RTL_PRIV *priv, unsigned int antPath );
...
#ifdef NEW_MAC80211_DRV
EXTERN int CamAddOneEntry(struct RTL_PRIV *priv, unsigned char *pucMacAddr,
				unsigned long ulKeyId, unsigned long ulEncAlg, unsigned long ulUseDK,
				unsigned char *pucKey);
EXTERN int CamDeleteOneEntry(struct RTL_PRIV *priv, unsigned char *pucMacAddr,
				unsigned long ulKeyId, unsigned int useDK);
EXTERN void CamResetAllEntry(struct RTL_PRIV *priv);
EXTERN void CamDumpAll(struct RTL_PRIV *priv);
EXTERN void CAM_read_entry(struct RTL_PRIV *priv, unsigned char index, unsigned char *macad,
...
EXTERN void set_mib_default_tbl(struct RTL_PRIV *priv);
...
EXTERN int rtl819x_ioctl(struct net_device *, struct ifreq *, int);
...
#else
EXTERN void MDL_DEVINIT rtl8190_proc_init (struct mac80211_shared_priv *priv);
EXTERN void __devexit rtl8190_proc_remove (struct mac80211_shared_priv *priv);
#endif
...
#ifdef NEW_MAC80211_DRV
extern int pci_dev_init(struct pci_dev *pdev, struct mac80211_shared_priv *priv, int *mem_len, int *io_len);
#else
extern int pci_dev_init(struct pci_dev *pdev, struct rtl8190_priv *priv, struct net_device *dev);
#endif
...
#else
EXTERN int rtl8190_open (struct ieee80211_hw *hw);
EXTERN void rtl8190_close(struct ieee80211_hw *hw);
#endif
...
EXTERN void derivePSK(struct rtl8190_priv *priv);
