 ...
 *
 *      $Id: 8190n_eeprom.c,v 1.6 2009/01/09 14:55:42 jimmylin Exp $
 */

#define _8190N_EEPROM_C_
...
static VOID ShiftOutBits(struct rtl8190_priv *priv, USHORT data, USHORT count);
static USHORT ShiftInBits(struct rtl8190_priv *priv);
static VOID RaiseClock(struct rtl8190_priv *priv, USHORT *x);
static VOID LowerClock(struct rtl8190_priv *priv, USHORT *x);
static VOID EEpromCleanup(struct rtl8190_priv *priv);
static USHORT WaitEEPROMCmdDone(struct rtl8190_priv *priv);
static VOID StandBy(struct rtl8190_priv *priv);
...
int WriteAdapterInfo(struct rtl8190_priv *priv, int entry_id, void *data)
{
	ULONG			ioaddr = priv->pshare->ioaddr;
	USHORT			Index;
	USHORT          usValue;
