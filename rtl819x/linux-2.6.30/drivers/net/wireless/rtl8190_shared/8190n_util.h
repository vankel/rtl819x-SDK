 ...
 *
 *      $Id: 8190n_util.h,v 1.31 2009/07/08 05:40:11 button Exp $
 */
...
#else // !USE_IO_OPS && !NEW_MAC80211_DRV
...
#if defined(USE_RTL8186_SDK) && !defined(CONFIG_RTL_819X)
	int swap[4]={3,2,1,0};
...
#if defined(CHECK_SWAP) && !defined(NEW_MAC80211_DRV)
...
static __inline__ struct list_head *dequeue_frame(struct RTL_PRIV *priv, struct list_head *head)
{
	unsigned long flags;
...
static __inline__ struct sk_buff *rtl_dev_alloc_skb(struct RTL_PRIV *priv,
...
static __inline__ void rtl_kfree_skb(struct RTL_PRIV *priv, struct sk_buff *skb, int flag)
{
#ifdef ENABLE_RTL_SKB_STATS
	if (flag & (_SKB_TX_ | _SKB_TX_IRQ_))
		rtl_atomic_dec(&priv->rtl_tx_skb_cnt);
...
#ifndef RTL8192SU
static __inline__ void rtl_cache_sync_wback(struct RTL_PRIV *priv, unsigned int start,
...
static __inline__ unsigned long get_physical_addr(struct RTL_PRIV *priv, void *ptr,
				unsigned int size, int direction)
{
#ifdef NEW_MAC80211_DRV
...
static __inline__ int get_rf_mimo_mode(struct RTL_PRIV *priv)
{
#ifdef NEW_MAC80211_DRV
	return MIMO_2T2R;
#else
