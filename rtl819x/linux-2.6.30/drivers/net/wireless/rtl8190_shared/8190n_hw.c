 ...
 *
 *      $Id: 8190n_hw.c,v 1.179 2009/07/10 14:01:03 button Exp $
 */
...
//#endif
void PHY_RF6052SetOFDMTxPower(struct RTL_PRIV *priv, unsigned char powerlevel_1ss, 
	unsigned char powerlevel_2ss);
static int LoadFirmware(struct RTL_PRIV *priv);
...
*/
unsigned int PHY_QueryBBReg(struct RTL_PRIV *priv, unsigned int RegAddr, unsigned int BitMask)
{
...
*/
void PHY_SetBBReg(struct RTL_PRIV *priv, unsigned int RegAddr, unsigned int BitMask, unsigned int Data)
{
 ...
 *		       ------------------------------------------------------------------
*/
void phy_RFSerialWrite(struct RTL_PRIV *priv, RF90_RADIO_PATH_E eRFPath, unsigned int Offset, unsigned int Data)
{
...
*			This function is equal to the combination of RF_ReadReg() and  RFLSSIRead()
*/
unsigned int phy_RFSerialRead(struct RTL_PRIV *priv, RF90_RADIO_PATH_E eRFPath, unsigned int Offset, unsigned int dbg_avoid)
{
	struct rtl8190_hw			*phw = GET_HW(priv);
...
__NOMIPS16
#endif
void PHY_SetRFReg(struct RTL_PRIV *priv, RF90_RADIO_PATH_E eRFPath, unsigned int RegAddr,
 ...
 *
 *---------------------------------------------------------------------------*/
int PHY_ConfigBBWithParaFile(struct RTL_PRIV *priv, int reg_file)
{
...
/*#if !defined(CONFIG_X86) && !defined(MERGE_FW)
...
#ifdef MERGE_FW
	if (reg_file==AGCTAB) {
		reg_table = (struct PhyRegTable *)pshare->agc_tab_buf;
		...
		pg_reg_table = (struct MacRegTable *)pshare->phy_reg_pg_buf;
		...
		if(pshare->rf_ft_var.pathB_1T == 0){//PATH A
			pg_reg_table = (struct MacRegTable *)pshare->phy_reg_2to1;
		...
		if(pshare->rf_ft_var.pathB_1T == 0){
			pg_reg_table = (struct MacRegTable *)pshare->phy_reg_2to1;
		...
		if (pshare->rf_ft_var.mp_specific) {
			reg_table = (struct PhyRegTable *)pshare->phy_reg_mp_buf;
...
#endif
		{
		reg_table = (struct PhyRegTable *)pshare->phy_reg_buf;
	...
	case PHYREG_2T4R:
		...
		break;
	case PHYREG_2T2R:
		//printk("try to load PHY reg from file\n");
#if	defined(RTL8190) || defined(RTL8192E)
...
#if defined(CHECK_HANGUP) && !defined(NEW_MAC80211_DRV)
 ...
 *---------------------------------------------------------------------------*/
#ifdef MERGE_FW
int PHY_ConfigRFWithParaFile(struct RTL_PRIV *priv, unsigned char *start, int read_bytes, RF90_RADIO_PATH_E eRFPath)
#else
int PHY_ConfigRFWithParaFile(struct RTL_PRIV *priv, unsigned char *pFileName, RF90_RADIO_PATH_E eRFPath)
#endif
{
	int           num;
#ifndef MERGE_FW
...
int PHY_ConfigMACWithParaFile(struct RTL_PRIV *priv)
{
...
#if defined(UNIVERSAL_REPEATER) && !defined(NEW_MAC80211_DRV)
...
#endif // UNIVERSAL_REPEATER && !NEW_MAC80211_DRV


void set_slot_time(struct RTL_PRIV *priv, int use_short)
{
	GET_PRIV_IOADDR;
...
void SwChnl(struct RTL_PRIV *priv, unsigned char channel, int offset)
{
#if defined(RTL8190) || defined(RTL8192E)
	...
	if (channel > 14)
		pshare->curr_band = BAND_5G;
	else
		pshare->curr_band = BAND_2G;

	if (pshare->CurrentChannelBW) {
		if (offset == 1)
			val -= 2;
		else
			val += 2;
	...
	if (pshare->rf_ft_var.use_frq_2_3G)
		val += 14;
#if	defined(RTL8192SE)
	{
//		return ; // for 40Mhz
	...
	PHY_RF6052SetOFDMTxPower(priv, priv->pmib->dot11RFEntry.pwrlevelOFDM_1SS[priv->pshare->working_channel-1],
		priv->pmib->dot11RFEntry.pwrlevelOFDM_2SS[pshare->working_channel-1]);

	if (!pshare->use_default_para) {	
#ifdef ADD_TX_POWER_BY_CMD
		if (pshare->rf_ft_var.txPowerPlus_cck != 0xff) 
...
void Switch_1SS_Antenna(struct RTL_PRIV *priv, unsigned int antPath )
{
	GET_PRIV_IOADDR;
...
// switch OFDM path
//antPath: 01 for PathA,10 for PathB, 11for Path AB
void Switch_OFDM_Antenna(struct RTL_PRIV *priv, unsigned int antPath )
{
	GET_PRIV_IOADDR;
	unsigned int dword = 0;
	if(get_rf_mimo_mode(priv) != MIMO_2T2R)
...
void enable_hw_LED(struct RTL_PRIV *priv, unsigned int led_type)
{
	GET_PRIV_IOADDR;
...
#elif defined(RTL8192SE) && !defined(NEW_MAC80211_DRV)
	switch (led_type) {
	case LEDTYPE_HW_TX_RX:
		RTL_W8(LEDCFG, 0x64);
		break;
...
*/
void phy_InitBBRFRegisterDefinition(struct RTL_PRIV *priv)
{
	struct rtl8190_hw *phw = GET_HW(priv);
...
void check_MIMO_TR_status(struct RTL_PRIV *priv)
{
#ifdef RTL8190
	unsigned long ioaddr = priv->pshare->ioaddr;
	int MIMO_TR_status;
...
void PHY_RF6052SetOFDMTxPower(struct RTL_PRIV *priv, unsigned char powerlevel_1ss, unsigned char powerlevel_2ss)
{
//	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
...
#if defined(HIGH_POWER_EXT_PA) && !defined(NEW_MAC80211_DRV)
	if(pshare->rf_ft_var.use_ext_pa)
...
#if defined(ADD_TX_POWER_BY_CMD) && !defined(NEW_MAC80211_DRV)
		byte0 = byte1 = byte2 = byte3 = 0;
		ASSIGN_TX_POWER_OFFSET(byte0, pshare->rf_ft_var.txPowerPlus_ofdm_18);
		ASSIGN_TX_POWER_OFFSET(byte1, pshare->rf_ft_var.txPowerPlus_ofdm_12);
		ASSIGN_TX_POWER_OFFSET(byte2, pshare->rf_ft_var.txPowerPlus_ofdm_9);
		ASSIGN_TX_POWER_OFFSET(byte3, pshare->rf_ft_var.txPowerPlus_ofdm_6);
		writeVal = (byte0<<24) | (byte1<<16) |(byte2<<8) | byte3;
		if (writeVal)
			RTL_W32(0xe00, RTL_R32(0xe00)+writeVal);
		
		byte0 = byte1 = byte2 = byte3 = 0;
		ASSIGN_TX_POWER_OFFSET(byte0, pshare->rf_ft_var.txPowerPlus_ofdm_54);
		ASSIGN_TX_POWER_OFFSET(byte1, pshare->rf_ft_var.txPowerPlus_ofdm_48);
		ASSIGN_TX_POWER_OFFSET(byte2, pshare->rf_ft_var.txPowerPlus_ofdm_36);
		ASSIGN_TX_POWER_OFFSET(byte3, pshare->rf_ft_var.txPowerPlus_ofdm_24);
		writeVal = (byte0<<24) | (byte1<<16) |(byte2<<8) | byte3;
		if (writeVal)
			RTL_W32(0xe04, RTL_R32(0xe04)+writeVal);
	
		byte0 = 0;
		ASSIGN_TX_POWER_OFFSET(byte0, pshare->rf_ft_var.txPowerPlus_cck);
		writeVal = (byte0<<8);
		if (writeVal)
			RTL_W16(0xe08, RTL_R16(0xe08)+(unsigned short)writeVal);

		byte0 = byte1 = byte2 = byte3 = 0;
		ASSIGN_TX_POWER_OFFSET(byte0, pshare->rf_ft_var.txPowerPlus_mcs_3);
		ASSIGN_TX_POWER_OFFSET(byte1, pshare->rf_ft_var.txPowerPlus_mcs_2);
		ASSIGN_TX_POWER_OFFSET(byte2, pshare->rf_ft_var.txPowerPlus_mcs_1);
		ASSIGN_TX_POWER_OFFSET(byte3, pshare->rf_ft_var.txPowerPlus_mcs_0);
		writeVal = (byte0<<24) | (byte1<<16) |(byte2<<8) | byte3;
		if (writeVal)
			RTL_W32(0xe10, RTL_R32(0xe10)+writeVal);
		
		byte0 = byte1 = byte2 = byte3 = 0;
		ASSIGN_TX_POWER_OFFSET(byte0, pshare->rf_ft_var.txPowerPlus_mcs_7);
		ASSIGN_TX_POWER_OFFSET(byte1, pshare->rf_ft_var.txPowerPlus_mcs_6);
		ASSIGN_TX_POWER_OFFSET(byte2, pshare->rf_ft_var.txPowerPlus_mcs_5);
		ASSIGN_TX_POWER_OFFSET(byte3, pshare->rf_ft_var.txPowerPlus_mcs_4);
		writeVal = (byte0<<24) | (byte1<<16) |(byte2<<8) | byte3;
		if (writeVal)
			RTL_W32(0xe14, RTL_R32(0xe14)+writeVal);

		byte0 = byte1 = byte2 = byte3 = 0;
		ASSIGN_TX_POWER_OFFSET(byte0, pshare->rf_ft_var.txPowerPlus_mcs_11);
		ASSIGN_TX_POWER_OFFSET(byte1, pshare->rf_ft_var.txPowerPlus_mcs_10);
		ASSIGN_TX_POWER_OFFSET(byte2, pshare->rf_ft_var.txPowerPlus_mcs_9);
		ASSIGN_TX_POWER_OFFSET(byte3, pshare->rf_ft_var.txPowerPlus_mcs_8);
		writeVal = (byte0<<24) | (byte1<<16) |(byte2<<8) | byte3;
		if (writeVal)
			RTL_W32(0xe18, RTL_R32(0xe18)+writeVal);

		byte0 = byte1 = byte2 = byte3 = 0;
		ASSIGN_TX_POWER_OFFSET(byte0, pshare->rf_ft_var.txPowerPlus_mcs_15);
		ASSIGN_TX_POWER_OFFSET(byte1, pshare->rf_ft_var.txPowerPlus_mcs_14);
		ASSIGN_TX_POWER_OFFSET(byte2, pshare->rf_ft_var.txPowerPlus_mcs_13);
		ASSIGN_TX_POWER_OFFSET(byte3, pshare->rf_ft_var.txPowerPlus_mcs_12);
		writeVal = (byte0<<24) | (byte1<<16) |(byte2<<8) | byte3;
		if (writeVal)
	...
	if (pshare->legacyOFDM_pwrdiff_A & 0x80)
		powerBase0 = powerlevel_1ss - (pshare->legacyOFDM_pwrdiff_A & 0x0f);
	else
		powerBase0 = powerlevel_1ss + (pshare->legacyOFDM_pwrdiff_A & 0x0f);

	for(index=0; index<2; index++)
	{
		byte0 = powerBase0 + pshare->phw->OFDMTxAgcOffset[0+index*4];
		byte1 = powerBase0 + pshare->phw->OFDMTxAgcOffset[1+index*4];
		byte2 = powerBase0 + pshare->phw->OFDMTxAgcOffset[2+index*4];
		byte3 = powerBase0 + pshare->phw->OFDMTxAgcOffset[3+index*4];
	...
	for(index=0; index<4; index++)
	{
		...
		byte0 = powerBase1 + pshare->phw->MCSTxAgcOffset[0+index*4];
		byte1 = powerBase1 + pshare->phw->MCSTxAgcOffset[1+index*4];
		byte2 = powerBase1 + pshare->phw->MCSTxAgcOffset[2+index*4];
		byte3 = powerBase1 + pshare->phw->MCSTxAgcOffset[3+index*4];
...
#ifndef NEW_MAC80211_DRV	//brian
	if (priv->pmib->dot11RFEntry.pwrlevelOFDM_2SS[pshare->working_channel-1+14] != 0)
		pshare->channelAB_pwrdiff =
		        (int)priv->pmib->dot11RFEntry.pwrlevelOFDM_2SS[pshare->working_channel-1+14] -
		        (int)priv->pmib->dot11RFEntry.pwrlevelOFDM_2SS[pshare->working_channel-1];
	else
		pshare->channelAB_pwrdiff = 0;
#endif
	PHY_SetBBReg(priv, rFPGA0_TxGainStage, bXBTxAGC, (pshare->channelAB_pwrdiff & 0x0f));
#if 0
	//Legacy OFDM rates
	powerBase0 = powerlevel + pshare->legacyOFDM_pwrdiff;
...
int phy_BB8192SE_Config_ParaFile(struct RTL_PRIV *priv)
{
//	unsigned long ioaddr = priv->pshare->ioaddr;
...
int phy_RF8225_Config_ParaFile(struct RTL_PRIV *priv)
{
	int rtStatus=0;
	RF90_RADIO_PATH_E eRFPath;
...
//	unsigned char RetryTimes;
	...
	pshare->phw->NumTotalRFPath = 2; // 8192SE only 1T2R or 2T2R, joshua

	for (eRFPath = RF90_PATH_A; eRFPath<pshare->phw->NumTotalRFPath; eRFPath++)
	{
...
*/
		...
		pPhyReg = &pshare->phw->PHYRegDef[eRFPath];
...
void check_and_set_ampdu_spacing(struct RTL_PRIV *priv, struct stat_info *pstat)
{
	GET_PRIV_IOADDR;
	...
	if (pshare->min_ampdu_spacing < _HTCAP_AMPDU_SPC_16_US_) {
		if (priv->pmib->dot1180211AuthEntry.dot11PrivacyAlgrthm != 0) {
			pshare->min_ampdu_spacing = _HTCAP_AMPDU_SPC_16_US_;
			...
			if (ampdu_des > pshare->min_ampdu_spacing) {
				pshare->min_ampdu_spacing = ampdu_des;
...
static void rtl8192SE_ReadIMG(struct RTL_PRIV *priv)
{
...
/*#endif
	...
	pshare->fw_DMEM_buf = (unsigned char *)fw_ptr;
	fw_ptr += FW_DMEM_SIZE;
#else	//ifndef MERGE_FW
/*#ifndef NEW_MAC80211_DRV
	old_fs = get_fs();
	set_fs(KERNEL_DS);
...
//#ifdef NEW_MAC80211_DRV
	...
	read_bytes += file_read(fp, read_bytes, pshare->fw_DMEM_buf, FW_DMEM_SIZE);
/*#else
	sys_read(fd, swap_arr, RT_8192S_FIRMWARE_HDR_SIZE);
	sys_read(fd, pshare->fw_DMEM_buf, FW_DMEM_SIZE);
#endif*/
	rtl_cache_sync_wback(priv, (unsigned int)pshare->fw_DMEM_buf, RT_8192S_FIRMWARE_HDR_SIZE, PCI_DMA_TODEVICE);
#endif // ifdef MERGE_FW

	rtl8192SE_FW_hdr_swap(swap_arr);
	pFwHdr = (struct _RT_8192S_FIRMWARE_HDR *)swap_arr;

	//set parameters
	pshare->fw_EMEM_len = pFwHdr->IMG_SRAM_SIZE;
	pshare->fw_IMEM_len = pFwHdr->IMG_IMEM_SIZE;
	pshare->fw_DMEM_len = pFwHdr->DMEMSize;
	pshare->fw_version     = pFwHdr->Version;
	pshare->fw_src_version = pFwHdr->source_version;
	pshare->fw_sub_version = pFwHdr->subversion;

	printk("fw_version: %04x(%d.%d), fw_EMEM_len: %d, fw_IMEM_len: %d, fw_DMEM_len: %d\n",
		pshare->fw_version, pshare->fw_src_version, pshare->fw_sub_version,
		pshare->fw_EMEM_len, pshare->fw_IMEM_len, pshare->fw_DMEM_len);

	pFwPriv = (struct _RT_8192S_FIRMWARE_PRIV *)pshare->fw_DMEM_buf;
	// for rf_config filed
	if (get_rf_mimo_mode(priv) == MIMO_1T2R ||get_rf_mimo_mode(priv) == MIMO_1T1R )
	...
	// root Fw beacon
	if (pshare->rf_ft_var.rootFwBeacon && (!pshare->rf_ft_var.mp_specific))
	...
	//read IMEM
#ifdef MERGE_FW
	pshare->fw_IMEM_buf = (unsigned char *)fw_ptr;
	fw_ptr +=  pshare->fw_IMEM_len;
#else
//#ifdef NEW_MAC80211_DRV
	read_bytes += file_read(fp, read_bytes, pshare->fw_IMEM_buf, pshare->fw_IMEM_len);
/*#else
	sys_read(fd, pshare->fw_IMEM_buf, pshare->fw_IMEM_len);
#endif*/
	rtl_cache_sync_wback(priv, (unsigned int)pshare->fw_IMEM_buf,  pshare->fw_IMEM_len, PCI_DMA_TODEVICE);
#endif

	//read EMEM
#ifdef MERGE_FW
	pshare->fw_EMEM_buf = (unsigned char *)fw_ptr;
	fw_ptr +=  pshare->fw_EMEM_len;
#else
/*#ifndef NEW_MAC80211_DRV
	sys_read(fd, pshare->fw_EMEM_buf, pshare->fw_EMEM_len);
#else*/
	read_bytes += file_read(fp, read_bytes, pshare->fw_EMEM_buf, pshare->fw_EMEM_len);
//#endif
	rtl_cache_sync_wback(priv, (unsigned int)pshare->fw_EMEM_buf,  pshare->fw_EMEM_len, PCI_DMA_TODEVICE);
#endif
 ...
 *
 *---------------------------------------------------------------------------*/
static	void	fw_SetRQPN(struct RTL_PRIV *priv)
{
...
static void LoadIMG(struct RTL_PRIV *priv, int fw_file)
{
	GET_PRIV_IOADDR;
	int bResult=0;
...
//	fw_SetRQPN(priv);	// For 92SE only
	...
	switch(fw_file) {
	case LOAD_IMEM:
		mem_ptr = pshare->fw_IMEM_buf;
		buf_len = pshare->fw_IMEM_len;
		break;
	case LOAD_EMEM:
		mem_ptr = pshare->fw_EMEM_buf;
		buf_len = pshare->fw_EMEM_len;
		break;
	case LOAD_DMEM:
		mem_ptr = pshare->fw_DMEM_buf;
		buf_len = pshare->fw_DMEM_len;
		break;
	default:
...
//
unsigned int FirmwareEnableCPU(struct RTL_PRIV *priv)
{
...
static int FirmwareCheckReady(struct RTL_PRIV *priv,int fw_file)
//static int FirmwareCheckReady(struct mac80211_shared_priv *priv,int fw_file)
{
	unsigned int rtStatus = SUCCESS;
	unsigned int PollingCnt = 100;
	unsigned char	 	/*tmpU1b, */ CPUStatus = 0;
	unsigned int		tmpU4b;
...
//			RT_TRACE(COMP_INIT, DBG_LOUD, ("Enable CPU fail ! \n") );
	...
	else if( fw_file == LOAD_DMEM)
	{
		...
		if(!(CPUStatus & DMEM_CODE_DONE)){
			printk("Check DMEM fail!, CPU Stats = %x, PollingCnt = %x\n", CPUStatus, PollingCnt);
			return FAIL;
		}
...
int LoadIMEMIMG(struct RTL_PRIV *priv)
{
...
int LoadDMEMIMG(struct RTL_PRIV *priv)
{
...
int LoadEMEMIMG(struct RTL_PRIV *priv)
{
...
//
unsigned char
HalSetSysClk8192SE(struct RTL_PRIV *priv,	unsigned char Data)
{
	GET_PRIV_IOADDR;
 ...
 *
 *---------------------------------------------------------------------------*/
static void  MacConfigBeforeFwDownload(struct RTL_PRIV *priv)
{
	unsigned char		i;
	unsigned char		tmpU1b;
	unsigned short		tmpU2b;
...
//	RT_TRACE(COMP_INIT, DBG_LOUD, 	("Enable AFE Mbias\r\n"));
	...
	//printk("sysiso = %x\n", ioread32((void *)priv->pshare->ioaddr));
	...
	delay_ms(1);
//	delay_us(100);
	RTL_W8(AFE_PLL_CTRL, (tmpU1b|BIT(0)|BIT(4)|BIT(6)));
	delay_ms(1);
//	delay_us(10);
	RTL_W8(AFE_PLL_CTRL, (tmpU1b|BIT(0)|BIT(4)));
	delay_ms(1);
...
//	RT_TRACE(COMP_INIT, DBG_LOUD, ("<---MacConfigBeforeFwDownload()\n"));

	RTL_W16(BCN_DRV_EARLY_INT, (10<<4)); // 2
#if defined(FW_SW_BEACON) && !defined(NEW_MAC80211_DRV)
 ...
 *
 *---------------------------------------------------------------------------*/
static void MacConfigAfterFwDownload(struct RTL_PRIV *priv)
{
//	unsigned char	i;
//	unsigned char	tmpU1b;
//	unsigned short	tmpU2b;
...
int rtl819x_init_hw_PCI(struct RTL_PRIV *priv)
{
...
//	GetHardwareVersion(priv); //we don't need to get hw version under 8192SE yet
	...
	ioaddr = pshare->ioaddr;
#ifdef DW_FW_BY_MALLOC_BUF
	if ((pshare->fw_IMEM_buf = kmalloc(FW_IMEM_SIZE, GFP_ATOMIC)) == NULL) {
		DEBUG_ERR("alloc fw_IMEM_buf failed!\n");
		return -1;
	}
	if ((pshare->fw_EMEM_buf = kmalloc(FW_EMEM_SIZE, GFP_ATOMIC)) == NULL) {
		DEBUG_ERR("alloc fw_EMEM_buf failed!\n");
		return -1;
	}
	if ((pshare->fw_DMEM_buf = kmalloc(FW_DMEM_SIZE, GFP_ATOMIC)) == NULL) {
...
//1 For Test, Firmware Downloading
	...
	if((pshare->txcmd_buf = (unsigned char *)kmalloc((LoadPktSize), GFP_ATOMIC)) == NULL) {
		printk("not enough memory for txcmd_buf\n");
		return -1;
	}

	pshare->cmdbuf_phyaddr = get_physical_addr(priv, pshare->txcmd_buf,
...
//		delay_ms(20);
	...
	//
#if 0
//#if defined(DRVMAC_LB) && !defined(NEW_MAC80211_DRV)
	if (!priv->pmib->miscEntry.drvmac_lb)
#endif
	{
		val32 = phy_BB8192SE_Config_ParaFile(priv);
		//printk("phy_BB8192SE_Config_ParaFile done\n");
		if (val32){
			printk("return -1\n");
			return -1;
	}
...
//			so far, 8192SE does not need to initialize BB or RF
	...
	if(pshare->rf_ft_var.use_ext_pa)
	{
		...
		if (get_rf_mimo_mode(priv) == MIMO_1T2R || get_rf_mimo_mode(priv) == MIMO_1T1R)
		{
			if(pshare->rf_ft_var.pathB_1T == 0)	// turn off PATH B
			{
...
#ifdef DW_FW_BY_MALLOC_BUF
	kfree(pshare->fw_IMEM_buf);
	kfree(pshare->fw_EMEM_buf);
	kfree(pshare->fw_DMEM_buf);
#endif
...
/*
	...
	if (IS_PCIBIOS_TYPE)
		pci_unmap_single(pshare->pdev, pshare->cmdbuf_phyaddr,
			(LoadPktSize), PCI_DMA_TODEVICE);
#endif
*/
#if	0
...
//	RTL_W16(BCNTCFG, 0x0204);
	...
	pshare->InterruptMask = (IMR_ROK | IMR_VODOK | IMR_VIDOK | IMR_BEDOK | IMR_BKDOK |		\
	IMR_HCCADOK | IMR_MGNTDOK | IMR_COMDOK | IMR_HIGHDOK | 					\
	IMR_BDOK | /*IMR_RXCMDOK | IMR_TIMEOUT0 |*/ IMR_RDU | IMR_RXFOVW	|			\
	IMR_BcnInt/* | IMR_TXFOVW*/ /*| IMR_TBDOK | IMR_TBDER*/) ;// IMR_ROK | IMR_BcnInt | IMR_RDU | IMR_RXFOVW | IMR_RXCMDOK;

//	pshare->InterruptMask = IMR_ROK| IMR_BDOK | IMR_BcnInt | IMR_MGNTDOK | IMR_TBDOK | IMR_RDU ;
//	pshare->InterruptMask  = 0;
	pshare->InterruptMaskExt = 0;
	RTL_W32(IMR, pshare->InterruptMask);
	RTL_W32(IMR+4, pshare->InterruptMaskExt);
...
#else	//Mesh Mode but mesh not enable
...
//	RTL_W16(_BCNTCFG_, 0x060a);
	...
	RTL_W32(IMR, 0x0);
	RTL_W32(IMR+4, 0x0);
	pshare->InterruptMask = _ROK_ | _BCNDMAINT_ | _RDU_ | _RXFOVW_ | _RXCMDOK_;
	pshare->InterruptMask = (IMR_ROK | IMR_VODOK | IMR_VIDOK | IMR_BEDOK | IMR_BKDOK |		\
	IMR_HCCADOK | IMR_MGNTDOK | IMR_COMDOK | IMR_HIGHDOK | 					\
	IMR_BDOK | IMR_RXCMDOK | /*IMR_TIMEOUT0 |*/ IMR_RDU | IMR_RXFOVW	|			\
	IMR_BcnInt/* | IMR_TXFOVW*/ /*| IMR_TBDOK | IMR_TBDER*/);// IMR_ROK | IMR_BcnInt | IMR_RDU | IMR_RXFOVW | IMR_RXCMDOK;
#endif
	pshare->InterruptMask = IMR_ROK | IMR_BcnInt |IMR_RDU | IMR_RXFOVW ;
#ifdef MP_TEST
	pshare->InterruptMask	|= IMR_BEDOK;
#endif
	pshare->InterruptMaskExt = 0;

#ifndef NEW_MAC80211_DRV
	if (opmode & WIFI_AP_STATE)
		pshare->InterruptMask |= IMR_BDOK;
#ifdef CLIENT_MODE
	else if (opmode & WIFI_ADHOC_STATE)
		pshare->InterruptMaskExt |= (IMR_TBDER | IMR_TBDOK);
#endif
...
//		RTL_W8(LBKMD_SEL, BIT(0)| BIT(1) | BIT(3));
	...
	if (priv->reset_hangup)
		pshare->CurrentChannelBW = pshare->is_40m_bw;
	else
#endif
	{
		if (opmode & WIFI_AP_STATE)
			...
			pshare->CurrentChannelBW = HT_CHANNEL_WIDTH_20;
	}
	...
	if ((priv->pmib->dot11RFEntry.LOFDM_pwd_A == 0) ||
		...
		pshare->use_default_para = 1;
		pshare->legacyOFDM_pwrdiff_A = 0;
		pshare->legacyOFDM_pwrdiff_B = 0;
	}
	else
	{
		pshare->use_default_para = 0;

		if (priv->pmib->dot11RFEntry.LOFDM_pwd_A >= 32)
			pshare->legacyOFDM_pwrdiff_A = (priv->pmib->dot11RFEntry.LOFDM_pwd_A - 32) & 0x0f;
		else {
			pshare->legacyOFDM_pwrdiff_A = (32 - priv->pmib->dot11RFEntry.LOFDM_pwd_A) & 0x0f;
			pshare->legacyOFDM_pwrdiff_A |= 0x80;
		}

		if (priv->pmib->dot11RFEntry.LOFDM_pwd_B >= 32)
			pshare->legacyOFDM_pwrdiff_B = (priv->pmib->dot11RFEntry.LOFDM_pwd_B - 32) & 0x0f;
		else {
			pshare->legacyOFDM_pwrdiff_B = (32 - priv->pmib->dot11RFEntry.LOFDM_pwd_B) & 0x0f;
			pshare->legacyOFDM_pwrdiff_B |= 0x80;
		}
	}
	}
	else
	{
		...
		else
		{
			...
			if (priv->pmib->dot11RFEntry.LOFDM_pwd_B != 0) {
				...
				pshare->legacyOFDM_pwrdiff_B = pshare->legacyOFDM_pwrdiff_A;
		}
	}
#endif //ifndef NEW_MAC80211_DRV
	// get default Tx AGC offset
	*(unsigned int *)(&pshare->phw->MCSTxAgcOffset[0])  = RTL_R32(rTxAGC_Mcs03_Mcs00);
	*(unsigned int *)(&pshare->phw->MCSTxAgcOffset[4])  = RTL_R32(rTxAGC_Mcs07_Mcs04);
	*(unsigned int *)(&pshare->phw->MCSTxAgcOffset[8])  = RTL_R32(rTxAGC_Mcs11_Mcs08);
	*(unsigned int *)(&pshare->phw->MCSTxAgcOffset[12]) = RTL_R32(rTxAGC_Mcs15_Mcs12);
	*(unsigned int *)(&pshare->phw->OFDMTxAgcOffset[0]) = RTL_R32(rTxAGC_Rate18_06);
	*(unsigned int *)(&pshare->phw->OFDMTxAgcOffset[4]) = RTL_R32(rTxAGC_Rate54_24);

#ifdef ADD_TX_POWER_BY_CMD
	ASSIGN_TX_POWER_OFFSET(pshare->phw->OFDMTxAgcOffset[0], pshare->rf_ft_var.txPowerPlus_ofdm_18);
	ASSIGN_TX_POWER_OFFSET(pshare->phw->OFDMTxAgcOffset[1], pshare->rf_ft_var.txPowerPlus_ofdm_12);
	ASSIGN_TX_POWER_OFFSET(pshare->phw->OFDMTxAgcOffset[2], pshare->rf_ft_var.txPowerPlus_ofdm_9);
	ASSIGN_TX_POWER_OFFSET(pshare->phw->OFDMTxAgcOffset[3], pshare->rf_ft_var.txPowerPlus_ofdm_6);
	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->OFDMTxAgcOffset[4], pshare->rf_ft_var.txPowerPlus_ofdm_54);
	ASSIGN_TX_POWER_OFFSET(pshare->phw->OFDMTxAgcOffset[5], pshare->rf_ft_var.txPowerPlus_ofdm_48);	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->OFDMTxAgcOffset[6], pshare->rf_ft_var.txPowerPlus_ofdm_36);
	ASSIGN_TX_POWER_OFFSET(pshare->phw->OFDMTxAgcOffset[7], pshare->rf_ft_var.txPowerPlus_ofdm_24);
	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[0], pshare->rf_ft_var.txPowerPlus_mcs_3);
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[1], pshare->rf_ft_var.txPowerPlus_mcs_2);	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[2], pshare->rf_ft_var.txPowerPlus_mcs_1);	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[3], pshare->rf_ft_var.txPowerPlus_mcs_0);
	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[4], pshare->rf_ft_var.txPowerPlus_mcs_7);	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[5], pshare->rf_ft_var.txPowerPlus_mcs_6);	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[6], pshare->rf_ft_var.txPowerPlus_mcs_5);	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[7], pshare->rf_ft_var.txPowerPlus_mcs_4);
	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[8], pshare->rf_ft_var.txPowerPlus_mcs_11);
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[9], pshare->rf_ft_var.txPowerPlus_mcs_10);	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[10], pshare->rf_ft_var.txPowerPlus_mcs_9);	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[11], pshare->rf_ft_var.txPowerPlus_mcs_8);	
	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[12], pshare->rf_ft_var.txPowerPlus_mcs_15);	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[13], pshare->rf_ft_var.txPowerPlus_mcs_14);
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[14], pshare->rf_ft_var.txPowerPlus_mcs_13);	
	ASSIGN_TX_POWER_OFFSET(pshare->phw->MCSTxAgcOffset[15], pshare->rf_ft_var.txPowerPlus_mcs_12);
#endif // ADD_TX_POWER_BY_CMD
...
#ifndef NEW_MAC80211_DRV
	SwBWMode(priv, pshare->CurrentChannelBW, pshare->offset_2nd_chan);
	SwChnl(priv, priv->pmib->dot11RFEntry.dot11channel, pshare->offset_2nd_chan);
#endif //ifndef NEW_MAC80211_DRV
	if(pshare->rf_ft_var.ofdm_1ss_oneAnt == 1){// use one PATH for ofdm and 1SS
...
void rtl8192se_ePhyInit(struct RTL_PRIV *priv)
{
	GET_PRIV_IOADDR;
...
#ifdef NEW_MAC80211_DRV
	priv->pshare->fw_version, priv->pshare_hw->fw_src_version, priv->pshare_hw->fw_sub_version);
#else
...
int rtl819x_stop_hw(struct RTL_PRIV *priv, int reset_bb)
{
	static RF90_RADIO_PATH_E eRFPath;
...
void SwBWMode(struct RTL_PRIV *priv, unsigned int bandwidth, int offset)
{
...
void init_EDCA_para(struct RTL_PRIV *priv, int mode)
{
...
#endif
#if defined(SEMI_QOS) && !defined(NEW_MAC80211_DRV)
	if (QOS_ENABLE)
...
void BE_switch_to_VI(struct rtl8190_priv *priv, int mode, char enable)
{
	...
	if (!enable) {
#if defined(RTL8192SE) && !defined(NEW_MAC80211_DRV)
...
#ifdef DWA125_IOT
	if(pstat->is_ralink_sta)
	  set_fw_reg(priv, 0xfd003614, 0, 0);
	else
#endif	
...
static UCHAR CAM_find_usable(struct RTL_PRIV *priv)
{
	GET_PRIV_IOADDR;
	ULONG ulCommand=0;
	ULONG ulContent=0;
...
static void CAM_program_entry(struct RTL_PRIV *priv,UCHAR index, UCHAR* macad,UCHAR* key128, USHORT config)
{
	GET_PRIV_IOADDR;
...
#ifndef NEW_MAC80211_DRV
int CamAddOneEntry(struct RTL_PRIV *priv,UCHAR *pucMacAddr, ULONG ulKeyId, ULONG ulEncAlg, ULONG ulUseDK, UCHAR *pucKey)
{
	UCHAR retVal = 0;
    UCHAR ucCamIndex = 0;
    USHORT usConfig = 0;
...
void CAM_read_mac_config(struct RTL_PRIV *priv,UCHAR ucIndex, UCHAR* pucMacad, USHORT* pusTempConfig)
{
	GET_PRIV_IOADDR;
	ULONG ulCommand=0;
	ULONG ulContent=0;
...
void CAM_empty_entry(struct RTL_PRIV *priv,UCHAR ucIndex)
{
	GET_PRIV_IOADDR;
	ULONG ulCommand=0;
	ULONG ulContent=0;
...
int CamDeleteOneEntry(struct RTL_PRIV *priv,UCHAR *pucMacAddr, ULONG ulKeyId, unsigned int useDK)
{
	UCHAR ucIndex;
	UCHAR ucTempMAC[6];
...
void CamResetAllEntry(struct RTL_PRIV *priv)
{
	UCHAR ucIndex;
...
void CAM_read_entry(struct RTL_PRIV *priv,UCHAR index, UCHAR* macad,UCHAR* key128, USHORT* config)
{
	GET_PRIV_IOADDR;
	ULONG target_command=0;
	ULONG target_content=0;
...
__NOMIPS16
#endif
void CamDumpAll(struct RTL_PRIV *priv)
{
...
__NOMIPS16
#endif
void CamDump4(struct RTL_PRIV *priv)
{
	UCHAR TempOutputMac[6];
	UCHAR TempOutputKey[16];
	USHORT TempOutputCfg=0;
