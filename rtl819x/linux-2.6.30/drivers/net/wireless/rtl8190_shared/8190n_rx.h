 ...
 *
 *      $Id: 8190n_rx.h,v 1.32 2009/07/08 05:40:11 button Exp $
 */
...
#if defined(CONFIG_RTL865XC) && !defined(NEW_MAC80211_DRV)
...
static __inline__ void init_rxdesc(struct sk_buff *pskb, int i, struct RTL_PRIV *priv)
{
	struct rtl8190_hw	*phw;
	struct rx_frinfo	*pfrinfo;
	int offset;
	...
	init_frinfo(pfrinfo);
#if (defined(UNIVERSAL_REPEATER) || defined(MBSSID)) && !defined(NEW_MAC80211_DRV)
	pfrinfo->is_br_mgnt = 0;
#endif
	...
	phw->rx_descL[i].Dword0 = set_desc((i == (NUM_RX_DESC - 1)? RX_EOR : 0) | RX_OWN | ((MAX_RX_BUF_LEN - sizeof(struct rx_frinfo)) & RX_LengthMask));
#if defined(CONFIG_RTL865XC) && !defined(NEW_MAC80211_DRV)
#if defined(RTL8190_RXRING_RANGEBASE_DCACHE_FLUSH)
	rtl_cache_sync_wback(priv, phw->rx_descL_dma_addr[i], sizeof(struct rx_desc), PCI_DMA_TODEVICE);
#else
	rtlglue_flushDCache(0, 0);
#endif
