 ...
 *      Utility routines
 *
 *      $Id: 8190n_util.c,v 1.170 2009/07/20 01:54:33 button Exp $
 */
...
unsigned char	remapped_aidarray[NUM_STAT];
 ...
 static
#endif
__inline__ void release_buf_to_poll(struct rtl8190_priv *priv, unsigned char *pbuf, struct list_head	*phead, unsigned int *count)
{
	unsigned long flags;
 ...
 static
#endif
__inline__ unsigned char *get_buf_from_poll(struct rtl8190_priv *priv, struct list_head *phead, unsigned int *count)
{
	unsigned long flags;
	unsigned char *buf;
	struct list_head *plist;
...
unsigned int find_rate_MP(struct rtl8190_priv *priv, struct stat_info *pstat, struct ht_cap_elmt * peer_ht_cap,  int peer_ht_cap_len, char *peer_rate,int peer_rate_len,int mode, int isBasicRate)
{
...
void (*wapiReleaseFragementQueue_hook)(wapiStaInfo *wapiInfo);
EXPORT_SYMBOL(wapiReleaseFragementQueue_hook);
#endif
...
//yschen 2009-03-04
	...
	if (pstat->wapiInfo)
	{
		...
		#if defined(CONFIG_RTL_WAPI_MODULE_SUPPORT)
			if(wapiReleaseFragementQueue_hook != NULL)	wapiReleaseFragementQueue_hook(pstat->wapiInfo);
		#else
...
int should_restrict_Nrate(struct rtl8190_priv *priv, struct stat_info *pstat)
{
...
// Client mode IOT issue, Button 2009.07.17
	...
	{
		if(!pstat->is_rtl8190_sta && !pstat->is_marvell_sta && pstat->is_legacy_encrpt)
		return 1; 
	}
#endif
 ...
 */
int get_assoc_sta_num(struct rtl8190_priv *priv)
{
	struct list_head *phead, *plist;
	struct stat_info *pstat;
	int sta_num;
...
#if defined(CONFIG_RTL_WAPI_MODULE_SUPPORT)
EXPORT_SYMBOL(wapi_event_indicate);
#endif
#endif
...
// Client mode IOT issue, Button 2009.07.17
	...
	if ((priv->pmib->dot11BssType.net_work_type & WIRELESS_11N) && pstat->ht_cap_len) {
		if (((pstat->tx_ra_bitmap & 0xff00000) && !(pstat->tx_ra_bitmap & 0x8000000)) ||
			...
			!(pstat->tx_ra_bitmap & 0x80000)) || (!IS_ROOT_INTERFACE(priv) && 
...
#if defined(CONFIG_RTL_WAPI_MODULE_SUPPORT)
EXPORT_SYMBOL(alloc_skb_from_queue);
#endif
