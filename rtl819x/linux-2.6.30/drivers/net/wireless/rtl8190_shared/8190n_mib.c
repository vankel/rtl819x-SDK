 ...
 *      SNMP MIB module
 *
 *      $Id: 8190n_mib.c,v 1.2 2008/07/22 12:58:36 jasonwang Exp $
 */
#define _8190N_MIB_C_
...
static int add_mib_entry(struct rtl8190_priv *priv, struct mib_val *ent)
{
	int i=0;
...
static struct mib_val *search_mib(struct rtl8190_priv *priv, char *id)
{
	int i=0;
	struct mib_entry *mib=priv->snmp_mib.mib_tree, *pre_mib=NULL;
	int oid[100];
...
static void build_tree(struct rtl8190_priv *priv)
{
	int i=0;
...
int mib_get(struct rtl8190_priv *priv, char *oid, unsigned char *data, int *pLen)
{
 	int iVal;
...
void mib_init(struct rtl8190_priv *priv)
{
	int val;
