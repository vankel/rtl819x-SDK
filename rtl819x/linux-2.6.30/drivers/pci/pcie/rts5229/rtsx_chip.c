...
static int rtsx_init_ocp(struct rtsx_chip *chip)
{
	...
	RTSX_WRITE_REG(chip, ocp_para2, SD_OCP_THD_MASK, chip->sd_400mA_ocp_thd);
		...
		struct rts5229_auto_load_map *map_item, u8 cnt)
{
	...
	if (CHECK_PID(chip, 0x5229)) {
		 ...
		 */
		retval = rtsx_write_cfg_dw(chip, 0, 0x814, 0xFF000000, 0x80000000);
		if (retval != STATUS_SUCCESS)
			TRACE_RET(chip, STATUS_FAIL);
	}
...
static int rtsx_init_from_hw(struct rtsx_chip *chip)
{
	...
	if (chip->hp_watch_bios_hotplug && CHK_AUTODELINK_EN(chip)) {
		...
		if (rtsx_read_pci_cfg_byte(chip, 0x00, 0x1C, 0x02, 0x58, &reg58) < 0) {
			return STATUS_SUCCESS;
		}
		if (rtsx_read_pci_cfg_byte(chip, 0x00, 0x1C, 0x02, 0x5B, &reg5b) < 0) {
			return STATUS_SUCCESS;
		}
		...
		if (chip->auto_delink_cnt <= delink_stage3_cnt) {
			if (chip->auto_delink_cnt == delink_stage1_cnt) {
				...
				if (chip->card_exist) {
					...
					if (CHECK_PID(chip, 0x5227))
						rtsx_write_register(chip, AUTOLOAD_CFG, 0x40, 0x00);
...
void rtsx_set_stat(struct rtsx_chip *chip, enum RTSX_STAT stat)
{
	...
	if (chip->rtsx_stat != stat) {
		chip->rtsx_stat = stat;
#ifdef LED_AUTO_BLINK
		if (!chip->led_test_mode && (stat == RTSX_STAT_RUN) && chip->blink_led) {
