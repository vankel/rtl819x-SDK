		...
		u16 byte_cnt, u16 blk_cnt, u8 bus_width, u8 *buf, int buf_len, int timeout)
{
	...
	if (cmd_len) {
		...
		for (i = 0; i < (cmd_len < 6 ? cmd_len : 6); i++) {
			rtsx_add_cmd(chip, WRITE_REG_CMD, SD_CMD0 + i, 0xFF, cmd[i]);
		}
	}
	rtsx_add_cmd(chip, WRITE_REG_CMD, SD_BYTE_CNT_L, 0xFF, (u8)byte_cnt);
	rtsx_add_cmd(chip, WRITE_REG_CMD, SD_BYTE_CNT_H, 0xFF, (u8)(byte_cnt >> 8));
	rtsx_add_cmd(chip, WRITE_REG_CMD, SD_BLOCK_CNT_L, 0xFF, (u8)blk_cnt);
	rtsx_add_cmd(chip, WRITE_REG_CMD, SD_BLOCK_CNT_H, 0xFF, (u8)(blk_cnt >> 8));
	...
	if (voltage == SD_IO_3V3) {
		...
		if (chip->asic_code) {
			...
			if (CHECK_PID(chip, 0x5227)) {
				retval = rtsx_write_phy_register(chip, 0x11, 0x3C02);
				if (retval != STATUS_SUCCESS)
					TRACE_RET(chip, STATUS_FAIL);
				phy = 0x4C80;
			} else {
			...
			retval = rtsx_write_phy_register(chip, 0x08, phy | chip->phy_voltage);
			if (retval != STATUS_SUCCESS) {
				TRACE_RET(chip, STATUS_FAIL);
			}
		} else {
...
static int sd_switch_function(struct rtsx_chip *chip, u8 bus_width)
{
	...
	if (func_to_switch) {
		...
		if (func_to_switch == SDR104_SUPPORT) {
			...
			chip->sd_default_tx_phase = chip->sd_sdr104_default_tx_phase;
			chip->sd_default_rx_phase = chip->sd_sdr104_default_rx_phase;
		} else if (func_to_switch == DDR50_SUPPORT) {
			SET_SD_DDR50(sd_card);
			chip->sd_default_tx_phase = chip->sd_ddr50_default_tx_phase;
			chip->sd_default_rx_phase = chip->sd_ddr50_default_rx_phase;
		} else if (func_to_switch == SDR50_SUPPORT) {
			SET_SD_SDR50(sd_card);
			chip->sd_default_tx_phase = chip->sd_sdr50_default_tx_phase;
			chip->sd_default_rx_phase = chip->sd_sdr50_default_rx_phase;
		} else {
			SET_SD_HS(sd_card);
		}
	}
...
static int sd_sdr_tuning(struct rtsx_chip *chip)
{
	...
	if (chip->sdr_tx_tuning_en) {
		...
		retval = sd_change_phase(chip, (u8)chip->sd_default_tx_phase, TUNE_TX);
		if (retval != STATUS_SUCCESS)
			TRACE_RET(chip, STATUS_FAIL);
	}
...
int sd_set_sd30_drive(struct rtsx_chip *chip, u8 voltage)
{
	...
	if (CHECK_PID(chip, 0x5229)) {
		...
		RTSX_WRITE_REG(chip, SD30_CLK_DRIVE_SEL, 0xFF, sd30_clk_drive_sel);
		RTSX_WRITE_REG(chip, SD30_CMD_DRIVE_SEL, 0xFF, sd30_cmd_drive_sel);
		RTSX_WRITE_REG(chip, SD30_DAT_DRIVE_SEL, 0xFF, sd30_dat_drive_sel);
	}
...
RTY_SD_RST:
	...
	retval = sd_send_cmd_get_rsp(chip, ALL_SEND_CID, 0, SD_RSP_TYPE_R2, sd_card->raw_cid, 16);
	if (retval != STATUS_SUCCESS) {
		TRACE_RET(chip, STATUS_FAIL);
	}
...
#else
		sd_card->capacity = ((u32)buf[215] << 24) | ((u32)buf[214] << 16) |
			((u32)buf[213] << 8) | ((u32)buf[212]);
#endif
	}
...
static void sd_auto_read2(struct rtsx_chip *chip, u32 data_addr, u16 sector_cnt)
{
	struct sd_info *sd_card = &(chip->sd_card);
	u8 cfg2;
	...
	rtsx_add_cmd(chip, WRITE_REG_CMD, SD_BLOCK_CNT_L, 0xFF, (u8)sector_cnt);
	rtsx_add_cmd(chip, WRITE_REG_CMD, SD_BLOCK_CNT_H, 0xFF, (u8)(sector_cnt >> 8));

	rtsx_add_cmd(chip, WRITE_REG_CMD, CARD_DATA_SOURCE, 0x01, RING_BUFFER);

	if (CHK_MMC_8BIT(sd_card)) {
		rtsx_add_cmd(chip, WRITE_REG_CMD, SD_CFG1, 0x03, SD_BUS_WIDTH_8);
...
static int sd_write(struct scsi_cmnd *srb, struct rtsx_chip *chip, u32 start_sector, u16 sector_cnt)
{
	struct sd_info *sd_card = &(chip->sd_card);
	u32 data_addr;
	u8 cfg2;
...
static int sd_read(struct scsi_cmnd *srb, struct rtsx_chip *chip, u32 start_sector, u16 sector_cnt)
{
	struct sd_info *sd_card = &(chip->sd_card);
	u32 data_addr;
	int retval;
...
int sd_execute_no_data(struct scsi_cmnd *srb, struct rtsx_chip *chip)
{
	...
	if ((sd_card->sd_lock_status & SD_LOCK_1BIT_MODE) == 0) {
		if (CHK_MMC_8BIT(sd_card)) {
			retval = rtsx_write_register(chip, SD_CFG1, 0x03, SD_BUS_WIDTH_8);
			if (retval != STATUS_SUCCESS) {
				TRACE_RET(chip, TRANSPORT_FAILED);
			}
...
int sd_execute_read_data(struct scsi_cmnd *srb, struct rtsx_chip *chip)
{
	...
	if (data_len <= 512) {   
		...
		rtsx_add_cmd(chip, WRITE_REG_CMD, SD_CMD0, 0xFF, 0x40 | cmd_idx);
		rtsx_add_cmd(chip, WRITE_REG_CMD, SD_CMD1, 0xFF, srb->cmnd[3]);
		rtsx_add_cmd(chip, WRITE_REG_CMD, SD_CMD2, 0xFF, srb->cmnd[4]);
		rtsx_add_cmd(chip, WRITE_REG_CMD, SD_CMD3, 0xFF, srb->cmnd[5]);
		rtsx_add_cmd(chip, WRITE_REG_CMD, SD_CMD4, 0xFF, srb->cmnd[6]);
