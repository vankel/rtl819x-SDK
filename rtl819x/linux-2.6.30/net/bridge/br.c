...
unsigned int rtl_br0_ip,rtl_br0_mask;
          ...
          if (tokptr != NULL) 
          {
			...
			else
			{
              ...
              if(igmpsnoopenabled)
              {
                  igmpsnoopenabled=1;
				  #if defined(CONFIG_RTL_IPTABLES_FAST_PATH) 
				  get_dev_ip_mask(RTL_PS_BR0_DEV_NAME, &rtl_br0_ip, &rtl_br0_mask);
				  #endif
              }
              else
              {
...
void br_igmpQueryTimerExpired(unsigned long arg)
{
	...
	skb_set_network_header(skb,14);			//set ip header
	skb_set_transport_header(skb,14+20); 	//set transport header
...
void br_mldQueryTimerExpired(unsigned long arg)
{
	...
	else
	{
 	...
 	skb_reset_mac_header(skb); 				//set mac header
	skb_set_network_header(skb,14);			//set ip header
	skb_set_transport_header(skb,14+48); 	//set transport header
