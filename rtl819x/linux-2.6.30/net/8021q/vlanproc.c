...
static int32 vlan_wan_vid_read( char *page, char **start, off_t off, int count, int *eof, void *data );
...
extern void rtk_add_eth_vlan(uint16 vid , uint32 portmask,uint32 tagmask);
...
int vlan_proc_init(struct net *net)
{
	...
	vlan_gp_entry=create_proc_entry("wanVlanId",0,vn->proc_vlan_dir);
	if(vlan_gp_entry)
	{
		vlan_gp_entry->read_proc=vlan_wan_vid_read;
		vlan_gp_entry->write_proc=vlan_wan_vid_write;
	}
	else{
		goto err;
	}
...
{
	int len;
	len = sprintf(page, "vlan enable:%x hw support enable:%d\n",vlan_ctl_p->flag, linux_vlan_hw_support_enable);
	return len;
}
...
static int32 vlan_pvid_write( struct file *filp, const char *buff,unsigned long len,void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		for(i=0;i<num;i++){
			...
			#if defined(CONFIG_RTL_8367R_SUPPORT)
			if(i<9 &&  pvid[i]){
				rtk_vlan_portPvid_set(i, vlan_ctl_p->pvid[i], 0);
			}
			#endif
		}
...
static void rtk_init_vlan_tbl(void)
{
   ...
   linux_vlan_group_t *vlan_group_p=NULL;	
	...
	vlan_group_p->tagMemberMask= 0 ;			
     ...
     vlan_ctl_p->pvid[RTK_CPU_PORT_IDX] = RTL_WANVLANID;   //mark_vc  , wan port pvid , wifi pvid ?
...
static int32 vlan_ctl_write( struct file *filp, const char *buff,unsigned long len,void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
       ...
       rtk_vlan_wan_vid = RTL_WANVLANID;
        ...
        if(vlan_ctl_p->flag  == VLAN_CTL_ON )
        {
...
static int32 vlan_wan_vid_read( char *page, char **start, off_t off, int count, int *eof, void *data )
{
   int len=0;	
	...
	len = sprintf(page, "rtk_wan_vid (NAT) = %d\n", rtk_vlan_wan_vid);	
   return len;
}
