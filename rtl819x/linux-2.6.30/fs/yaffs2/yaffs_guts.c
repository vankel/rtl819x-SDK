...
static int yaffs_init_blocks(struct yaffs_dev *dev)
{
	...
	if (!dev->block_info) {
		/*winfred_wang*/
		/*dev->block_info =
		    kmalloc(n_blocks * sizeof(struct yaffs_block_info),GFP_KERNEL);*/
		dev->block_info =
	...
	if (!dev->chunk_bits) {
		/*winfred_wang*/
		/*dev->chunk_bits =
		    kmalloc(dev->chunk_bit_stride * n_blocks,GFP_KERNEL);*/
		dev->chunk_bits =
...
printk("%s: data_bytes_per_chunk %d tags2:%d\n",__FUNCTION__, dev->data_bytes_per_chunk, sizeof(struct yaffs_packed_tags2_tags_only));
