...
#define _ALC5633Q_CLI_PROTECT

/*
@func int | __i2c_rawWrite_alc5633Q | Write several bits to device
...
@comm
*/
static void __i2c_rawWrite_alc5633Q( i2c_dev_t* pI2C_Dev, const unsigned char *pDEV_ID, unsigned char *pReg, unsigned short int *pData)
{
	int i;
	//char j;
	//unsigned int buf;
...
/*
@func int | __i2c_rawRead | Read several bits from device
@parm i2c_dev_t* | pI2C_Dev | Structure containing device information
@parm unsigned char* | pDEV_ID | i2c id address
@parm unsigned char* | pReg | i2c register address
@parm unsigned short int* | pData | i2c data
@comm
*/
static void __i2c_rawRead_alc5633Q( i2c_dev_t* pI2C_Dev, const unsigned char *pDEV_ID, unsigned char *pReg, unsigned short int *pData)
{
	int i;
...
unsigned short int read_ALC5633Q(unsigned char Reg)
{
	unsigned short int buf;
	const unsigned char dev_id = 0x1C;
#ifdef _ALC5633Q_CLI_PROTECT	
	unsigned long flags;
	local_irq_save(flags);
#endif
	__i2c_rawRead_alc5633Q(&i2c_dev,&dev_id,&Reg,&buf);
#ifdef _ALC5633Q_CLI_PROTECT	
...
void write_ALC5633Q(unsigned char Reg,unsigned short int data)
{
	const unsigned char dev_id = 0x1C;
#ifdef _ALC5633Q_CLI_PROTECT	
	unsigned long flags;
	local_irq_save(flags);
#endif
	__i2c_rawWrite_alc5633Q(&i2c_dev,&dev_id,&Reg,&data);
#ifdef _ALC5633Q_CLI_PROTECT	
	local_irq_restore(flags);
#endif	
	return;
...
unsigned short int read_ALC5633Q_hidden(unsigned char index)
{

	write_ALC5633Q(0x6a, index);

	return read_ALC5633Q(0x6c);
}

void write_ALC5633Q_hidden(unsigned char index, unsigned short int data)
{
	write_ALC5633Q(0x6a, index);

	write_ALC5633Q(0x6c, data);
}
