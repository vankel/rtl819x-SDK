...
static int __init voip_con_init_polling_timer( void )
{
	...
	extern void Init_RFC2833_TX_Timer( void );
	Init_RFC2833_TX_Timer();
#endif
...
static int voip_con_main_proc_read( struct seq_file *m, void *v )
{
...
#define S_PTR( ptr, fld )	( ptr ? ptr ->fld : NULL )
	...
	seq_printf( m, "cch E B dsp sch T name    bch name    bch2 name    dch name     \n" );
	seq_printf( m, "--- - - --- ------------- ----------- ------------ ------------\n" );
	...
	for( i = 0; i < CON_CH_NUM; i ++ ) {
		...
		seq_printf( m, "%3d %d %X %u%c%u %3d %d %-7s %3d %-7s %4d %-7s %3d %-7s\n",
				S_VAL( p_con, cch ),
				S_VAL( p_con, enabled ),
	...
	seq_printf( m, "cch LED0         LED1         Relay \n" );
	seq_printf( m, "--- ----=------- ----=------- ----=------- \n" );
	...
	for( i = 0; i < CON_CH_NUM; i ++ ) {
		...
		seq_printf( m, "%3d " "%4X %-7s " "%4X %-7s " "%4X %-7s " "\n",
				S_VAL( p_con, cch ),
...
#ifdef CONFIG_LINUX_KERNEL_3_10
static int voip_global_num_proc_read( struct seq_file *m, void *v )
{
	if( m->index ) {	/* In our case, we write out all data at once. */
		return 0;
	}
	
	seq_printf( m, "con_ch_num = %d\n", con_ch_num );
	seq_printf( m, "dsp_ch_num = %d\n", dsp_ch_num );
	seq_printf( m, "dsp_ss_num = %d\n", dsp_ss_num );
...
#ifdef CONFIG_LINUX_KERNEL_3_10
static int voip_con_main_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, voip_con_main_proc_read, NULL);
}

static const struct file_operations voip_con_main_fops = 
{
	.open = voip_con_main_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
};

static int voip_global_num_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, voip_global_num_proc_read, NULL);
}

static const struct file_operations voip_global_num_fops = 
{
	.open = voip_global_num_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
};
#endif
...
{
#ifdef CONFIG_LINUX_KERNEL_3_10
	proc_create_data( PROC_VOIP_DIR "/" PROC_VOIP_CH_MAPS_DIR "/bind", 0, NULL, 
			&voip_con_main_fops, NULL );
	
	proc_create_data( PROC_VOIP_DIR "/global_num", 0, NULL, &voip_global_num_fops, NULL );
#else
