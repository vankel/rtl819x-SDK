...
static int32 PCM_RX(uint32 chid, uint32 *rxBuf, const uint16 *lec_ref)
{
    static int idx[MAX_DSP_RTK_SS_NUM] = {[0 ... MAX_DSP_RTK_SS_NUM-1] = -1};
	...
	struct Tdtmf_det_2833 {
		int dtmf_det_duration;
		int dtmf_det_end;
	};

	static struct Tdtmf_det_2833 dtmf_2833[MAX_DSP_RTK_CH_NUM] = {[0 ... MAX_DSP_RTK_CH_NUM-1] = {0, 0}};
	static int rfc2833_cnt[MAX_DSP_RTK_SS_NUM] = {[0 ... MAX_DSP_RTK_SS_NUM-1] = 0};
	static char dtmf_in_digit[MAX_DSP_RTK_CH_NUM] = {[0 ... MAX_DSP_RTK_CH_NUM-1] = 'Z'}; 
	static int dtmf_on_index[MAX_DSP_RTK_CH_NUM] = {[0 ... MAX_DSP_RTK_CH_NUM-1] = -1}; 
	...
	if (dtmf_chid[chid][0] == 1)
	{
		...
		dtmf_in_digit[chid] = 'Z';

		if (dtmf_digit.digitOnOff!=DSIL) {
			dtmf_on_cnt_10ms[chid][0]++;
			if (dtmf_on_cnt_10ms[chid][0]==dtmf_det_on_time_get(chid, 0))
			{
				dtmf_in_digit[chid] = dtmf_conv_table[dtmf_digit.digitOnOff];
				voip_event_dtmf_in( chid, dtmf_in_digit[chid] 0, 0, 0 );
				dtmf_on_flag[chid][0]=1;
				dtmf_2833[chid].dtmf_det_end = 0;

				//PRINT_G("IN\n"); 
				
#if 1
				if (dtmf_on_index[chid] == -1)
				{
					dtmf_on_index[chid] = dtmf_det_get_index(chid, 0);
					//PRINT_G("idx=%d\n", dtmf_on_index[chid]);
				}
				else
					PRINT_R("error, dtmf_on_index[%d] != -1\n", chid);
#endif
			}
		//} else if (dtmf_pre_stat[chid][0]!=DSIL) {
			...
			//if (( 1 == dtmf_on_flag[chid][0] ) && ( 1 == dtmf_det_duration_get(chid, 0, &dtmf_2833[chid].dtmf_det_duration, dtmf_det_get_index(chid, 0))))
			if (( 1 == dtmf_on_flag[chid][0] ) && (dtmf_on_index[chid] != -1) && ( 1 == dtmf_det_duration_get(chid, 0, &dtmf_2833[chid].dtmf_det_duration,  dtmf_on_index[chid])))
			{
				//voip_event_dtmf_in(chid, 'E', 0, get_dtmf_dBFS(chid, 0, 1, dtmf_det_get_index(chid, 0)), dtmf_2833[chid].dtmf_det_duration);
				voip_event_dtmf_in(chid, 'E', 0, get_dtmf_dBFS(chid, 0, 1, dtmf_on_index[chid]), dtmf_2833[chid].dtmf_det_duration);
				dtmf_2833[chid].dtmf_det_end = 1;
			    dtmf_on_flag[chid][0]=0;
				dtmf_on_index[chid] = -1;
...
SKIP_AES:
	...
	/**********************************************
		...
		if (dtmf_chid[chid][0] == 1)
		{
			...
			static char lec_ref_digit_pre[MAX_DSP_RTK_CH_NUM] = {[ 0 ... MAX_DSP_RTK_CH_NUM-1] = 16};
			static char lec_ref_digit2_pre[MAX_DSP_RTK_CH_NUM] = {[ 0 ... MAX_DSP_RTK_CH_NUM-1] = 'Z'};
		...
		if (dtmf_chid[chid][LEC_REF_DIR] == 1)
		{
			if (dtmf_digit.digit != 'Z') 
			{
				...
				if ((lec_ref_digit2_pre[chid] == dtmf_digit.digit) || (lec_ref_digit2[chid] == dtmf_digit.digit))
				{
			...
			lec_ref_digit2_pre[chid] = lec_ref_digit2[chid];
		}
#endif
		...
		if (dtmf_digit.digitOnOff!=DSIL){
			...
			if (dtmf_on_cnt_10ms[chid][0]==dtmf_det_on_time_get(chid, 0))
			{
				...
				if ((lec_ref_digit_pre[chid] != dtmf_digit.digitOnOff) || (lec_ref_digit[chid] != dtmf_digit.digitOnOff))
				{
					dtmf_in_digit[chid] = dtmf_conv_table[dtmf_digit.digitOnOff];
					voip_event_dtmf_in( chid, dtmf_in_digit[chid], 0, 0, 0 );
				}
...
SKIP_DTMF_END:
		...
		dtmf_pre_stat[chid][0] = dtmf_digit.digitOnOff;
#ifdef DTMF_ECHO_SUPPRESS
		lec_ref_digit_pre[chid] = lec_ref_digit[chid];
#endif
#else
		voip_event_dtmf_in( chid, dtmf_digit.digit, 0, 0, 0 );
#endif		
					...
					if (send_2833_by_ap[chid] == 0)
			  		{
			  			...
			  			extern int Rfc2833EventBufWriteUpdate(uint32 sid, int idx, int event, int cnt, int end);
			  			extern int Rfc2833EventBufWrite(uint32 sid, int event, int cnt);
			  			extern short dtmf_on_flag[][2];
			  			
			  			if ( dtmf_on_flag[chid][0] == 1 )
			  			{
				  			digit_tmp = dtmf_in_digit[chid];
	
							if (digit_tmp >= 48 && digit_tmp <= 57 )	/* 0 to 9  */
							{
								digit_tmp = g_digit[pSessRank[j]] = digit_tmp-48;
								//printk("->%d\n",g_digit[pSessRank[j]]);
							}
							else if (digit_tmp >= 65 && digit_tmp <= 68 )	/* A to D  */
							{
								digit_tmp = g_digit[pSessRank[j]] = digit_tmp-65+12;
								//printk("->%d\n",g_digit[pSessRank[j]]);
							}
							else if (digit_tmp == 42 )	/* (*) */
							{
								digit_tmp = g_digit[pSessRank[j]] = digit_tmp-42+10;
								//printk("->%d\n",g_digit[pSessRank[j]]);
							}
							else if (digit_tmp == 35 )	/* (#) */
							{
								digit_tmp = g_digit[pSessRank[j]] = digit_tmp-35+11;
								//printk("->%d\n",g_digit[pSessRank[j]]);
							}
							else if (digit_tmp == 'Z')
							{
							...
							if (digit_tmp != 'Z')
							{
								idx[pSessRank[j]] = Rfc2833EventBufWrite(pSessRank[j], g_digit[pSessRank[j]], dtmf_det_on_time_get(chid, 0));
								rfc2833_cnt[pSessRank[j]] = dtmf_det_on_time_get(chid, 0);
                                //PRINT_Y("W, idx%d, sid%d\n", idx[pSessRank[j]], pSessRank[j]);
							}
							else
							{
								Rfc2833EventBufWriteUpdate(pSessRank[j], idx[pSessRank[j]], g_digit[pSessRank[j]], 1, 0);
								rfc2833_cnt[pSessRank[j]]++;
                                //PRINT_Y("Wup, idx%d, sid%d\n", idx[pSessRank[j]], pSessRank[j]);
							}
							
						}
						
						if ((dtmf_2833[chid].dtmf_det_end == 1) && (g_digit[pSessRank[j]]!= -1))
						{
							int d;
							int duration;
							
							if ((dtmf_2833[chid].dtmf_det_duration - ((dtmf_2833[chid].dtmf_det_duration/10)*10)) >= 5)
								duration = (dtmf_2833[chid].dtmf_det_duration/10) + 1;
							else
								duration = dtmf_2833[chid].dtmf_det_duration/10;

							//PRINT_R("%d-%d-%d\n", duration, rfc2833_cnt[pSessRank[j]], pSessRank[j]);
							//printk("g_digit[%d]=%d\n", pSessRank[j], g_digit[pSessRank[j]]);
							
							if ( (d = (duration - rfc2833_cnt[pSessRank[j]])) > 0 )
								Rfc2833EventBufWriteUpdate(pSessRank[j], idx[pSessRank[j]], g_digit[pSessRank[j]], d, 1);
							else
								Rfc2833EventBufWriteUpdate(pSessRank[j], idx[pSessRank[j]], g_digit[pSessRank[j]], 0, 1);
							
						
							rfc2833_cnt[pSessRank[j]] = 0;
			  		...
			  		if (send_2833_by_ap[chid] == 0)
			  		{
						...
						digit_tmp = dtmf_in_digit[chid];
#else
						digit_tmp = dtmf_digit.digit;
#endif
						if (digit_tmp >= 48 && digit_tmp <= 57 && send_dtmf_flag[pSessRank[j]] == 0)	/* 0 to 9  */
						{
							send_dtmf_flag[pSessRank[j]] = 1;
							g_digit[pSessRank[j]] = digit_tmp-48;
							//PRINT_G("->%d\n",g_digit[pSessRank[j]]);
						}
						else if (digit_tmp >= 65 && digit_tmp <= 68 && send_dtmf_flag[pSessRank[j]] == 0)	/* A to D  */
						{
							send_dtmf_flag[pSessRank[j]] = 1;
							g_digit[pSessRank[j]] = digit_tmp-65+12;
							//PRINT_G("->%d\n",g_digit[pSessRank[j]]);
						}
						else if (digit_tmp == 42 && send_dtmf_flag[pSessRank[j]] == 0)	/* (*) */
						{
							send_dtmf_flag[pSessRank[j]] = 1;
							g_digit[pSessRank[j]] = digit_tmp-42+10;
							//PRINT_G("->%d\n",g_digit[pSessRank[j]]);
						}
						else if (digit_tmp == 35 && send_dtmf_flag[pSessRank[j]] == 0)	/* (#) */
						{
							send_dtmf_flag[pSessRank[j]] = 1;
							g_digit[pSessRank[j]] = digit_tmp-35+11;
							//PRINT_G("->%d\n",g_digit[pSessRank[j]]);
						}
						else if (digit_tmp == 'Z' && send_dtmf_flag[pSessRank[j]] == 0)
						{
							g_digit[pSessRank[j]] = -1;
							//PRINT_R("->%d\n",g_digit[pSessRank[j]]);
						}
...
label_send_2833_now:
						...
						{
#ifdef CUSTOMIZE_DTMF_MINIMUM_ON_TIME
							rfc2833_count_add(pSessRank[j], dtmf_det_on_time_get(chid, 0));
							rfc2833_cnt[pSessRank[j]] = dtmf_det_on_time_get(chid, 0);
#else
							rfc2833_count_add(pSessRank[j], 2); // add 20ms 
							rfc2833_cnt[pSessRank[j]] =2;
#endif
							//PRINT_R("1:%d-%d\n", rfc2833_cnt[pSessRank[j]], pSessRank[j]);
						}
						...
						else if ((dtmf_removal[chid][0] == 0) && (dtmf_2833[chid].dtmf_det_end == 0))
						{
							if (rfc2833_cnt[pSessRank[j]] != 0)
							{
								rfc2833_count_add(pSessRank[j], 1); // add 10ms
								rfc2833_cnt[pSessRank[j]] ++;
								//PRINT_R("3:%d-%d\n", rfc2833_cnt[pSessRank[j]], pSessRank[j]);
						...
						else if ((dtmf_removal[chid][0] == 0) && (dtmf_2833[chid].dtmf_det_end == 1))
						{
							//PRINT_R("%d-", dtmf_2833[chid].dtmf_det_duration);
							if ((dtmf_2833[chid].dtmf_det_duration - ((dtmf_2833[chid].dtmf_det_duration/10)*10)) >= 5)
								dtmf_2833[chid].dtmf_det_duration = (dtmf_2833[chid].dtmf_det_duration/10) + 1;
							else
								dtmf_2833[chid].dtmf_det_duration = dtmf_2833[chid].dtmf_det_duration/10;
							//PRINT_R("%d-%d-%d\n", dtmf_2833[chid].dtmf_det_duration, rfc2833_cnt[pSessRank[j]], chid);
							if ( (dtmf_2833[chid].dtmf_det_duration - rfc2833_cnt[pSessRank[j]]) > 0 )
								rfc2833_count_add(pSessRank[j], dtmf_2833[chid].dtmf_det_duration - rfc2833_cnt[pSessRank[j]]);
							rfc2833_cnt[pSessRank[j]] = 0;
							dtmf_2833[chid].dtmf_det_end = 0;
						}
