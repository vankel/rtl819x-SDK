    ...
    Vp890DeviceObjectType   *pDevObj)
{
     ...
     */
    for (channelCount = 0; channelCount < VP890_MAX_NUM_CHANNELS; channelCount++) {
    ...
    pDevCtx->funPtrsToApiFuncs.ObjectDump       = Vp890ObjectDump;
#endif
    ...
    VpDevCtxType            *pDevCtx)
{
    ...
    if (channelId >= VP890_MAX_NUM_CHANNELS) {
    ...
    Vp890DeviceObjectType *pDevObj)
{
    ...
    /* This is adjusted when we know the silicon we're working with. */
...
InitLineObject(
    Vp890LineObjectType *pLineObj)
{
    VpMemSet(pLineObj, 0, sizeof(Vp890LineObjectType));
