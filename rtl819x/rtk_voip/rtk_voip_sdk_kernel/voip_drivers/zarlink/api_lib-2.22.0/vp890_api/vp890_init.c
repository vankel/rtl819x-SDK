 ...
 *
 * $Revision: 11118 $
 * $LastChangedDate: 2013-08-08 16:47:35 -0500 (Thu, 08 Aug 2013) $
 */
    ...
    VpProfilePtrType        pRingProfile)
{
    ...
    if (pLineObj->status & VP890_IS_FXO) {
        ...
        if (pDcFxoCfgProf != VP_PTABLE_NULL) {
            ...
            /* This cache is needed for VP_OPTION_ID_DCFEED VpGetOption() */
            VpMemCpy(pLineObj->calLineData.loopSup, loopSupervision, VP890_LOOP_SUP_LEN);
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    */
    ...
    if (readBuffer[1] != 0x65) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp890QuickMpiTest: Test 1 failed. Read value incorrect (0x%02X)",
            readBuffer[1]));
        return VP_STATUS_ERR_SPI;
    }
