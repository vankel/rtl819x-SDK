 ...
 * the Vp886 device API. This includes the Vp8860 and Vp8870 series.
 *
 * Copyright (c) 2011, Microsemi Corporation
 *
 * $Revision: 11140 $
 * $LastChangedDate: 2013-08-23 10:36:20 -0500 (Fri, 23 Aug 2013) $
 */
    ...
    VpProfilePtrType pFxoCfgProfile)
{
    ...
    switch(pDevProfile[VP886_DEV_PROFILE_DEV_CFG_DATA0] & VP886_R_DEVCFG_CLKSEL) {
        ...
        default:
            VP_ERROR(VpDevCtxType, pDevCtx, ("Invalid PCM Clock Rate selection in device profile, 0x%02X",
                pDevProfile[VP886_DEV_PROFILE_DEV_CFG_DATA0]));
            pDevObj->busyFlags &= ~VP_DEV_INIT_IN_PROGRESS;
            Vp886ExitCritical(pDevCtx, VP_NULL, "Vp886InitDevice");
            return VP_STATUS_ERR_PROFILE;
    }
    ...
    /* The current understanding is that:
       Device profile version 0 doesn't support the adaptive ringing power management algorithms
       Device profile version 1 supports the adaptive ringing power management algorithms, but
       that field is always 0xFF, which indicates the power management algorithms are disabled.
       Device profile version 2 supports the adaptive ringing power management algorithms, and
       that field of the profile should actually contain a meaningful target SLIC power, if
       the algorithms are going to be enabled */
    /* If the device profile version is less than 2 ... */
    if (pDevProfile[VP_PROFILE_VERSION] < 2) {
        /* Thermal ringing not supported in the profile */
        pDevObj->devProfileData.adaptiveRingingMaxPower = VP_ADAPTIVE_RINGING_DISABLED;
    } else {
        /* If this is a version 2 or later profile */
        /* Grab the maximum target SLIC power for the adaptive ringing power management algorithm */
        pDevObj->devProfileData.adaptiveRingingMaxPower = *pProf;
    }
#endif
    ...
    /* Unmask all device interrupts except for those we know we will not use.
       ...
       - Device cadencer interrupts are generated while we perform DTMF caller
         ID, but they are of no use to us. */
    {
        uint8 intMask[VP886_R_INTMASK_LEN];
        intMask[0] = VP886_R_INTMASK_IO2 | VP886_R_INTMASK_CAD;
        intMask[1] = VP886_R_INTMASK_IO2 | VP886_R_INTMASK_CAD;
        intMask[2] = 0x00;
        intMask[3] = 0x00;
        VpSlacRegWrite(pDevCtx, VP_NULL, VP886_R_INTMASK_WRT, VP886_R_INTMASK_LEN, intMask);
    }
    ...
    while (runAnotherState) {
        ...
        switch (pDevObj->initDeviceState) {
            ...
            case VP886_INIT_DEVICE_CFAIL_CHECK: {
                /* TODO: Do we need to put this in the Device Object ? */
                uint8 sigReg[4];
                uint8 clkNotStable;

                /* Read and unlock the signalling register  */
                ...
                /* Check for CFAIL */
                clkNotStable = sigReg[0] & VP886_R_SIGREG_CFAIL;
                ...
                /* If clock fault persisted for too long ... finish up */
                if (clkNotStable) {
            ...
            case VP886_INIT_DEVICE_ENABLE_VREF: {
                ...
                /* Enable Vref in ICR3 */
                icr3Data[0] = 0x01; icr3Data[1] = 0x01;
            ...
            case VP886_INIT_DEVICE_VREF_CHECK: {
                ...
                timerDuration = VP886_VREF_WAIT_TIME;
                if (pDevObj->devProfileData.cpEnable) {
            ...
            case VP886_INIT_DEVICE_CP_CHECK: {
                ...
                if ((sigReg[2] & VP886_R_SIGREG_CPUVLO) == VP886_R_SIGREG_CPUVLO) {
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    /* Look for the bit that indicates a shared-supply device. If it is set, we
       need to remember for later use such as checking the device profile and
       modified line test behaviors.
       Also, if it is a shared-supply device and the device type passed in
       to VpMakeDeviceObject does not match the PCN code, change the PCN to match
       the device type.  Then during InitDevice we will reprogram the part to
       behave as the requested type.  In most cases this will mean changing an
       ABS device to tracker, but there were a few demo parts made that need to
       be able to be changed from tracker to ABS. */
    ...
    if (fuse8[0] & VP886_R_FUSE_REG_8_SHSUP) {
        if (VP886_DEVICE_SERIES(pDevObj) != pDevCtx->deviceType) {
            if (pDevCtx->deviceType == VP_DEV_886_SERIES) {
                /* Clearing this bit changes the tracker PCN to ABS */
        ...
        if (VP886_IS_TRACKER(pDevObj)) {
            /* Only set the shared supply flag if the device is configured as
               tracker.  If the device is being treated as ABS, then it is not
               really using a shared supply. */
    ...
    uint16* eventData)
{
    ...
    if ((sigReg[0] & VP886_R_SIGREG_OCALM) == VP886_R_SIGREG_OCALM) {

        /* Send a Device Init Complete event with the CH1_OC flag.
           Go to the VP886_INIT_DEVICE_GEN_EVENT state. */
        VP_ERROR(VpDevCtxType, pDevCtx, ("Device Failed to Reset Channel 1 Over Current"));
    ...
    if ((sigReg[2] & VP886_R_SIGREG_OVALM) == VP886_R_SIGREG_OVALM) {

        /* Send a Device Init Complete event with the CH1_OV flag.
           Go to the VP886_INIT_DEVICE_GEN_EVENT state. */
        VP_ERROR(VpDevCtxType, pDevCtx, ("Device Failed to Reset Channel 1 Over Voltage"));
    ...
    if ((sigReg[1] & VP886_R_SIGREG_OCALM) == VP886_R_SIGREG_OCALM) {

        /* Send a Device Init Complete event with the CH2_OC flag.
           Go to the VP886_INIT_DEVICE_GEN_EVENT state. */
        VP_ERROR(VpDevCtxType, pDevCtx, ("Device Failed to Reset Channel 2 Over Current"));
    ...
    if ((sigReg[3] & VP886_R_SIGREG_OVALM) == VP886_R_SIGREG_OVALM) {

        /* Send a Device Init Complete event with the CH2_OV flag.
           Go to the VP886_INIT_DEVICE_GEN_EVENT state. */
        VP_ERROR(VpDevCtxType, pDevCtx, ("Device Failed to Reset Channel 2 Over Voltage"));
    ...
    VpProfilePtrType pRingProfile)
{
    ...
    if (!Vp886IsReady(pDevCtx, VP_NULL)) {
    ...
    bool initDevice)
{
    ...
    if (initDevice && status == VP_STATUS_SUCCESS) {
        ...
        /* Get the current interrupt mask reg contents */
        VpSlacRegRead(pDevCtx, NULL, VP886_R_INTMASK_RD, VP886_R_INTMASK_LEN, intMask);
        ...
        if (io2Use == VP886_DEV_PROFILE_IO2_USE_DIG_SIG) {
            ...
            intMask[channelId] |= VP886_R_INTMASK_IO2;
        ...
        VpSlacRegWrite(pDevCtx, NULL, VP886_R_INTMASK_WRT, VP886_R_INTMASK_LEN, intMask);
    ...
    bool initDevice)
{
    ...
    if (VP886_IS_TRACKER(pDevObj)) {
        /* Set switcher params based on the device profile */
    ...
    pLineObj->registers.icr3[0] |= VP886_R_ICR4_VREF_EN;
    pLineObj->registers.icr3[1] |= VP886_R_ICR4_VREF_EN;
    ...
    VpProfilePtrType pRingProfile)
{
    ...
    if (!Vp886IsReady(VP_NULL, pLineCtx)) {
    ...
    bool initDevice)
{
    ...
    if (pRingProfile != VP_PTABLE_NULL) {
        ...
        /* Add Ringing parameters to SLAC write buffer */
        {
            ...
            /* Save the ringing bias and amplitude */
            if (cmd == VP886_R_RINGGEN_WRT) {
                pLineObj->ringBias = ((pData[1] << 8) | pData[2]);
                pLineObj->ringAmplitude = ((pData[5] << 8) | pData[6]);
            } else {
                ...
                pLineObj->ringBias = ((ringGen[1] << 8) | ringGen[2]);
                pLineObj->ringAmplitude = ((ringGen[5] << 8) | ringGen[6]);
            }
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    */
    ...
    if (readBuffer[1] != 0x65) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp886QuickMpiTest: Test 1 failed. Read value incorrect (0x%02X)",
            readBuffer[1]));
        return VP_STATUS_ERR_SPI;
    }
