    ...
    uint16 handle)
{
    ...
    if (!Vp886IsReady(pDevCtx, pLineCtx)) {
    ...
    void *pValue)
{
    ...
    if (option != VP_OPTION_ID_DEBUG_SELECT && !Vp886IsReady(pDevCtx, VP_NULL)) {
        VP_API_FUNC_INT(VpDevCtxType, pDevCtx, ("Vp886SetOptionDevice-"));
        return VP_STATUS_DEVICE_BUSY;
    }
    ...
    switch (option) {
        ...
        case VP_DEVICE_OPTION_ID_ADAPTIVE_RINGING: {
            ...
            if (pUserBuf->validMask & VP_ADAPTIVE_RINGING_CFG_MODE) {
                ...
                pCache->adaptiveRinging.mode = pUserBuf->mode;
            }
            break;
        }
#endif
    ...
    void *pValue)
{
    ...
    if (option != VP_OPTION_ID_DEBUG_SELECT && !Vp886IsReady(VP_NULL, pLineCtx)) {
        VP_API_FUNC_INT(VpLineCtxType, pLineCtx, ("Vp886SetOptionLine-"));
        return VP_STATUS_DEVICE_BUSY;
    }
    ...
    switch (option) {
        ...
        case VP_OPTION_ID_SWITCHER_CTRL: {
            ...
            /* Set or clear the appropriate bits as neceesary */
            if (shutDownEn == TRUE) {
    ...
    VpOptionTimeslotType *pTimeslot)
{
    ...
    if (pDevObj->stateInt & VP886_ZSI_DETECTED) {
        if (txSlot == 0) {
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886SetOptionTimeslot() - Timeslot 0 invalid in ZSI mode"));
            return VP_STATUS_INVALID_ARG;
        }
        txSlot--;
    }
    ...
    VpDeviceIoAccessDataType *pDeviceIoData)
{
    ...
    for (chanNum = 0; chanNum < VP886_MAX_NUM_CHANNELS; chanNum++) {
        uint16 dataMask;
        uint8 loopCnt;
        uint16 tempData;
...
#ifdef VP_CSLAC_SEQ_EN
    if (pLineObj->cid.active) {
        /* During CID, use this information to validate detect intervals.
