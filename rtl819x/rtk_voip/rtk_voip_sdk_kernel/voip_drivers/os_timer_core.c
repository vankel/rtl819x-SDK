...
static int voip_timer_proc_read( struct seq_file *m, void *data )
{
	int n = 0;
	int i;
	const timer_entry_t *p_entry;
	...
	seq_printf(m, "timetick=%lu\n", timetick );
	seq_printf(m, "         next\tperiod\tfn\t\tpriv\n" );
	
	for( i = 0; i < TIMER_ENTRIES_NUM; i ++ ) {
		
		p_entry = &timer.entries[ i ];
		
		seq_printf(m, "[%2d] %8lu\t%u%s\t%8p\t%p %s\n", i,
...
{
	return single_open(file, voip_timer_proc_read, NULL);
}

static const struct file_operations voip_timer_fops = 
{
...
{
#ifdef CONFIG_LINUX_KERNEL_3_10
	create_voip_proc_read_entry( PROC_VOIP_DIR "/timer", 0, NULL, &voip_timer_fops, NULL );
#else
