...
**
** $Id: si3217x_intf.c 4461 2014-07-16 15:39:20Z nizajerk $
**
** SI3217X_Intf.c
...
int Si3217x_Calibrate(proslicChanType_ptr *pProslic, int maxChan, uInt8 *calr, int maxTime)
{
    ...
    */
    do 
    {
        ...
        for(i=0;i<maxChan;i++)
        {
            if(pProslic[i]->channelType != PROSLIC) continue; /* Skip if DAA */
...
int Si3217x_Init_with_Options (proslicChanType_ptr *pProslic, int size, initOptionsType init_opt){
    ...
    */
	...
	if(init_opt != INIT_REINIT)
	{
        ...
        ** VDC Range Check -
        **
        ** Prevent use of deprecated VDC ranges
        */
        ...
        for (k=0;k<size;k++)
        {
  		    if ((pProslic[k]->channelEnable)&&(pProslic[k]->channelType == PROSLIC))
		    {
	            if(Si3217x_General_Configuration.bom_option == BO_DCDC_PMOS_BUCK_BOOST)
	            {
		            if(Si3217x_General_Configuration.vdc_range == VDC_3P0_6P0)
                    {
					    pProslic[k]->channelEnable = 0;
					    pProslic[k]->error = RC_UNSUPPORTED_VDC_RANGE;
                        return RC_UNSUPPORTED_VDC_RANGE;
                    }
                }
            }
        }
...
#else
    ReadReg(pProHW,pProslic->channel,SI3217X_COM_REG_IRQ1);
    ReadReg(pProHW,pProslic->channel,SI3217X_COM_REG_IRQ2);
    ReadReg(pProHW,pProslic->channel,SI3217X_COM_REG_IRQ3);
    ReadReg(pProHW,pProslic->channel,SI3217X_COM_REG_IRQ4);
#endif
    return RC_NONE;
}
...
int Si3217x_PLLFreeRunStart (proslicChanType *pProslic){
    ...
    WriteReg(pProHW,pProslic->channel,SI3217X_COM_REG_ZCAL_EN,0x00);
