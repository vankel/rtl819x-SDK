...
/**
 * @ingroup TAPI_DSP_EC
 * @brief Turn on or off EC (Echo Canceller)
 * @param chid The channel number.
 * @param ec_enable EC switch <br> 0: Turn off EC<br> 1: Turn on EC
 * @param ec_on_state EC is on when <br> 0: TDM active <br> 1: Media active <br>
 * Media acitve means that EC is on only when media (Vocoder, or VoIP session) is active.<br>
 * TDM acitve means that EC is on when TDM is active. It includes the scope of media active.<br>
 * In TDM acitve case, EC is on when TDM is enable, even if media(Vocoder, or VoIP session) is in-active.<br>
 * @retval 0 Success
 */
int32 rtk_SetG168Lec(uint32 chid, uint32 support_lec, uint32 ec_on_state);
 ...
 * @param chid The channel number. 
 * @param ec_select Selection of EC alogrithm <br>
 * 0: DSP default<br> 1: EC-32 (support 2 to 32ms echo tail length)<br> 2: EC-128 (support 32 to 128ms echo tail length)<br>
 * @param echo_tail The echo tail length that is the length of time over which it can cancel echoes. <br>
 * If it is set to 0 (NULL), DSP will not update the echo tail length.<br>
 * Value range: 2 to 32ms for EC-32, 32 to 128ms for EC-128 <br> Unit: ms <br>
...
/**
 * @ingroup TAPI_DSP_TONE
 * @brief Set the call-progress tone, busy tone detection parametes and SLIC/DAA impedance according to the selected country.
...
/**
 * @ingroup TAPI_PHONE_FXS
 * @brief Set the SLIC/DAA impedance according to the selected country.
...
/**
 * @ingroup TAPI_DSP_TONE
 * @brief Set the call-progress tone, and busy tone detection parametes according to the selected country.
