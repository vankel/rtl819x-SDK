...
int main(void)
{
	...
	for (chid=0; chid<CON_CH_NUM; chid++)
	{
		...
		rtk_SetEchoTail(chid, 1, 64, 1, 5, 5, 0);			// EC-128, 64ms, NLP on
...
int do_fax_event( int chid, VoipEventID evt )
{
	...
	if (CHANNEL_IS_FXS(chid))
	{
		...
		//rtk_GetFaxModemEvent(chid, NULL, &channel->fax, 0);
			
		if (((evt == VEID_FAXMDM_FAX_CED) || 
			(evt == VEID_FAXMDM_FAX_DIS_RX) ||
			(evt == VEID_FAXMDM_V21FLAG_LOCAL) ||
			(evt == VEID_FAXMDM_ANSTONE_ANS_LOCAL))
			&& (channel->fax_clear ==1))
		{
		...
		if ((evt == VEID_FAXMDM_FAX_DCN_RX) && (channel->fax_end_clear == 1))
		{
