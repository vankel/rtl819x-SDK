...
int main( int argc, char **argv )
{
...
#else
	if( ( fp = fopen( "/usr/722_raw", "rb" ) ) == NULL ) {
		printf( "Open error\n" );
		return 1;
	}
#endif
	...
	rtk_SetPlayTone(0, 0, DSPCODEC_TONE_BUSY, 0, DSPCODEC_TONEDIRECTION_LOCAL);
	rtk_SetPlayTone(1, 0, DSPCODEC_TONE_BUSY, 0, DSPCODEC_TONEDIRECTION_LOCAL);
	
	printf("stop busy tone\n");

	// PCM mode MUST be set to wideband	
	codec_config.chid = 0;
	codec_config.sid  = 0;
	codec_config.local_pt  = rtpPayloadG722;
	codec_config.remote_pt = rtpPayloadG722;
