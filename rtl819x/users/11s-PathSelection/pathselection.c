...
{	
    ...
    strcpy(interface_rawsocket, "wlan-msh");
    ...
    // destroy old process and create a PID file
    {
        ...
        sprintf(pid_file_name,"/var/run/pathsel-%s.pid", interface_ioctl);
	...
	//initail path_selection table
//	ioctl(g_ioctrl_socket_fd, INIT_PATHTABLE, 0);

#if defined(CONFIG_RTL_MESH_AUTOPORTAL_SUPPORT)
	reset_pann_from_driver();
#endif
...
#if defined(CONFIG_RTL_MESH_AUTOPORTAL_SUPPORT)
	//by brian, register pathsel pid to bridge for event deliver
	{
		int br_socket_fd = -1;
        unsigned long arg[3];

        arg[0] = BRCTL_SET_MESH_PATHSELPID;	//shoulde be BRCTL_SET_MESH_PATHSELPID
        arg[1] = getpid();
		arg[2] = 0;
