...
static int hostapd_ctrl_iface_wps_pin(struct hostapd_data *hapd, char *txt)
{
	...
	if (pin == NULL) {
		wpa_printf(MSG_ERROR, "%s:%d - Empty PIN", __func__, __LINE__);
		return -1;
	}
	*pin++ = '\0';
				       ...
				       void *sock_ctx)
{
	...
	if (res < 0) {
		wpa_printf(MSG_DEBUG, "%s:%d recvfrom(ctrl_iface) error", __func__, __LINE__);
		return;
	}
	buf[res] = '\0';
	...
	if (reply == NULL) {
		wpa_printf(MSG_ERROR, "%s:%d malloc size=%d fail", __func__, __LINE__, reply_size);
	...
	os_memcpy(reply, "OK\n", 3);
	reply_len = 3;
	wpa_printf(MSG_DEBUG, "%s:%d [ %s ]", __func__, __LINE__, buf);
...
int hostapd_ctrl_iface_init(struct hostapd_data *hapd)
{
	...
	if (hapd->conf->ctrl_interface == NULL) {
		wpa_printf(MSG_DEBUG, "%s:%d FAIL! ctrl_interface is NULL", __func__, __LINE__);
		return 0;
	}
	...
	hapd->ctrl_sock = s;
	wpa_printf(MSG_DEBUG, "%s:%d sock:%d ifname:%s", __func__, __LINE__, s, fname);
