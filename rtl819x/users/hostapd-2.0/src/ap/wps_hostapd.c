		     ...
		     struct hostapd_bss_config *conf)
{
	...
	if (conf->wps_state == 0) {
		wpa_printf(MSG_DEBUG, "%s:%d wps_state=0", __func__, __LINE__);
			...
			const char *uuid, const char *pin, int timeout)
{
	...
	wpa_printf(MSG_DEBUG,"%s:%d data.added:%d data.pin=%s", __func__, __LINE__, data.added, pin);
				    ...
				    int ssi_signal)
{
	...
	if (hapd->wps == NULL)
		return 0;
	wpa_printf(MSG_DEBUG, "%s:%d parse ProbeReq from "MACSTR, __func__, __LINE__, MAC2STR(addr));
	...
	if (wpabuf_len(wps_ie) > 0) {
		...
		wpa_printf(MSG_DEBUG, "%s:%d wps_ie->used=0\n", __func__, __LINE__);
