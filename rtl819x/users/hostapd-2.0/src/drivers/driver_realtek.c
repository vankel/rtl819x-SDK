...
extern int hostapd_set_drv_ieee8021x(struct hostapd_data *hapd, const char *ifname,
			      int enabled);
...
static int
set80211priv(struct realtek_driver_data *drv, int op, void *data, int len) 
{
	struct iwreq iwr;
	...
	if (first <= op &&
		idx < (int) (sizeof(opnames) / sizeof(opnames[0])) &&
		opnames[idx])
		wpa_printf(MSG_DEBUG, "%s: [0x%X] %s", __func__, op, opnames[idx]);
	else
		wpa_printf(MSG_DEBUG, "%s: [0x%X] ioctl[unknown???]", __func__, op);
...
static int
set80211param(struct realtek_driver_data *drv, int op, int arg)
{
	struct iwreq iwr;	
...
#if 1//def STAND_ALONE
	if (ioctl(drv->ioctl_sock, IEEE80211_IOCTL_SETPARAM, &iwr) < 0) {
		printf("ioctl[IEEE80211_IOCTL_SETPARAM]\n");
...
realtek_set_wpa(char *ifname,struct realtek_driver_data *drv, int wpa, int psk, int cipher)
{
	...
	if(psk & WPA_KEY_MGMT_PSK)
		{//PSK mode, set PSK & cipher
			if (set80211param(drv, IEEE80211_PARAM_KEYMGTALGS, wpa)) 
				{
					wpa_printf(MSG_ERROR, "Unable to set key management algorithms");
					return -1;
				}					
			if (set80211param(drv, IEEE80211_PARAM_UCASTCIPHERS, cipher)) 
				{
		...
		{//Enterprise mode, Disable PSK & set cipher.	
			if (set80211param(drv, IEEE80211_PARAM_KEYMGTALGS, 0)) 
				{
					wpa_printf(MSG_ERROR, "Unable to set key management algorithms");
					return -1;
				}	
	...
	if (set80211param(drv, IEEE80211_PARAM_WPA, wpa)) 
		{
			wpa_printf(MSG_ERROR, "Unable to set WPA");
			return -1;
		}

	return set80211param(drv, IEEE80211_PARAM_AUTHMODE, IEEE80211_AUTH_WPA);
 ...
 */
static int
realtek_configure_wpa(struct realtek_driver_data *drv,
...
{	
	...
	wpa_printf(MSG_DEBUG, "realtek_configure_wpa +++ wpa=0x%x, psk=0x%x, wpa_group=0x%x, wpa_pairwise=0x%x", 
			params->wpa, params->wpa_key_mgmt, params->wpa_group, params->wpa_pairwise);
	...
	wpa_printf(MSG_DEBUG, "%s: key management algorithms=0x%x",
		   __func__, params->wpa);	//BIT(0)=WPA, BIT(1)=WPA2
...
static int 
realtek_set_ieee8021x(void *priv, struct wpa_bss_params *params)
{
	struct realtek_driver_data *drv = priv;
	struct hostapd_data *hapd = drv->hapd;
	...
	wpa_printf(MSG_DEBUG, "realtek_set_ieee8021x+++ \n"
		   "enabled=%d, conf->auth_algs=%d, wep.keys_set=%d, conf->ieee802_1x=%d security_policy=%d"
			,params->enabled, conf->auth_algs, wep.keys_set, conf->ieee802_1x, conf->ssid.security_policy);

	if(set80211param(drv, IEEE80211_PARAM_UCASTKEYLEN, conf->individual_wep_key_len))
		return -1;
	...
	if (!params->enabled) {
		...
		if(conf->auth_algs==1 && wep.keys_set==0)
			return set80211param(drv, IEEE80211_PARAM_AUTHMODE, IEEE80211_AUTH_OPEN);
			...
			return set80211param(drv, IEEE80211_PARAM_AUTHMODE, IEEE80211_AUTH_SHARED);
		else if(conf->auth_algs==3)
			return set80211param(drv, IEEE80211_PARAM_AUTHMODE, IEEE80211_AUTH_AUTO);
		else if(conf->auth_algs==8) //RTK_HAPD, add auth_algs=BIT(3) as value of none authentication. 
			return set80211param(drv, IEEE80211_PARAM_AUTHMODE, IEEE80211_AUTH_NONE);
	}
	...
	if (params->wpa && realtek_configure_wpa(drv, params) != 0) {
...
static int
realtek_set_privacy(void *priv, int enabled)
{
	struct realtek_driver_data *drv = priv;
...
static int
realtek_sta_set_flags(void *priv, const u8 *addr, 
...
static int
realtek_del_key(void *priv, const u8 *addr, int key_idx)
{
	struct realtek_driver_data *drv = priv;
	//struct hostapd_data *hapd = drv->hapd;
...
static int
realtek_set_key(const char *ifname, void *priv, enum wpa_alg alg,
...

{
	...
	wpa_printf(MSG_DEBUG, "%s:%d start +++", __func__, __LINE__);
	wpa_printf(MSG_DEBUG, "alg = %d\n, addr = %s\n, key_index = %d", alg,  ether_sprintf(addr), key_idx);

	if (alg == WPA_ALG_NONE)
		return realtek_del_key(drv, addr, key_idx); 

	if ((alg == WPA_ALG_WEP) && ((wep.keys_set >= 1) && (wep.keys_set <= 4)))
	...
	wpa_printf(MSG_DEBUG, "%s:%d end ---", __func__, __LINE__);
	return ret;
}
...
static int
realtek_set_opt_ie(void *priv, const u8 *ie, size_t ie_len)
{
...
static int
realtek_sta_deauth(void *priv, const u8 *own_addr, const u8 *addr,
		   int reason_code)
{
	struct realtek_driver_data *drv = priv;
	struct hostapd_data *hapd = drv->hapd;
	...
	if (!realtek_is_ifup(conf->iface,drv)) {
		wpa_printf(MSG_DEBUG, "%s:%d fail!", __func__, __LINE__);
		return EINVAL;
	}
#ifdef INBAND_CTRL //HOST_LITTLE_ENDIAN 
	//im_reason need swap 	
...
#ifdef CONFIG_WPS
#ifdef IEEE80211_IOCTL_FILTERFRAME
static void realtek_raw_receive(void *ctx, const u8 *src_addr, const u8 *buf,
				size_t len)
{
...
static int
realtek_receive_probe_req(struct realtek_driver_data *drv)
{
	int ret = 0;
#ifdef CONFIG_WPS
#ifdef IEEE80211_IOCTL_FILTERFRAME
...
static int
realtek_sta_disassoc(void *priv, const u8 *own_addr, const u8 *addr,
		     int reason_code)
{
	struct realtek_driver_data *drv = priv;
	//struct hostapd_data *hapd = drv->hapd;
...
{	
	...
	if (set80211priv(drv, HAPD_IOCTL_GETWPAIE, &ie, sizeof(ie))) 
	{
	...
	if ((ie.wps_ie[0] == WLAN_EID_VENDOR_SPECIFIC) && ie.wps_ie[1]) 
	{
		//Test rtl8192su usb-dongle wps_pbc/wps_pin ok via here.
		 ...
		 //sta->wps_ie->ext_data = os_malloc(ielen);	//TODO:wpabuf_alloc_ext_data
		 //memcpy(sta->wps_ie->ext_data, ie.wps_ie, ielen);
		 ext_data = wpabuf_alloc_ext_data(sta->wps_ie, ielen);
		 memcpy(ext_data, ie.wps_ie, ielen);
...
realtek_new_sta(struct hostapd_data *hapd,struct realtek_driver_data *drv, u8 addr[IEEE80211_ADDR_LEN])
{
	...
	if (!sta->wpa_sm)
		wpa_printf(MSG_DEBUG, "%s:%d wpa_sm=NULL", __func__, __LINE__);
...
#else
static void
realtek_new_sta(struct realtek_driver_data *drv, u8 addr[IEEE80211_ADDR_LEN])
{
	struct hostapd_data *hapd = drv->hapd;
	struct ieee80211req_wpaie ie;
	int ielen = 0;
	...
	wpa_hexdump(MSG_MSGDUMP, "realtek req WPA IE",
	...
	if (iebuf[0] != WLAN_EID_VENDOR_SPECIFIC)
		iebuf[1] = 0;
#ifdef RTK_HAPD
	wpa_hexdump(MSG_MSGDUMP, "realtek req RSN IE",
...
{	
	...
	switch(flags)
	{
		...
		case HAPD_WPS_PROBEREQ:
		{
			...
			realtek_raw_receive(hapd, wps_ie->MACAddr, wps_ie->ProbeIE, wps_ie->ProbeIELen);
			break;
		}
#endif		
	}
...
static void
realtek_wireless_event_wireless(struct realtek_driver_data *drv,
					    char *data, int len)
{
	struct iw_event iwe_buf, *iwe = &iwe_buf;
	char *pos, *end, *custom, *buf;
	//wpa_printf(MSG_DEBUG, "%s:%d +++", __func__);
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		case IWEVEXPIRED:
			...
			break;
		case IWEVREGISTERED:
			realtek_new_sta(drv, (u8 *) iwe->u.addr.sa_data);
			//realtek_new_sta(drv->hapd, drv, (u8 *) iwe->u.addr.sa_data);
			break;
		case IWEVCUSTOM:
			...
			realtek_wireless_event_wireless_custom(drv->hapd,drv, iwe->u.data.flags, buf, iwe->u.data.length);
			free(buf);
			break;			
		}
...
static void
realtek_wireless_event_rtm_newlink(void *ctx, struct ifinfomsg *ifi,
				   u8 *buf, size_t len)
{
	struct realtek_driver_data *drv = ctx;
	int attrlen, rta_len;
	...
	while (RTA_OK(attr, attrlen)) {
		if (attr->rta_type == IFLA_WIRELESS) {
			realtek_wireless_event_wireless(
...
static int
realtek_wireless_event_init(struct realtek_driver_data *drv)
{
#if 0	
	int s;
	...
	if (cfg == NULL)
		return -1;
	cfg->ctx = drv;
	cfg->newlink_cb = realtek_wireless_event_rtm_newlink;
...
#ifdef RTK_MBSSID
static int realtek_bss_add(void *priv, enum wpa_driver_if_type type, const char *ifname, const u8 *addr, void *bss_ctx, void **drv_priv, char *force_ifname, u8 *if_addr, const char *bridge)
{	
	struct realtek_driver_data *drv = priv;
	...
	if(set80211priv(drv, HAPD_IOCTL_SETCONFIG, &config, sizeof(config))) 
	{
	...
	if((hapd->conf->wpa == 0) && (hapd->conf->ieee802_1x == 0 ))
		hostapd_set_drv_ieee8021x(hapd, ifname, 0);	
...
static int realtek_bss_remove(void *priv, enum wpa_driver_if_type type, const char *ifname)
{	
...
realtek_driver_init(struct hostapd_data *hapd)
{
	...
	realtek_set_privacy(drv, 0); /* default to no privacy */

	if(realtek_hapd_config(drv)) 
		goto bad;

	if((hapd->conf->wpa == 0) && (hapd->conf->ieee802_1x == 0 ))
		hostapd_set_drv_ieee8021x(hapd, drv->iface, 0);
...
static void *
realtek_init(struct hostapd_data *hapd, struct wpa_init_params *params)
{
	struct realtek_driver_data *drv;
	struct ifreq ifr;
	struct iwreq iwr;
	...
	drv = os_zalloc(sizeof(struct realtek_driver_data));
	if (drv == NULL) {
		wpa_printf(MSG_ERROR,"Could not allocate memory for realtek driver data\n");
		goto bad;
	}
	...
	if (realtek_hapd_config(drv)) {
		wpa_printf(MSG_ERROR, "realtek_hapd_config failed!");
		goto bad;
	}	

	realtek_receive_probe_req(drv);	
	if (realtek_wireless_event_init(drv))
		goto bad;
...
static int
realtek_set_ssid(void *priv, const u8 *buf, int len)
{
	struct realtek_driver_data *drv = priv;	
	struct iwreq iwr;
...
static int
realtek_get_ssid(void *priv, u8 *buf, int len)
{
	struct realtek_driver_data *drv = priv;	
	struct iwreq iwr;
	int ret = 0;
...
#ifdef CONFIG_WPS
static int
realtek_set_wps_ie(void *priv, const u8 *ie, size_t len, u32 frametype)
{
	struct realtek_driver_data *drv = priv;
	u8 buf[256];
...
static int
realtek_set_ap_wps_ie(void *priv, const struct wpabuf *beacon,
		      const struct wpabuf *proberesp,
		      const struct wpabuf *assocresp)
{
	if (realtek_set_wps_ie(priv, beacon ? wpabuf_head(beacon) : NULL,
			       beacon ? wpabuf_len(beacon) : 0,
			       IEEE80211_APPIE_FRAME_BEACON) < 0)
		return -1;
	return realtek_set_wps_ie(priv,
...
static int realtek_read_hapd_cfg(struct hostapd_data *hapd,void *priv, struct rtk_hapd_config* config)
{
	...
	if(iconf->supported_rates && realtek_config_rate(iconf->supported_rates, &(config->oprates)))
		return -1;
	
	if(iconf->basic_rates && realtek_config_rate(iconf->basic_rates, &(config->basicrates)))
		return -1;
...
static int realtek_hapd_config(void *priv)
{
...
#endif
	wpa_printf(MSG_DEBUG, "%s:%d realtek_hapd_config +++", __func__, __LINE__);
	...
	if(realtek_read_hapd_cfg(hapd,priv, config)) {
		wpa_printf(MSG_DEBUG, "realtek_read_hapd_cfg fail!");
		return -1;
	}
	if(realtek_read_priv_cfg(priv, config)) {
		wpa_printf(MSG_DEBUG, "realtek_read_priv_cfg fail!");
		return -1;
	}
#ifdef INBAND_CTRL //HOST_LITTLE_ENDIAN		
	//   rtk_hapd_config need to swap  , write a function to do it	
	...
	if(set80211priv(drv, HAPD_IOCTL_SETCONFIG, config, sizeof(struct rtk_hapd_config))) 
	{
		wpa_printf(MSG_ERROR, "%s:%d Failed to set Configurations", __func__, __LINE__);
		return -1;
	}
...
const struct wpa_driver_ops wpa_driver_realtek_ops = {
	.name					= "realtek",
	.desc					= "Realtek hostapd-2.0 support",
	.set_key					= realtek_set_key,
	.hapd_init				= realtek_init,
	.hapd_deinit				= realtek_deinit,
	...
	.hapd_send_eapol			= realtek_send_eapol,
	.sta_disassoc				= realtek_sta_disassoc,
	.sta_deauth				= realtek_sta_deauth,
#ifdef RTK_MBSSID	
	.if_add 					= realtek_bss_add,
	.if_remove 				= realtek_bss_remove,
#endif	
#ifdef RTK_HAPD
	.sta_remove				= realtek_sta_remove,
#endif
	.hapd_set_ssid			= realtek_set_ssid,
	.hapd_get_ssid			= realtek_get_ssid,
	.hapd_set_countermeasures	= realtek_set_countermeasures,
	.commit					= realtek_commit,
#ifdef CONFIG_WPS
	.set_ap_wps_ie			= realtek_set_ap_wps_ie,
#endif /* CONFIG_WPS */
#ifdef RTK_HAPD
