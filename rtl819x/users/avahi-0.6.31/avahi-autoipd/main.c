...
static int defend_confilcted=0;
    ...
    for (;;) {
		...
		defend_confilcted=0;
            ...
            if (packet_parse(in_packet, in_packet_len, &info) < 0)
                ...
                if (info.sender_ip_address == addr) {
                    ...
                    if (memcmp(hw_address, info.sender_hw_address, ETHER_ADDRLEN)) {
                        /* Normal conflict */
			//defend implement , mark_bct
			 if ((state == STATE_RUNNING || state == STATE_ANNOUNCING)&& (defend_confilcted<1))
			{
			 out_packet = packet_new_announcement(addr, hw_address, &out_packet_len);
			 
			//daemon_log(LOG_INFO, "state=%d DEFEND_INTERVAL IP address %s",state,inet_ntop(AF_INET, &addr, buf, sizeof(buf)));
           		 
			elapse_time(&next_wakeup, DEFEND_INTERVAL*1000, 0);
		         next_wakeup_valid = 1;
			 defend_confilcted++;				
			}
			else
			{
                        ...
                        daemon_log(LOG_INFO, "Received ARP packet back on source interface. Ignoring.");
                ...
                if (conflict) {
                    ...
                    //if (state == STATE_RUNNING || state == STATE_ANNOUNCING) //mark_bct
            ...
            /* The user requested a reannouncing of the address by a SIGHUP */
                      daemon_log(LOG_INFO, "Re-probing address.");
    	    out_packet = packet_new_probe(addr, hw_address, &out_packet_len);
            set_state(STATE_PROBING, 0, addr);
