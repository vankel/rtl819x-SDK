...
#define USB_MNT_DIR		"/mnt"
#define USB_MNT_FILE		"/tmp/usb/mnt_map" // file contains 32bit INT representing mounted paritions.
#else
#define USB_MNT_DIR		"/tmp/usb"
#define USB_MNT_FILE		USB_MNT_DIR"/mnt_map" // file contains 32bit INT representing mounted paritions.
#endif
	...
	*data = NULL;
	...
	/*
		...
		if( getInFlags("eth0", &flags)==1 )
		{
			...
			*data = strdup( "Error" );
	*/
		unsigned int instnum,mask;

		instnum = getUSBIFInstNum(name);
		mask = file_read_number(USB_MNT_FILE);
		printf("\ninstnum=%d,mask=%x\n",instnum,mask);
		//if((mask & (1 << instnum))!=0)
		if(mask!=0)
			*data = strdup( "Up" );
