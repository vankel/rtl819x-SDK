...
static int tcp_init_gsoap(struct soap*);
...
*/
		...
		if( n > 0x4000 )
		{
			//fprintf( stderr, "call sslWrite() with size 0x4000\n" );
			nwritten = sslWrite(soap->conn, (char*)s, 0x4000, &status);
		}
		else{
			//fprintf( stderr, "call sslWrite() with size 0x%x\n", n );
...
#ifdef OSK
//star:20100825 IPV6_RELATED
        if(nwritten<=0) soap_socket_errno = -nwritten;
//star:20100825 IPV6_RELATED END
#endif
}
...
#ifndef PALM_1
static int
tcp_init_gsoap(struct soap *soap)
{ soap->errmode = 1;
#ifdef WIN32
  if (tcp_done)
    return 0;
...
//star:20100825 IPV6_RELATED END
  ...
  { struct linger linger;
    ...
    linger.l_onoff = 1;
    linger.l_linger = 0;
    if( opt_ret=setsockopt((SOAP_SOCKET)fd, SOL_SOCKET, SO_LINGER, (char*)&linger, sizeof(struct linger)) )
    { 
#ifdef OSK
//star:20100825 IPV6_RELATED
      soap_socket_errno = -opt_ret;
//star:20100825 IPV6_RELATED END
#endif
      soap->errnum = soap_socket_errno;
  ...
  else if (soap->connect_flags && (opt_ret=setsockopt((SOAP_SOCKET)fd, SOL_SOCKET, soap->connect_flags, (char*)&set, sizeof(int))) )
  { 
#ifdef OSK
//star:20100825 IPV6_RELATED
      soap_socket_errno = -opt_ret;
//star:20100825 IPV6_RELATED END
  ...
  if (soap->keep_alive && (opt_ret=setsockopt((SOAP_SOCKET)fd, SOL_SOCKET, SO_KEEPALIVE, (char*)&set, sizeof(int))) )
  { 
#ifdef OSK
//star:20100825 IPV6_RELATED
      soap_socket_errno = -opt_ret;
//star:20100825 IPV6_RELATED END
  ...
  if (opt_ret=setsockopt((SOAP_SOCKET)fd, SOL_SOCKET, SO_SNDBUF, (char*)&len, sizeof(int)))
  { 
#ifdef OSK
//star:20100825 IPV6_RELATED
      soap_socket_errno = -opt_ret;
...
#endif //_SUPPORT_TR143_
  if (opt_ret=setsockopt((SOAP_SOCKET)fd, SOL_SOCKET, SO_RCVBUF, (char*)&len, sizeof(int)))
  { 
#ifdef OSK
//star:20100825 IPV6_RELATED
      soap_socket_errno = -opt_ret;
...
#ifdef TCP_NODELAY
  if (!(soap->omode & SOAP_IO_UDP) && (opt_ret=setsockopt((SOAP_SOCKET)fd, IPPROTO_TCP, TCP_NODELAY, (char*)&set, sizeof(int))) )
  { 
#ifdef OSK
//star:20100825 IPV6_RELATED
      soap_socket_errno = -opt_ret;
//star:20100825 IPV6_RELATED END
  ...
  if( strlen(soap->itfname) )
  {
...
#endif
	xprintfk("tcp_connect> set SO_BINDTODEVICE=%s\n", soap->itfname );
#if 0
	int ifindex = 0;
	ipa host = 0xffffffff;
	struct sockaddr_in src;
	xprintfk("%s:%d itfname=%s,ifindex=%d,host=%s\n",__FUNCTION__,__LINE__,soap->itfname,ifindex,getIPString(host));
	if(strcmp(soap->itfname,"all")==0)
		host = getDefaultGateway();
	else {
		if(((ifindex = ifGetIndexByIfName(soap->itfname))==0)
			||((host = getIpFromPif(ifindex))==FALSE)) {
			soap->errnum = soap_socket_errno;
			soap_set_sender_error(soap, tcp_error(soap), "get host failed in tcp_connect()", SOAP_TCP_ERROR);
			soap->fclosesocket(soap, (SOAP_SOCKET)fd);
			return SOAP_INVALID_SOCKET;
		}
	}
	...
	src.sin_port = 7557;
//star:20100825 IPV6_RELATED
	src.sin_addr.s_addr = host;
//star:20100825 IPV6_RELATED END
	//if(soap_bind(&c_soap, getIPString(host), 7557, 5)<0)
	if(bind( (SOAP_SOCKET)fd, (struct sockaddr*)&src, sizeof(struct sockaddr_in) ))
	{
		soap->errnum = soap_socket_errno;
		soap_set_sender_error(soap, tcp_error(soap), "bind failed in tcp_connect()", SOAP_TCP_ERROR);
		soap->fclosesocket(soap, (SOAP_SOCKET)fd);
		return SOAP_INVALID_SOCKET;
	}
	xprintfk("%s:%d itfname=%s,ifindex=%d,host=%s\n",__FUNCTION__,__LINE__,soap->itfname,ifindex,getIPString(host));
  ...
  {
    int nonblocking = 1;
    s_ioctl( (SOAP_SOCKET)fd, SIOCNBIO, &nonblocking, sizeof(int) );
  }
#else
  ...
  {
    int blocking = 0;
    s_ioctl( (SOAP_SOCKET)fd, SIOCNBIO, &blocking, sizeof(int) );
  }
#else
    fcntl((SOAP_SOCKET)fd, F_SETFL, fcntl((SOAP_SOCKET)fd, F_GETFL)&~O_NONBLOCK);
#endif
  ...
  { 
    ...
    { 
    	...
    	gettime(&soap->tcp_open_request_time);
    	//fprintf( stderr, "set tcp_open_request_time={%d,%d}\n", soap->tcp_open_request_time.tv_sec, soap->tcp_open_request_time.tv_usec );
    }
#endif //_SUPPORT_TR143_    
#ifdef WITH_IPV6
    if (opt_ret=connect((SOAP_SOCKET)fd, res->ai_addr, res->ai_addrlen))
#else
    if (opt_ret=connect((SOAP_SOCKET)fd, (struct sockaddr*)&soap->peer, sizeof(soap->peer)))
#endif
    { 
#ifdef OSK
//star:20100825 IPV6_RELATED
...
//star:20100825 IPV6_RELATED END
	...
	k = (SOAP_SOCKLEN_T)sizeof(soap->errnum);
        if (!(opt_ret=getsockopt((SOAP_SOCKET)fd, SOL_SOCKET, SO_ERROR, (char*)&soap->errnum, &k)) && !soap->errnum)	/* portability note: see SOAP_SOCKLEN_T definition in stdsoap2.h */
          break;
#ifdef OSK
//star:20100825 IPV6_RELATED
        soap_socket_errno = -opt_ret;
...
/*ping_zhang:20090209 START:patch tr143 from telefonica linux 2.6*/
  ...
  if(soap->get_state_for_tr143)
  {
	...
	gettime(&soap->tcp_open_response_time);
...
#endif
#ifndef WITH_LEAN
  if (soap->bind_flags && (opt_ret=setsockopt((SOAP_SOCKET)soap->master, SOL_SOCKET, soap->bind_flags, (char*)&set, sizeof(int))) )
  { 
#ifdef OSK
//star:20100825 IPV6_RELATED
    soap_socket_errno = -opt_ret;
//star:20100825 IPV6_RELATED END
  ...
  if (((soap->imode | soap->omode) & SOAP_IO_KEEPALIVE) && (opt_ret=setsockopt((SOAP_SOCKET)soap->master, SOL_SOCKET, SO_KEEPALIVE, (char*)&set, sizeof(int))) )
  { 
#ifdef OSK
//star:20100825 IPV6_RELATED
    soap_socket_errno = -opt_ret;
//star:20100825 IPV6_RELATED END
  ...
  if (opt_ret=setsockopt((SOAP_SOCKET)soap->master, SOL_SOCKET, SO_SNDBUF, (char*)&len, sizeof(int)))
  { 
#ifdef OSK
//star:20100825 IPV6_RELATED
    soap_socket_errno = -opt_ret;
//star:20100825 IPV6_RELATED END
  ...
  if (opt_ret=setsockopt((SOAP_SOCKET)soap->master, SOL_SOCKET, SO_RCVBUF, (char*)&len, sizeof(int)))
  {
#ifdef OSK
//star:20100825 IPV6_RELATED
    soap_socket_errno = -opt_ret;
...
#ifdef TCP_NODELAY
  if (!(soap->omode & SOAP_IO_UDP) && (opt_ret=setsockopt((SOAP_SOCKET)soap->master, IPPROTO_TCP, TCP_NODELAY, (char*)&set, sizeof(int))) )
  { 
#ifdef OSK
//star:20100825 IPV6_RELATED
    soap_socket_errno = -opt_ret;
...
#endif
#ifdef WITH_IPV6
/*star:20110119 START add to accept ipv4 connection*/
 int unset=0;
 if (opt_ret=setsockopt(soap->master, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&unset, sizeof(int)))
  {
#ifdef OSK
//star:20100825 IPV6_RELATED
    soap_socket_errno = -opt_ret;
...
/*star:20110119 END*/
  soap->errmode = 0;
  if (opt_ret=bind((SOAP_SOCKET)soap->master, res.ai_addr, res.ai_addrlen))
  { 
#ifdef OSK
//star:20100825 IPV6_RELATED
    soap_socket_errno = -opt_ret;
...
//star:20100825 IPV6_RELATED END
  ...
  soap->peer.sin_port = htons((short)port);
  soap->errmode = 0;
  if (opt_ret=bind((SOAP_SOCKET)soap->master, (struct sockaddr*)&soap->peer, soap->peerlen))
  { 
#ifdef OSK
//star:20100825 IPV6_RELATED
    soap_socket_errno = -opt_ret;
...
#endif
  if (!(soap->omode & SOAP_IO_UDP) && (opt_ret=listen((SOAP_SOCKET)soap->master, backlog)) )
  { 
#ifdef OSK
//star:20100825 IPV6_RELATED
    soap_socket_errno = -opt_ret;
...
//star:20100825 IPV6_RELATED
    if(r<0) soap_socket_errno=-r;
//star:20100825 IPV6_RELATED END
    //sfd & xfd set together, why????
    if (r > 0 
    	&& !( (FD_ISSET((SOAP_SOCKET)soap->socket, &rfd)!=0) ||  (FD_ISSET((SOAP_SOCKET)soap->socket, &sfd)!=0) ) 
    	&& FD_ISSET((SOAP_SOCKET)soap->socket, &xfd))
      r = -1;
#else
...
//star:20100825 IPV6_RELATED END
	...
	{ u_long nonblocking = 1;
        ...
        {
          int nonblocking = 1;
          s_ioctl( (SOAP_SOCKET)soap->master, SIOCNBIO, &nonblocking, sizeof(int) );
        }
#else
      ...
      {
        int blocking = 0;
        s_ioctl( (SOAP_SOCKET)soap->master, SIOCNBIO, &blocking, sizeof(int) );
      }
#else
        fcntl((SOAP_SOCKET)soap->master, F_SETFL, fcntl((SOAP_SOCKET)soap->master, F_GETFL)&~O_NONBLOCK);
#endif
...
/*Note: if enable the codes below, the program may stop and no error messages., why?*/
#if !defined(WITH_LEAN) && !defined(OSK)
...
struct soap_shutdowned_socket psoap_shutdowned_socket={0,0,0};

void soap_sdsocket_add(int sdsocket)
{
	struct soap_shutdowned_socket *sdtmp=psoap_shutdowned_socket.next;
	struct soap_shutdowned_socket *pbefore,*pnew;

	while(sdtmp){
		if(sdtmp->shutdownsocket==sdsocket)
			return;
		sdtmp=sdtmp->next;
	}

	pnew=(struct soap_shutdowned_socket*)malloc(sizeof(struct soap_shutdowned_socket));
	pnew->next=NULL;
	pnew->shutdownsocket=sdsocket;
	pnew->shutdowntime=0;
	sdtmp=psoap_shutdowned_socket.next;
	pbefore=&psoap_shutdowned_socket;
	while(sdtmp){
		pbefore=sdtmp;
		sdtmp=sdtmp->next;
	}
	pbefore->next=pnew;
	xprintfk("ssl connection shutdown socket %d\n",pnew->shutdownsocket);
}

void soap_sdsocket_check(struct soap *soap)
{
	struct soap_shutdowned_socket *sdtmp=psoap_shutdowned_socket.next;
	struct soap_shutdowned_socket *pbefore=&psoap_shutdowned_socket;

	while(sdtmp){
		sdtmp->shutdowntime++;
		if((sdtmp->shutdownsocket!=soap->socket) && (sdtmp->shutdowntime>=3)){ //10 secs
			xprintfk("close shutdowned socket %d\n",sdtmp->shutdownsocket);
			soap->fclosesocket(soap,(SOAP_SOCKET)sdtmp->shutdownsocket);
			pbefore->next=sdtmp->next;
			free(sdtmp);
			sdtmp=pbefore->next;
			continue;
		}
...
{ if (soap_element_begin_in(soap, tag, 0))
  ...
  { if (soap_s2int(soap, soap_value(soap), p))
  	{
  	/*ping_zhang:20090408 START:continue process and call soap_element_end_in()*/
	soap_element_end_in(soap, tag);
	/*ping_zhang:20090408 END*/
	return NULL;
  	}
  }
...
extern int zfclose(FILE *pf);
...
#elif defined(OSK)
					if( xfindfile(soap->tmpbuf) )
					{
						fp = xfopen( soap->tmpbuf, "r" );
						if(fp)
						{
							int read_size;
							char outbuf[1024];

							while( read_size=xfread(outbuf, 1, 1023, fp) )
							{
								outbuf[read_size]=0;
								//fprintf( stderr, "content:%s\n", outbuf );
								if( soap_send_raw(soap, outbuf, strlen(outbuf)) )
					...
					if(fp)
					{
						while( feof(fp)==0 )
						{
							...
							read_size = fread( outbuf, 1, 1023, fp );
							outbuf[read_size]=0;
							//fprintf( stderr, "content:%s\n", outbuf );
							if( soap_send_raw(soap, outbuf, strlen(outbuf)) )
							{
...
{ if (s)
  { struct tm T;
    ...
    if (strchr(s, '-'))
#ifdef OSK
      t = "%ld-%ld-%ldT%ld:%ld:%ld%31s";
#else
      t = "%d-%d-%dT%d:%d:%d%31s";
#endif
    else if (strchr(s, ':'))
#ifdef OSK
      t = "%4ld%2ld%2ldT%ld:%ld:%ld%31s";
#else
      t = "%4d%2d%2dT%d:%d:%d%31s";
#endif
    else /* parse non-XSD-standard alternative ISO 8601 format */
#ifdef OSK
      t = "%4ld%2ld%2ldT%2ld%2ld%2ld%31s";
#else
...
{ register int c;
  ...
  while ((c = *s++) && --n > 0)
  {
/*star:20090925 START Motive ACS server can't realize %x type of [ and ]*/
  	// if (c > ' ' && c < 128 && !strchr("()<>@,;:\\\"/[]?={}", c))
   	if (c > ' ' && c < 128 && !strchr("()<>@,;:\\\"/?={}", c))
/*star:20090925 END*/
      *t++ = c;
  ...
  { s = soap_decode_key(tmp, sizeof(tmp), s);
    ...
    { struct tm T;
      ...
      { memset((void*)&T, 0, sizeof(T));
        ...
        a[1] = tmp[9];
#ifdef OSK
        //osk,  invalid operands to binary -
        T.tm_mon = (  (unsigned int *)strstr(mns, a) - (unsigned int *)mns) / 2;
#else
...
/******************************************************************************/
//osk, undefined reference to `getenv'
#ifndef OSK
SOAP_FMAC1
int
SOAP_FMAC2
