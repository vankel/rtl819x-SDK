...
int main(int argc, char *argv[])
{
	...
	int InitValue=0;
	char fileName[64]={0};
	static const char short_options[] = "hnlLD:qt:c:f:r:d:x:MNF:A:R:T:B:vV:IPCi";
			...
			if (interleaved) {
				if (optind > argc - 1) {
					if (stream == SND_PCM_STREAM_PLAYBACK){
						//printf("%s:%d, optind=%d, argc=%d\n", __FUNCTION__, __LINE__,optind,argc);
						playback(NULL);
					}else
						capture(NULL);
				} else {
...
static ssize_t test_wavefile(int fd, u_char *_buffer, size_t size)
{
	...
	///fprintf(stderr, "\n\n\n%s:%d, fmt_bytes=%d\n\n\n", __FUNCTION__, __LINE__, LE_SHORT(f->bit_p_spl));
...
{
	off64_t count;
//printf("%s:%d,timelimit=%d\n", __FUNCTION__, __LINE__,timelimit);
	if (timelimit == 0) {
		count = pbrec_count;
	} else {
...
//	printf("%s:%d,count=%d, pbrec_count=%d\n", __FUNCTION__, __LINE__,count, pbrec_count);
...
static void playback_go(int fd, size_t loaded, off64_t count, int rtype, char *name)
{
	...
	//fprintf(stderr, "%s,chunk_bytes=%d, chunk_size=%d\n", __FUNCTION__,chunk_bytes,chunk_size);
	...
	#if 0	
		if(NeedSwap==1){
			//fprintf(stderr, "%s,DoByteSwap\n", __FUNCTION__);
			SwapStart =(unsigned short *)(audiobuf);
			for(i=0;i<(l/2);i++){
				*SwapStart=WORD_SWAP(*SwapStart);
				SwapStart++;
			}
		}
	#endif	
