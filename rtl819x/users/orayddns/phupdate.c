...
int RecvKeepaliveResponse(PHGlobal *phglobal)
{
	...
	LOG(("RecvKeepaliveResponse() Data comes, OPCODE:%d\n"),data.lOpCode);
...
int ExecuteUpdate(PHGlobal *phglobal)
{
	...
	if (len <=0 )
	{
		LOG("ExecuteUpdate Recv server hello string failed.\n");
		phClose(&phglobal->m_tcpsocket);
		phglobal->nAddressIndex++;
		return errorConnectFailed;
	}

	LOG(("SEND AUTH REQUEST COMMAND..."));
	...
	if (len <=0 )
	{
		LOG(("ExecuteUpdate Recv server key string failed.\n"));
		phClose(&phglobal->m_tcpsocket);
		return errorConnectFailed;
	}
    LOG(("SERVER SIDE KEY \"%s\" RECEIVED.\n"),buffer);
	...
	if (len <=0 )
	{
		LOG(("ExecuteUpdate Recv server auth response failed.\n"));
		phClose(&phglobal->m_tcpsocket);
	...
	{
		LOG("CTcpThread::ExecuteUpdate auth failed.\n");
		phClose(&phglobal->m_tcpsocket);
	...
	if (totaldomains<=0)
	{
		LOG(("ExecuteUpdate Domain List Failed.\n"));
		phClose(&phglobal->m_tcpsocket);
	...
	for (i=0;i<totaldomains-1;i++)
    {
		...
		if (len <= 0)
		{
			LOG(("ExecuteUpdate Recv server confirm response failed.\n"));
			phClose(&phglobal->m_tcpsocket);
	...
	if (len <= 0)
	{
		LOG(("ExecuteUpdate Recv server confirmed chatID response failed.\n"));
		phClose(&phglobal->m_tcpsocket);
		return errorDomainRegisterFailed;
	}
	LOG(("%s\n"),buffer);
	...
	LOG(("ExecuteUpdate nChatID:%d, nStartID:%d\n"),phglobal->nChatID,phglobal->nStartID);
	//////////////////////////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////
	...
	{
		LOG("CTcpThread::ExecuteUpdate Recv server confirmed stat user response failed.\n");
		phClose(&phglobal->m_tcpsocket);
	...
	if (len <= 0)
	{
		LOG(("ExecuteUpdate Recv server goodbye response failed.\n"));
		phClose(&phglobal->m_tcpsocket);
...
int phddns_step(PHGlobal *phglobal)
{
	...
	if (phglobal->bNeed_connect)
	{
		...
		{
			LOG("InitializeSockets failed, waiting for 5 seconds to retry...\n");
		...
		if (ret == okDomainsRegistered) 
		{
			...
			LOG("ExecuteUpdate OK, BeginKeepAlive!\n");
		...
		else 
		{
			if (ret == okRedirecting)
			{
				...
				LOG("Need redirect, waiting for 5 seconds...\n");
				return 5;
			}
			if (ret == errorAuthFailed)
			{
				LOG("ExecuteUpdate AuthFailed, waiting for 600 seconds to retry...\n");
				return 600;
			}
			
			LOG("ExecuteUpdate failed, waiting for 30 seconds to retry...\n");
			return 30;
		}
	...
	else
	{
		...
		{
			LOG("RecvKeepaliveResponse failed, waiting for 30 seconds to reconnect...\n");
			phglobal->bNeed_connect = TRUE;
			phglobal->bTcpUpdateSuccessed = FALSE;
			return 30;
		}
		else
		{
			if (ret == okKeepAliveRecved)
			{
				...
				LOG("Keepalive response received, client ip: %s\n",inet_ntoa(t));
		...
		{
			LOG("No response from server for %d seconds, reconnect immediately...\n", (phglobal->nUserType == 1 ? 160 : 320));
			phglobal->bTcpUpdateSuccessed = FALSE;
			phglobal->bNeed_connect = TRUE;
			return 1;
		}
