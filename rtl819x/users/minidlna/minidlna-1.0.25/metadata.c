...
GetImageMetadata(const char * path, char * name)
{
    ...
    // Added by Paula. 2011/11/02
	// Mason Yu. Remove jpeg and exif lib
	//if( ends_with(path, ".gif") || ends_with(path, ".png"))
	{
		if(ends_with(path, ".gif"))
			asprintf(&m.mime, "image/gif");
		else if (ends_with(path, ".jpeg") || ends_with(path, ".jpg"))
			asprintf(&m.mime, "image/jpeg");
		else 
			asprintf(&m.mime, "image/png");			

		m.date = NULL;		
		asprintf(&m.resolution, "30x30");		
		ret = sql_exec(db, "INSERT into DETAILS"
                   " (PATH, TITLE, SIZE, TIMESTAMP, RESOLUTION, MIME) "
                   "VALUES"
                   " (%Q, '%q', %lld, %ld, %Q, %Q);",
                   path, name, file.st_size, file.st_mtime, m.resolution, m.mime);
...
no_exifdata:
	...
	return ret;
#endif //Mason Yu. Remove jpeg and exif lib
}
#define SIMPLE_DLNA
sqlite_int64
GetVideoMetadata(const char * path, char * name)
{
	...
	if(ends_with(path, ".rmvb") || ends_with(path, ".rm"))
	{
...
#endif
#else
	if(ends_with(path, ".asf")) 		
		asprintf(&m.mime, "video/x-ms-asf");
	else if(ends_with(path, ".avc")
		||ends_with(path, ".avi")
		||ends_with(path, ".divx")
		)		
			asprintf(&m.mime, "video/avi");
	else if(ends_with(path, ".dv"))	
		asprintf(&m.mime, "video/x-dv");
	else if(ends_with(path, ".wmv"))	
		asprintf(&m.mime, "video/x-ms-wmv");
	else if(ends_with(path, ".mjpg")
		||ends_with(path, ".mjpeg")
		)	
		asprintf(&m.mime, "video/x-motion-jpeg");
		...
		asprintf(&m.mime, "video/mpeg");
	else if(ends_with(path, ".mp2p")
		||ends_with(path, ".vob")
		)	
		asprintf(&m.mime, "video/mp2p");
	else if(ends_with(path, ".mp2t"))	
		asprintf(&m.mime, "video/mp2t");
	else if(ends_with(path, ".m2v")
		||ends_with(path, ".mpg2")
		||ends_with(path, ".mpeg2")
		||ends_with(path, ".ts")
		||ends_with(path, ".bin")
		||ends_with(path, ".iso")
		)		
		asprintf(&m.mime, "video/mpeg2");
	else if(ends_with(path, ".m4v")
		||ends_with(path, ".m4p")
		||ends_with(path, ".mp4")
		)	
		asprintf(&m.mime, "video/mp4");
	else if(ends_with(path, ".mp4ps"))	
		asprintf(&m.mime, "video/x-nerodigital-ps");
	else if(ends_with(path, ".mov")
		||ends_with(path, ".hdmov")
		||ends_with(path, ".qt")
		)	
		asprintf(&m.mime, "video/quicktime");
