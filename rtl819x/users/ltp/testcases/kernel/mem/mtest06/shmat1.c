...
sig_handler (int signal,        /* signal number, set to handle SIGALRM       */
             int code, struct ucontext *ut)     /* contains pointer to sigcontext structure   */
{
#ifdef __i386__
  ...
  switch (except)
    {
    case 10:
      fprintf (stderr, "Exception - invalid TSS, exception #[%ld]\n", except);
      break;
    case 11:
      fprintf (stderr,
    ...
    case 13:
      ...
      break;
    case 14:
      fprintf (stderr, "Exception - page fault, exception #[%ld]\n", except);
      ret = 1;
      break;
    default:
      fprintf (stderr,
  ...
  if (ret)
    {
      if (scp->edi == (int) map_address)
        {
          fprintf (stdout, "page fault at [%#lx] - ignore\n", scp->edi);
          siglongjmp (jmpbuf, 1);
        }
  ...
  while (shm_ndx++ < (int) locargs[0])
    {
      ...
      if ((map_address = shmat (shmid, (void *) 0, 0)) == (void *) -1)
        {
  ...
  for (;;)
    {
      ...
      if (pthread_create (&thid[0], NULL, shmat_shmdt, chld_args))
        ...
        fprintf (stdout, "created thread id[%#lx], execs fn shmat_shmdt()\n",
                 thid[0]);
      ...
      if (pthread_create (&thid[1], NULL, write_to_mem, chld_args))
        ...
        fprintf (stdout, "created thread id[%#lx], execs fn write_to_mem()\n",
                 thid[1]);
      ...
      if (pthread_create (&thid[2], NULL, read_from_mem, chld_args))
        ...
        fprintf (stdout, "created thread id[%#lx], execs fn read_from_mem()\n",
                 thid[2]);
