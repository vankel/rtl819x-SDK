...
usage(char *progname)           /* name of this program                       */{
    fprintf(stderr,
	       ...
	       "\t -a set map_flags to MAP_ANONYMOUS\n"
               ...
               "\t -s size of the file/memory to be mmaped.\tdefault: 1MB\n"
     ...
     char **argv)	    /* pointer to the command line arguments.         */
{
    ...
    int   fsize = 1;	    /* size of the temp file created. default 1MB     */
    ...
    while ((c =  getopt(argc, argv, "ahps:x:")) != -1)
    {
		...
		if ((fsize = atoi(optarg)) == 0)
		    fprintf(stderr, "Using default fsize %d MB\n", fsize = 1);
		break;
            case 'x':
