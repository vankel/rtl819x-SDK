...
#endif
static int cmd_sitesurvey(int index,int argc, char *argv[]);
...
struct inband_cmd_table_t inband_cmd_table[]=
{
...
#ifdef HOST_SEND_CONFIG_FILE
    {id_send_config_file,"send_config_file",     cmd_send_config_file,   NULL,"inband send_config_file /tmp/xxx.conf"},
    {id_get_config_file,"get_config_file",     cmd_get_config_file,   NULL,"inband get_config_file all/wlan/hw"},
#endif    
    ...
    {id_request_scan,"request_scan",     cmd_sitesurvey,   NULL,"inband request_scan wlan_interface"},
	{id_get_scan_result,"get_scan_result",     cmd_read,  print_scan_result, "inband get_scan_result wlan_interface"},
...
static int host_firmware_write(char *data,int len)
{
  ...
  char *buf_p;
   ...
   ret = inband_rcv_data_and_seq(hcd_inband_chan,&rx_seq,&rx_cmd_type,&buf_p,20000*4); //return data length 	
...
static int cmd_sitesurvey(int index,int argc, char *argv[])
{
	...
	if(ret < 0)
		return -1;
	count = inband_rcv_data_and_seq(hcd_inband_chan,&rx_seq,&rx_cmd_type,&buf_p,1000);
	if(count < 0)
	 	return -1;
...
int cmd_send_config_file(int index,int argc, char *argv[])
{
	...
	if(argc > 3)//enable or disable config file
	{
