...
int mld_inf_create(char *ifname)
{
	...
	{	
		...
		ra[2] = 0;
		ra[3] = 0;
		setsockopt(dp->sock, IPPROTO_IPV6, IP_OPTIONS, ra, 4);//IP_OPTIONS’‚∏ˆ—°œÓ??
	}
	#endif
	/* init mld */	
...
//make packet
   ...
   //mldv2			  = (struct mld2_query *)mld6_send_buf;
	...
	//mldv1->mld_cksum	=in_cksum((u_short *)mldv1, MLD_MINLEN); //MLD_MINLEN ”¶∏√…Ë÷√Œ™∂‡…Ÿ??
#endif
...
void sigifup(int signum)
{
	...
	//ioctlªÒ»°interfaceµƒ–≈œ¢ipv6?÷ª  ”√”⁄ipv4,ipv6ø…ƒ‹≤ªƒ‹ªÒ»°
