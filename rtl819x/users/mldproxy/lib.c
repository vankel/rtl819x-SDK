...
*/
{
  sprintf( St, "%u.%u.%u.%u.%u.%u.%u.%u", 
	   ((uint8 *)&InAdr.s6_addr)[ 0 ],
	   ((uint8 *)&InAdr.s6_addr)[ 1 ],
	   ((uint8 *)&InAdr.s6_addr)[ 2 ],
	   ((uint8 *)&InAdr.s6_addr)[ 3 ], 
	   ((uint8 *)&InAdr.s6_addr)[ 4 ],
	   ((uint8 *)&InAdr.s6_addr)[ 5 ],
	   ((uint8 *)&InAdr.s6_addr)[ 6 ],
	   ((uint8 *)&InAdr.s6_addr)[ 7 ] 
	   );
...
*/
{
  ...
  if( SockAdrPt->sin_family != AF_INET6 )
    log( LOG_ERR, 0, "fmtSockAdr: not an INET6 address" );
