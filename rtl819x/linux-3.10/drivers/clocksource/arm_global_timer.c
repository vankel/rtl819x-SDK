...
#include <clocksource/arm_global_timer.h>
...
static void gt_update_frequency(void *new_rate)
{
	...
	clockevents_update_freq(__this_cpu_ptr(gt_evt), gt_clk_rate);
}
...
{
	if (gt_evt && __this_cpu_ptr(gt_evt) && !IS_ERR(gt_clk))
		return clk_notifier_register(gt_clk, &gt_clk_nb);
	return 0;
}
core_initcall(gt_clk_init);

static void gt_get_clock(void)
{
	int err;

	gt_clk = clk_get_sys("smp_ca9", NULL);

	if (IS_ERR(gt_clk)) {
		pr_err("smp_ca9: clock not found %d\n", (int) PTR_ERR(gt_clk));
		return;
	}

	err = clk_prepare_enable(gt_clk);
	if (err) {
		pr_err("smp_ca9: clock failed to prepare+enable: %d\n", err);
		clk_put(gt_clk);
		return;
	}
...
#else
int __init global_timer_register(struct gt_timer_device *gdev)
{
	int err = 0;
	...
	gt_ppi = gdev->res[1].start;
	if (!gt_ppi) {
		pr_warn("global-timer: unable to parse irq\n");
		return -ENODEV;
	}

	gt_base = ioremap(gdev->res[0].start, resource_size(&gdev->res[0]));
	if (!gt_base) {
		pr_warn("global-timer: invalid base address\n");
		return -ENODEV;
	}
