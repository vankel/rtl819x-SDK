...
#if 0
int yaffs_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, yaffs_proc_read, NULL));
}
static ssize_t yaffs_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return yaffs_proc_write(file, userbuf,count, off);
}
struct file_operations yaffs_fops = {
        .open           = yaffs_single_open,
        .write		    = yaffs_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
