...
#if defined(CONFIG_SW_8367R) || defined(CONFIG_SW_8370) || defined(CONFIG_SW_8367MB)
#include <asm/delay.h>
#endif
...
#if defined(CONFIG_SW_8367R) || defined(CONFIG_SW_8367MB)
...
#define RESET_CNR		PABCDCNR
#define RESET_DIR		PABCDDIR
#define RESET_DAT		PABCDDAT
...
#elif defined(CONFIG_SW_8367MB)
#define GPIO_PINMUX_MASK	(3<<10)
#define GPIO_RESET	19	//GPIO_G3
#define RESET_CNR		PEFGHCNR
#define RESET_DIR		PEFGHDIR
#define RESET_DAT		PEFGHDAT
...
int32 swCore_init()
{
	...
	for (i=0; i<3; i++) 
	{
		...
		// MDC: F5, MDIO: F6
		WRITE_MEM32(PEFGHCNR, READ_MEM32(PEFGHCNR) & (~I2C_PINMUX_MASK));	//set GPIO pin
		WRITE_MEM32(PEFGHDIR, READ_MEM32(PEFGHDIR) | (I2C_PINMUX_MASK)); //output pin
	...
	REG32(PABCDCNR) &= (~(0x00000002)); //set GPIO pin, A1
	REG32(PABCDDIR) |= (0x00000002); //output pin

	for (i=0; i<3; i++) 
	{
		// for 8367r h/w reset pin
		{
		REG32(PABCDDAT) &= (~(0x00000002)); 
		__mdelay(1000);
		REG32(PABCDDAT) |= (0x00000002); 
		__mdelay(1000);
		}
...
//		WRITE_MEM32(PEFGHDIR, READ_MEM32(PEFGHDIR) | ((0x00006000))); //output pin
		...
		//MA21(GPIOC4) => IO
		// MDC: C5, MDIO: C4
		WRITE_MEM32(PABCDCNR, READ_MEM32(PABCDCNR) & (~(0x00300000)));	//set GPIO pin, C5 and C4
		WRITE_MEM32(PABCDDIR, READ_MEM32(PABCDDIR) | ((0x00300000))); //output pin
...
//		REG32(PCRP0) = REG32(PCRP0) | (0 << ExtPHYID_OFFSET) | AcptMaxLen_16K | EnablePHYIf | MacSwReset;   //move to below
		...
		P0phymode=2;
		P0miimode=2;
	#elif defined(CONFIG_SW_P0_RGMII) || defined(CONFIG_SW_8367R) || defined(CONFIG_SW_8370) || defined(CONFIG_SW_8367MB)
		P0phymode=3;
		P0miimode=3;
	...
	{
		REG32(PCRP0) = (REG32(PCRP0) & ~(ExtPHYID_MASK)) | ((P0_EXT_PHY_ID << ExtPHYID_OFFSET) | MIIcfg_RXER |  EnablePHYIf | MacSwReset);	//external
		{
		int reg;

		// enable flow control ability
		rtl8651_getAsicEthernetPHYReg(P0_EXT_PHY_ID, 4, &reg );
		reg |= (BIT(10) | BIT(11));		
		rtl8651_setAsicEthernetPHYReg( P0_EXT_PHY_ID, 4, reg );
		}
	...
	*/	
#if defined(CONFIG_SW_8367R) || defined(CONFIG_SW_8370)
