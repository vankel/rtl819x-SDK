...
{	
	...
	src = strtoul((const char*)(argv[0]), (char **)NULL, 16);	
	ret = rtl8370_getAsicReg(src, &value); 
			
	if(ret==0)
		dprintf("rtl8370_getAsicReg: reg= %x, data= %x\n", src, value);
	else
		dprintf("get fail %d\n", ret);
...
{	
	...
	ret = rtl8370_setAsicReg(src, value); 
			
	if(ret==0)
		dprintf("rtl8370_setAsicReg: reg= %x, data= %x\n", src, value);
	else
		dprintf("set fail %d\n", ret);
