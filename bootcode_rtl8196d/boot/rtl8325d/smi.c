...
__inline__ void __nano_delay__(unsigned long nsecs)
{
	...
	lo = (((nsecs * 0x000010c6) >> 12) * ((loops_per_jiffy * 100) >> 20)) / 1000;
	__delay(lo);
}

//#define __mdelay__(x) { int i=x; while(i--) __udelay(1000, loops_per_jiffy * 100); }

//#define __udelay__(x) { __udelay(x, loops_per_jiffy * 100); }

//#define CLK_DURATION(clk)            { __udelay__(1); }
